...
class Brizy_Editor_Post extends Brizy_Editor_Entity {
	...
	private function getPostContent( $noFilters ) {
		...
		if ( $noFilters ) {
			...
			$context             = Brizy_Content_ContextFactory::createContext(
			...
			list( $placeholders, $placeholderInstances, $content ) = $extractor->extract(
				$this->get_compiled_page()->getPageContent()
			);
	...
	protected function loadInstanceData() {
		...
		$this->setTitle( get_the_title( $this->getWpPostId() ) );// check for deprecated forms of posts
		if ( $storage_post instanceof self ) {
			...
			$this->dependencies       = isset( $storage_post['dependencies'] ) ? $storage_post['dependencies'] : [];
		}
	}
	...
	protected function populateAutoSavedData( $autosave ) {
		...
		$autosave->setDependencies( $this->getDependencies() );
