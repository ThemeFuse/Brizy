...
class Brizy_Editor_Post extends Brizy_Editor_Entity
{
    ...
    public static function mark_all_for_compilation()
    {
        ...
        $wpdb->update(
            $wpdb->postmeta,
            array('meta_value' => 1),
            array('meta_key' => self::BRIZY_POST_NEEDS_COMPILE_KEY)
        );
    }
    ...
    public function savePost($createRevision = false)
    {
        ...
        $content = $this->get_compiled_page()->get_body() ? $this->get_compiled_page()->get_body(
    ...
    public function compile_page()
    {
        ...
        foreach ($compiledData['pageScripts'] as $i => $set) { //pro || free
            foreach ($set as $k => $scripts) { // groups
                ...
                if ($k == 'main') {
                    $compiledData['pageScripts'][$i][$k]['content'] = Brizy_SiteUrlReplacer::hideSiteUrl(
                        $compiledData['pageScripts'][$i][$k]['content']
                    );
                    continue;
                }

                foreach ($scripts as $l => $script) {
                    $compiledData['pageScripts'][$i][$k][$l]['content'] = Brizy_SiteUrlReplacer::hideSiteUrl(
                        $script['content']
                    );
                }
        ...
        foreach ($compiledData['pageStyles'] as $i => $set) {
            foreach ($set as $k => $styles) {
                ...
                if ($k == 'main') {
                    $compiledData['pageStyles'][$i][$k]['content'] = Brizy_SiteUrlReplacer::hideSiteUrl(
                        $compiledData['pageStyles'][$i][$k]['content']
                    );
                    continue;
                }
    ...
    protected function loadInstanceData()
    {
        ...
        if ($storage_post instanceof self) {
            ...
            $this->set_needs_compile(
                metadata_exists('post', $this->getWpPostId(), self::BRIZY_POST_NEEDS_COMPILE_KEY) ? (bool)get_post_meta(
                    $this->getWpPostId(),
                    self::BRIZY_POST_NEEDS_COMPILE_KEY,
                    true
                ) : $data_needs_compile
            );
            ...
            $this->set_plugin_version(
                isset($storage_post['plugin_version']) ? $storage_post['plugin_version'] : BRIZY_VERSION
            );
