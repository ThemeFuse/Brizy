...
class Brizy_Editor_Editor_Editor
{
    ...
    public function config()
    {
        ...
        $config = $this->addFunnelData($config);
    ...
    private function getTemplateType($template_rules)
    {
        foreach ($template_rules as $rule) {
            ...
            if (in_array(
				...
				Brizy_Admin_Rule::YEAR_ARCHIVE,Brizy_Admin_Rule::TAXONOMY,
                        Brizy_Admin_Rule::WOO_SHOP_PAGE,
                    ]
                ) ) {
               if ( $rule->getAppliedFor() == Brizy_Admin_Rule::WOO_SHOP_PAGE && in_array($rule->getEntityType(), ['product', 'shop_page'
                ] ) ) {
					return 'product_archive';
				} else {
					return 'archive';
    ...
    public function getApiActions()
    {
        ...
        return array(
            ...
            'placeholderContent'         => $pref.Brizy_Editor_API::AJAX_PLACEHOLDER_CONTENT,'placeholdersContent'         => $pref . Brizy_Editor_API::AJAX_PLACEHOLDERS_CONTENT,
            ...
            'getFunnelPosts'             => $pref.Brizy_Admin_Funnels_Api::GET_FUNNEL_POSTS,
            'createFunnelPost'           => $pref.Brizy_Admin_Funnels_Api::CREATE_FUNNEL_POST,
            'updateFunnelPost'           => $pref.Brizy_Admin_Funnels_Api::UPDATE_FUNNEL_POST,
            'deleteFunnelPost'           => $pref.Brizy_Admin_Funnels_Api::DELETE_FUNNEL_POST,
        );
    }

    public function addFunnelData($config)
    {
        if (Brizy_Admin_Funnels_Manager::isSupportedPostType($config['wp']['post_type'])) {
            $config['funnel'] = [
                'parentId' => $this->post->getWpPostParentId(),
            ];
        }
