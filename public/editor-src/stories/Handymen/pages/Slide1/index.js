module.exports = {
  id: "HandymenSlide1",
  thumbnailWidth: 437,
  thumbnailHeight: 728,
  title: "",
  keywords: "",
  cat: [0, 5],
  resolve: {
    blocks: [
      {
        type: "StoryItem",
        value: {
          _styles: ["story-item"],
          items: [
            {
              type: "StoryWrapper",
              value: {
                _styles: ["wrapper--story", "wrapper--story-richText"],
                items: [
                  {
                    type: "RichText",
                    value: {
                      _styles: ["story-richText"],
                      _id: "sthihagsyjisvllsgclmoydoriyovoaegkfc",
                      text:
                        '<p class="brz-text-lg-left brz-ff-dm_serif_display brz-ft-google brz-fw-lg-400 brz-fw-sm-im-400 brz-fw-xs-im-400 brz-fs-lg-50 brz-fs-sm-im-49 brz-fs-xs-im-49 brz-ls-lg-m_2 brz-ls-sm-im-m_1 brz-ls-xs-im-m_1 brz-lh-lg-1_1 brz-lh-sm-im-1_1 brz-lh-xs-im-1_1"><span style="color: rgb(52, 46, 54);">Find the best handymen in your area</span></p>',
                      width: 61,
                      offsetY: 11,
                      offsetX: 10
                    }
                  }
                ],
                _id: "waabohzcwklluqrnxcelwrqztlkvgefuejak"
              }
            },
            {
              type: "StoryWrapper",
              value: {
                _styles: ["wrapper--story", "wrapper--story-button"],
                items: [
                  {
                    type: "Button",
                    value: {
                      _styles: ["story-button"],
                      _id: "njgspznpqnxjexiubxbysnzgiagfmspmrznk",
                      offsetY: 75,
                      offsetX: 17,
                      bgColorPalette: "",
                      bgColorHex: "#ff6158",
                      fontFamily: "lato",
                      fontFamilyType: "google",
                      fontStyle: "",
                      fontSize: 12,
                      fontWeight: 700,
                      letterSpacing: 2.5,
                      lineHeight: 1.6,
                      text: "SWIPE TO GET STARTED",
                      width: 66,
                      height: 20,
                      borderRadiusType: "custom",
                      tempBorderRadiusType: "custom",
                      paddingRight: 0,
                      paddingLeft: 0,
                      fillType: "filled",
                      borderRadius: 2,
                      borderWidth: 0,
                      borderColorOpacity: 0,
                      borderColorPalette: 0,
                      hoverBgColorOpacity: 0.8,
                      hoverBorderColorOpacity: 0.8,
                      tempBorderWidth: 2
                    }
                  }
                ],
                _id: "eqhrsfindieuaitafhizlarhyzbhtnliobbf"
              }
            },
            {
              type: "StoryWrapper",
              value: {
                _styles: ["wrapper--story", "wrapper--story-richText"],
                items: [
                  {
                    type: "RichText",
                    value: {
                      _styles: ["story-richText"],
                      _id: "fpsdalmxhuackvfxdtxgbkhmbmrwuqgeljma",
                      text:
                        '<p class="brz-ls-lg-0 brz-ls-sm-im-0 brz-ls-xs-im-0 brz-ff-dm_serif_display brz-ft-google brz-fw-lg-400 brz-fw-sm-im-400 brz-fw-xs-im-400 brz-fs-lg-17 brz-fs-sm-im-16 brz-fs-xs-im-16 brz-lh-lg-1_7 brz-lh-sm-im-1_4 brz-lh-xs-im-1_4"><span style="color: rgb(255, 97, 88);">2300+ available professionals</span></p>',
                      offsetY: 4,
                      offsetX: 11
                    }
                  }
                ],
                _id: "fptchwouckkgdusjkzirukzrqhnsrvcahhae"
              }
            },
            {
              type: "StoryWrapper",
              value: {
                _styles: ["wrapper--story", "wrapper--story-icon"],
                items: [
                  {
                    type: "Icon",
                    value: {
                      _styles: ["story-icon"],
                      _id: "uxsfwfhoggejauvzzxrfppjkgcrbvjmocmly",
                      offsetY: 4,
                      offsetX: 82,
                      name: "settings",
                      type: "outline",
                      customSize: 8,
                      size: "custom",
                      colorPalette: "",
                      colorHex: "#342e36"
                    }
                  }
                ],
                _id: "paltbgdzdpaamfvvawpoedzluinajdcxgtya"
              }
            }
          ],
          _id: "gjhknaxchxbbadoptaqarsutkjwqxvkcvtnm",
          bgImageWidth: 1080,
          bgImageHeight: 1920,
          bgImageExtension: "jpg",
          bgImageSrc: "58ac80ef6c5537d8e0601b3cb7f06fdb.jpg",
          bgColorOpacity: 0,
          tempBgColorOpacity: 0.9,
          borderRadius: 0,
          borderTopLeftRadius: 0,
          borderTopRightRadius: 0,
          borderBottomLeftRadius: 0,
          borderBottomRightRadius: 0,
          tempBorderTopLeftRadius: 0,
          tempBorderTopRightRadius: 0,
          tempBorderBottomLeftRadius: 0,
          tempBorderBottomRightRadius: 0,
          bgColorType: "none",
          tempBgColorType: "solid",
          bgColorHex: "#ffffff",
          bgColorPalette: "",
          tempBgColorPalette: "",
          gradientColorHex: "#009900",
          gradientColorOpacity: 1,
          tempGradientColorOpacity: 1,
          gradientColorPalette: "",
          tempGradientColorPalette: "",
          gradientType: "linear",
          gradientStartPointer: 0,
          gradientFinishPointer: 100,
          gradientActivePointer: "startPointer",
          gradientLinearDegree: 90,
          gradientRadialDegree: 90,
          bgPositionX: 45,
          bgPositionY: 0,
          contrast: 106,
          saturation: 109,
          brightness: 102
        }
      }
    ]
  }
};
