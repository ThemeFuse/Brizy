...
import { AddImageData } from "../types/Media";
...
export function uploadImage(imageFile: File, options?: Partial<Options>): void {
  const { acceptedExtensions, onBase64, onUpload, onError }: Options =
...
export const uploadImagePromise = (
  image: File,
  extensions: string[]
): Promise<AddImageData> => {
  return new Promise<AddImageData>((res, rej) => {
    uploadImage(image, {
