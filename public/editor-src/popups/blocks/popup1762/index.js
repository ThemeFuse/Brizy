module.exports = {
  id: "popup1762",
  thumbnailWidth: 600,
  thumbnailHeight: 377,
  title: "popup1762",
  
  keywords: "",
  cat: [1493],
  type: 0,
  pro: true,
  resolve: {
    "type": "SectionPopup2",
    "value": {
        "_styles": [
            "sectionPopup2"
        ],
        "items": [
            {
                "type": "Row",
                "value": {
                    "_styles": [
                        "row"
                    ],
                    "items": [
                        {
                            "type": "Column",
                            "value": {
                                "_styles": [
                                    "column"
                                ],
                                "items": [
                                    {
                                        "type": "Wrapper",
                                        "value": {
                                            "_styles": [
                                                "wrapper",
                                                "wrapper--richText"
                                            ],
                                            "items": [
                                                {
                                                    "type": "RichText",
                                                    "value": {
                                                        "_styles": [
                                                            "richText"
                                                        ],
                                                        "text": "<p class=\"brz-ls-xs-1_5 brz-fw-sm-700 brz-mb-lg-0 brz-mt-lg-0 brz-text-lg-center brz-ls-lg-0 brz-fs-lg-44 brz-ff-saira_extra_condensed brz-fw-lg-700 brz-lh-lg-1_3 brz-fs-sm-28 brz-ls-sm-1_5 brz-fs-xs-23 brz-fw-xs-800 brz-mb-xs-10 brz-mt-sm-0 brz-mb-sm-0\"><span style=\"color: rgb(255, 255, 255);\">END OF SEASON SALE</span></p><p class=\"brz-fs-sm-18 brz-text-lg-center brz-lh-lg-1_3 brz-ls-lg-0 brz-ff-average_sans brz-fw-lg-400 brz-fs-lg-19 brz-fs-xs-18\"><span style=\"color: rgb(0, 6, 7);\">--- Our Mid Top sneakers are now ---</span>&nbsp;</p><p class=\"brz-fs-xs-50 brz-fw-sm-800 brz-mt-lg-0 brz-text-lg-center brz-lh-lg-1_3 brz-ls-lg-0 brz-ff-saira brz-fw-lg-900 brz-mb-lg-0 brz-fs-lg-63 brz-fs-sm-70 brz-fw-xs-800 brz-mt-xs-0\"><span style=\"color: rgb(255, 255, 255);\">30% OFF&nbsp;</span></p>"
                                                    }
                                                }
                                            ],
                                            "tabletMarginBottom": 0,
                                            "tabletMarginBottomSuffix": "px",
                                            "tabletMargin": 0,
                                            "tabletMarginType": "grouped",
                                            "mobileMarginType": "grouped"
                                        }
                                    },
                                    {
                                        "type": "Wrapper",
                                        "value": {
                                            "_styles": [
                                                "wrapper",
                                                "wrapper--image"
                                            ],
                                            "items": [
                                                {
                                                    "type": "Image",
                                                    "value": {
                                                        "_styles": [
                                                            "image"
                                                        ],
                                                        "resize": 65,
                                                        "imageWidth": 700,
                                                        "imageHeight": 287,
                                                        "imageSrc": "c88d5afe4807f0c392f3e001042854c4.png",
                                                        "height": 100,
                                                        "positionX": 50,
                                                        "positionY": 50,
                                                        "imagePopulation": "",
                                                        "mobilePositionX": 52,
                                                        "mobilePositionY": 100,
                                                        "mobileHeight": 84,
                                                        "mobileResize": 80,
                                                        "tabletResize": 61,
                                                        "tabletHeight": 90,
                                                        "tabletPositionX": 52,
                                                        "tabletPositionY": 100
                                                    }
                                                }
                                            ],
                                            "marginType": "grouped",
                                            "tabletMarginType": "grouped",
                                            "mobileMarginType": "grouped"
                                        }
                                    },
                                    {
                                        "type": "Cloneable",
                                        "value": {
                                            "_styles": [
                                                "wrapper-clone",
                                                "wrapper-clone--button"
                                            ],
                                            "items": [
                                                {
                                                    "type": "Button",
                                                    "value": {
                                                        "_styles": [
                                                            "button"
                                                        ],
                                                        "bgColorPalette": "color8",
                                                        "tempBgColorPalette": "color8",
                                                        "bgColorOpacity": 1,
                                                        "borderRadiusType": "custom",
                                                        "fillType": "filled",
                                                        "borderColorPalette": 0,
                                                        "tempBorderColorPalette": "color8",
                                                        "hoverBgColorOpacity": 0.91,
                                                        "colorHex": "#0bb9c0",
                                                        "colorOpacity": 1,
                                                        "colorPalette": "",
                                                        "tempColorOpacity": 1,
                                                        "hoverColorHex": "#ffffff",
                                                        "hoverColorOpacity": 1,
                                                        "text": "SHOP NOW",
                                                        "fontSize": 15,
                                                        "fontFamily": "saira_extra_condensed",
                                                        "lineHeight": 1.8,
                                                        "letterSpacing": 3,
                                                        "fontWeight": 700,
                                                        "fontStyle": "",
                                                        "borderRadius": 4,
                                                        "iconName": "triangle-right-17",
                                                        "iconType": "glyph",
                                                        "tempBorderRadiusType": "custom",
                                                        "paddingRight": 42,
                                                        "paddingLeft": 42,
                                                        "borderWidth": 2,
                                                        "borderColorOpacity": 0,
                                                        "hoverBorderColorOpacity": 0,
                                                        "mobileBorderRadius": 4,
                                                        "tempBorderRadius": 4,
                                                        "tempMobileBorderRadius": 4,
                                                        "boxShadow": "on",
                                                        "boxShadowColorHex": "#053433",
                                                        "boxShadowColorOpacity": 1,
                                                        "boxShadowColorPalette": "",
                                                        "boxShadowSpread": -6,
                                                        "boxShadowVertical": 9,
                                                        "boxShadowBlur": 24,
                                                        "hoverBgColorPalette": "",
                                                        "hoverBorderColorPalette": "",
                                                        "tempHoverBorderColorPalette": "",
                                                        "hoverBgColorHex": "#010809",
                                                        "tempHoverBgColorOpacity": 0.91,
                                                        "tempHoverBgColorPalette": "",
                                                        "hoverBorderColorHex": "#010809",
                                                        "tempBorderWidth": 2,
                                                        "tabletSize": "medium",
                                                        "tabletFontSize": 15,
                                                        "tabletPaddingTB": 14,
                                                        "tabletPaddingTop": 14,
                                                        "tabletPaddingBottom": 14,
                                                        "tempTabletPaddingTB": 14,
                                                        "tempTabletPaddingTop": 14,
                                                        "tempTabletPaddingBottom": 14,
                                                        "tabletPaddingRL": 42,
                                                        "tabletPaddingRight": 42,
                                                        "tabletPaddingLeft": 42,
                                                        "tempTabletPaddingRL": 42,
                                                        "tempTabletPaddingRight": 42,
                                                        "tempTabletPaddingLeft": 42,
                                                        "tabletBorderRadius": 4,
                                                        "tabletLineHeight": 1.8,
                                                        "tabletLetterSpacing": 3,
                                                        "tabletFontWeight": 600,
                                                        "tabletFontStyle": "",
                                                        "mobileSize": "medium",
                                                        "mobileFontSize": 16,
                                                        "mobilePaddingTB": 14,
                                                        "mobilePaddingTop": 14,
                                                        "mobilePaddingBottom": 14,
                                                        "tempMobilePaddingTB": 14,
                                                        "tempMobilePaddingTop": 14,
                                                        "tempMobilePaddingBottom": 14,
                                                        "mobilePaddingRL": 42,
                                                        "mobilePaddingRight": 42,
                                                        "mobilePaddingLeft": 42,
                                                        "tempMobilePaddingRL": 42,
                                                        "tempMobilePaddingRight": 42,
                                                        "tempMobilePaddingLeft": 42,
                                                        "mobileLineHeight": 1.8,
                                                        "mobileLetterSpacing": 3,
                                                        "mobileFontWeight": 600,
                                                        "mobileFontStyle": "",
                                                        "size": "medium",
                                                        "paddingTB": 14,
                                                        "paddingTop": 14,
                                                        "paddingBottom": 14,
                                                        "tempPaddingTB": 14,
                                                        "tempPaddingTop": 14,
                                                        "tempPaddingBottom": 14,
                                                        "paddingRL": 42,
                                                        "tempPaddingRL": 42,
                                                        "tempPaddingRight": 42,
                                                        "tempPaddingLeft": 42,
                                                        "fontFamilyType": "google"
                                                    }
                                                }
                                            ]
                                        }
                                    },
                                    {
                                        "type": "Wrapper",
                                        "value": {
                                            "_styles": [
                                                "wrapper",
                                                "wrapper--richText"
                                            ],
                                            "items": [
                                                {
                                                    "type": "RichText",
                                                    "value": {
                                                        "_styles": [
                                                            "richText"
                                                        ],
                                                        "text": "<p class=\"brz-mb-lg-0 brz-mt-lg-0 brz-fs-sm-18 brz-fw-lg-400 brz-ff-average_sans brz-ls-lg-0 brz-lh-lg-1_7 brz-text-lg-center brz-fs-lg-15 brz-fs-xs-16\"><span style=\"opacity: 0.59; color: rgb(0, 6, 7);\">Final Prices As Marked. Only Select Items Apply As Stock Lasts</span></p>"
                                                    }
                                                }
                                            ],
                                            "mobilePaddingType": "ungrouped",
                                            "mobilePaddingRight": 28,
                                            "mobilePaddingRightSuffix": "px",
                                            "mobilePadding": 0,
                                            "mobilePaddingLeft": 27,
                                            "mobilePaddingLeftSuffix": "px"
                                        }
                                    }
                                ],
                                "width": 100,
                                "paddingTop": 25,
                                "paddingTopSuffix": "px",
                                "padding": 15,
                                "paddingBottom": 20,
                                "paddingBottomSuffix": "px",
                                "bgColorPalette": "",
                                "bgColorHex": "#0bb9c0",
                                "bgColorOpacity": 1,
                                "tabletPaddingRight": 10,
                                "tabletPaddingLeft": 10,
                                "mobilePaddingRight": 10,
                                "mobilePaddingLeft": 10,
                                "borderColorHex": "#ffffff",
                                "borderColorOpacity": 1,
                                "borderColorPalette": "",
                                "tempBorderColorOpacity": 1,
                                "borderWidth": 8,
                                "borderTopWidth": 8,
                                "borderRightWidth": 8,
                                "borderBottomWidth": 8,
                                "borderLeftWidth": 8,
                                "borderRadius": 10,
                                "borderTopLeftRadius": 10,
                                "borderTopRightRadius": 10,
                                "borderBottomRightRadius": 10,
                                "borderBottomLeftRadius": 10,
                                "tempBorderRadius": 10,
                                "tempBorderTopLeftRadius": 10,
                                "tempBorderTopRightRadius": 10,
                                "tempBorderBottomRightRadius": 10,
                                "tempBorderBottomLeftRadius": 10,
                                "tempBorderWidth": 8,
                                "tempBorderTopWidth": 8,
                                "tempBorderRightWidth": 8,
                                "tempBorderBottomWidth": 8,
                                "tempBorderLeftWidth": 8,
                                "boxShadow": "on",
                                "boxShadowBlur": 41,
                                "boxShadowColorOpacity": 1,
                                "boxShadowSpread": -26,
                                "boxShadowVertical": 36,
                                "tabletPaddingTop": 20,
                                "tabletPaddingTopSuffix": "px",
                                "tabletPadding": 15,
                                "tabletPaddingBottom": 20,
                                "tabletPaddingBottomSuffix": "px",
                                "tabletMarginRight": 40,
                                "tabletMarginRightSuffix": "px",
                                "tabletMargin": 0,
                                "tabletMarginLeft": 40,
                                "tabletMarginLeftSuffix": "px",
                                "mobilePaddingTop": 20,
                                "mobilePaddingTopSuffix": "px",
                                "mobilePadding": 0,
                                "mobilePaddingBottom": 20,
                                "mobilePaddingBottomSuffix": "px",
                                "tabsState": "",
                                "tabsColor": "",
                                "borderStyle": "solid",
                                "mobilePaddingType": "ungrouped",
                                "mobileMarginType": "grouped",
                                "mobileMarginSuffix": "px",
                                "mobileMarginTopSuffix": "px",
                                "mobileMarginRightSuffix": "px",
                                "mobileMarginBottomSuffix": "px",
                                "mobileMarginLeftSuffix": "px",
                                "mobileMargin": 0,
                                "mobileMarginTop": 0,
                                "mobileMarginRight": 0,
                                "mobileMarginBottom": 0,
                                "mobileMarginLeft": 0
                            }
                        }
                    ],
                    "bgColorPalette": "",
                    "bgColorHex": "#0bb9c0",
                    "bgColorOpacity": 0,
                    "mobileBgColorHex": "#000000",
                    "mobileBgColorOpacity": 0,
                    "mobileBgColorPalette": "",
                    "mobilePaddingRight": 0,
                    "mobilePaddingLeft": 0,
                    "borderRadiusType": "grouped",
                    "borderTopLeftRadius": 0,
                    "borderTopRightRadius": 0,
                    "borderBottomLeftRadius": 0,
                    "borderBottomRightRadius": 0,
                    "borderRadius": 0,
                    "tempBorderTopLeftRadius": 10,
                    "tempBorderRadius": 0,
                    "tempBorderBottomRightRadius": 10,
                    "tempBorderTopRightRadius": 10,
                    "tempBorderBottomLeftRadius": 10,
                    "tempBgColorOpacity": 1,
                    "borderColorHex": "#ffffff",
                    "borderColorOpacity": 0,
                    "borderColorPalette": "",
                    "tempBorderColorOpacity": 1,
                    "borderWidth": 0,
                    "borderTopWidth": 0,
                    "borderRightWidth": 0,
                    "borderBottomWidth": 0,
                    "borderLeftWidth": 0,
                    "tempBorderWidth": 8,
                    "tempBorderTopWidth": 8,
                    "tempBorderRightWidth": 8,
                    "tempBorderBottomWidth": 8,
                    "tempBorderLeftWidth": 8,
                    "tabletPaddingRight": 0,
                    "tabletPaddingLeft": 0,
                    "tabsState": "",
                    "tabsColor": ""
                }
            }
        ],
        "_blockVisibility": "unlisted",
        "bgImageWidth": 1920,
        "bgImageHeight": 1281,
        "bgImageSrc": "e299aa3097f4fba0829ff206b8fb13aacce12344.jpeg",
        "bgPositionX": 50,
        "bgPositionY": 50,
        "bgColorOpacity": 0.9,
        "tempBgColorOpacity": 0.9,
        "mobileBgImageWidth": 1920,
        "mobileBgImageHeight": 1281,
        "mobileBgImageSrc": "e299aa3097f4fba0829ff206b8fb13aacce12344.jpeg",
        "mobileBgPositionX": 50,
        "mobileBgPositionY": 50,
        "mobileBgColorOpacity": 0.9,
        "tempMobileBgColorOpacity": 1,
        "bgColorHex": "#1b2321",
        "bgColorPalette": "",
        "mobileBgColorHex": "#1b2321",
        "mobileBgColorPalette": "",
        "popupId": "xdqspithmvtpildnxhtvrtgrzytoynepmeov",
        "width": 600,
        "widthSuffix": "px",
        "closeHorizontalPosition": 30,
        "closeSize": "custom",
        "closeCustomSize": 20,
        "closeVerticalPosition": 30,
        "closePosition": "inside",
        "tabletWidth": 85,
        "tabletWidthSuffix": "%",
        "tabletCloseHorizontalPosition": 60,
        "tabletCloseVerticalPosition": 20,
        "tabletCloseSize": "medium",
        "tabletCloseCustomSize": 24,
        "mobileWidth": 85,
        "mobileWidthSuffix": "%",
        "mobileCloseHorizontalPosition": 20,
        "mobileCloseVerticalPosition": 20,
        "mobileCloseSize": "medium",
        "mobileCloseCustomSize": 24
    }
}
};