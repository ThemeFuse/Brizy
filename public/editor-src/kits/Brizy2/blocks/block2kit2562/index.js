module.exports = {
  id: "block2kit2562",
  thumbnailWidth: 600,
  thumbnailHeight: 267,
  title: "block2kit2562",

  keywords: "button,hero,image",
  cat: [5582, 100],
  type: 1,

  resolve: {
    type: "Section",
    value: {
      _styles: ["section"],
      items: [
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            items: [
              {
                type: "Row",
                value: {
                  _styles: ["row", "hide-row-borders", "padding-0"],
                  items: [
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h2 class="brz-text-xs-center brz-tp-heading2"><span class="brz-cp-color8">If the wind will not serve, take to the oars.</span></h2>'
                                  }
                                }
                              ],
                              paddingType: "ungrouped",
                              paddingRightSuffix: "px",
                              paddingRight: 20,
                              padding: 0,
                              paddingSuffix: "px"
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 10,
                                    tabletHeight: 0,
                                    mobileHeight: 0
                                  }
                                }
                              ],
                              showOnTablet: "off",
                              showOnMobile: "off"
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-text-xs-center brz-tp-subtitle"><span class="brz-cp-color6" style="opacity: 0.75;"><span>The more the better. So you will get also a lot of keywords which you must pay for only the minimal commandment of 5 cents.</span></span></p>'
                                  }
                                }
                              ],
                              paddingType: "ungrouped",
                              paddingRightSuffix: "px",
                              paddingRight: 35,
                              padding: 0,
                              paddingSuffix: "px"
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 20,
                                    tabletHeight: 0,
                                    mobileHeight: 0
                                  }
                                }
                              ],
                              showOnTablet: "off",
                              showOnMobile: "off"
                            }
                          },
                          {
                            type: "Cloneable",
                            value: {
                              _styles: [
                                "wrapper-clone",
                                "wrapper-clone--button"
                              ],
                              items: [
                                {
                                  type: "Button",
                                  value: {
                                    _styles: ["button"],
                                    text: "Get iOS Version",
                                    iconName: "apple",
                                    iconType: "glyph",
                                    iconPosition: "left",
                                    borderRadiusType: "custom",
                                    tempBorderRadiusType: "custom",
                                    paddingRight: 26,
                                    paddingLeft: 26,
                                    fillType: "filled",
                                    borderRadius: 4,
                                    borderWidth: 2,
                                    borderColorOpacity: 1,
                                    borderColorPalette: "color3",
                                    bgColorOpacity: 1,
                                    bgColorPalette: "color3",
                                    hoverBgColorOpacity: 1,
                                    hoverBorderColorOpacity: 1,
                                    tempBorderRadius: 4,
                                    tempBorderWidth: 2,
                                    hoverBgColorPalette: "color8",
                                    hoverBorderColorPalette: "color8",
                                    tempHoverBorderColorPalette: "color8",
                                    hoverBgColorHex: "#1c1c1c",
                                    tempHoverBgColorOpacity: 1,
                                    tempHoverBgColorPalette: "color3",
                                    hoverBorderColorHex: "#0ad1bd",
                                    fontStyle: "button",
                                    size: "small",
                                    fontSize: 11,
                                    paddingTB: 11,
                                    paddingTop: 11,
                                    paddingBottom: 11,
                                    tempPaddingTB: 11,
                                    tempPaddingTop: 11,
                                    tempPaddingBottom: 11,
                                    paddingRL: 26,
                                    tempPaddingRL: 26,
                                    tempPaddingRight: 26,
                                    tempPaddingLeft: 26,
                                    tempBgColorPalette: "color3",
                                    tempBorderColorPalette: "color3",
                                    colorPalette: "color2",
                                    colorOpacity: 1,
                                    hoverColorPalette: "color2",
                                    hoverColorOpacity: 1
                                  }
                                },
                                {
                                  type: "Button",
                                  value: {
                                    _styles: ["button"],
                                    text: "Android Version",
                                    iconName: "android",
                                    iconType: "glyph",
                                    iconPosition: "left",
                                    borderRadiusType: "custom",
                                    tempBorderRadiusType: "custom",
                                    paddingRight: 26,
                                    paddingLeft: 26,
                                    fillType: "outline",
                                    borderRadius: 4,
                                    borderWidth: 2,
                                    borderColorOpacity: 1,
                                    borderColorPalette: "color8",
                                    bgColorOpacity: 0,
                                    bgColorPalette: "",
                                    hoverBgColorOpacity: 1,
                                    hoverBorderColorOpacity: 0,
                                    tempBorderRadius: 4,
                                    tempBorderWidth: 2,
                                    hoverBgColorPalette: "color8",
                                    hoverBorderColorPalette: "color8",
                                    tempHoverBorderColorPalette: "color8",
                                    hoverBgColorHex: "#1c1c1c",
                                    tempHoverBgColorOpacity: 1,
                                    tempHoverBgColorPalette: "color3",
                                    hoverBorderColorHex: "#0ad1bd",
                                    fontStyle: "button",
                                    size: "small",
                                    fontSize: 11,
                                    paddingTB: 11,
                                    paddingTop: 11,
                                    paddingBottom: 11,
                                    tempPaddingTB: 11,
                                    tempPaddingTop: 11,
                                    tempPaddingBottom: 11,
                                    paddingRL: 26,
                                    tempPaddingRL: 26,
                                    tempPaddingRight: 26,
                                    tempPaddingLeft: 26,
                                    tempBgColorPalette: "color3",
                                    tempBorderColorPalette: "color8",
                                    bgColorHex: "#1c1c1c",
                                    tempBgColorOpacity: 1,
                                    borderColorHex: "#1c1c1c",
                                    colorPalette: "color8",
                                    colorOpacity: 1,
                                    hoverColorPalette: "color2",
                                    hoverColorOpacity: 1,
                                    tempFillType: "outline"
                                  }
                                }
                              ],
                              horizontalAlign: "left",
                              itemPadding: 20,
                              itemPaddingRight: 20,
                              itemPaddingLeft: 20,
                              mobileHorizontalAlign: "center"
                            }
                          }
                        ],
                        paddingLeft: 15,
                        paddingLeftSuffix: "px",
                        padding: 15,
                        paddingRight: 40,
                        paddingRightSuffix: "px",
                        tabsState: "tabNormal",
                        tabsCurrentElement: "tabCurrentElement",
                        tabsColor: "tabOverlay",
                        width: 43,
                        verticalAlign: "center",
                        tabletWidth: 50,
                        mobilePaddingRightSuffix: "px",
                        mobilePaddingRight: 20,
                        mobilePadding: 0,
                        mobilePaddingSuffix: "px",
                        mobilePaddingLeftSuffix: "px",
                        mobilePaddingLeft: 20,
                        paddingSuffix: "px"
                      }
                    },
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--image"],
                              items: [
                                {
                                  type: "Image",
                                  value: {
                                    _styles: ["image"],
                                    imageWidth: 1000,
                                    imageHeight: 587,
                                    imageSrc:
                                      "d04-macbookvectorial-black-1.png",
                                    height: 100,
                                    positionX: 50,
                                    positionY: 50,
                                    imagePopulation: "",
                                    mobileResize: 90
                                  }
                                }
                              ]
                            }
                          }
                        ],
                        width: 57,
                        tabletWidth: 50,
                        verticalAlign: "center",
                        tabsState: "tabNormal",
                        tabsCurrentElement: "tabCurrentElement",
                        tabsColor: "tabOverlay"
                      }
                    }
                  ],
                  mobileReverseColumns: "on"
                }
              }
            ],
            paddingType: "ungrouped",
            paddingTop: 100,
            paddingBottom: 100,
            padding: 75,
            tabsColor: "tabOverlay",
            bgColorPalette: "color2",
            tempBgColorPalette: "color2",
            bgColorOpacity: 1,
            borderRadius: 0,
            borderTopLeftRadius: 0,
            borderTopRightRadius: 0,
            borderBottomLeftRadius: 0,
            borderBottomRightRadius: 0,
            tempBorderTopLeftRadius: 0,
            tempBorderTopRightRadius: 0,
            tempBorderBottomLeftRadius: 0,
            tempBorderBottomRightRadius: 0,
            tabsState: "tabNormal",
            tabsCurrentElement: "tabCurrentElement"
          }
        }
      ]
    }
  }
};
