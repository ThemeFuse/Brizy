module.exports = {
  id: "block2kit1851",
  thumbnailWidth: 600,
  thumbnailHeight: 248,
  title: "block2kit1851",

  keywords: "address,contact,email,image",
  cat: [5586, 100],
  type: 0,

  resolve: {
    type: "Section",
    value: {
      _styles: ["section"],
      items: [
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            items: [
              {
                type: "Row",
                value: {
                  _styles: ["row"],
                  items: [
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h2 class="brz-tp-heading2"><span class="brz-cp-color2">There is no denying the fact that the success of an advertisement lies mostly in the headline.</span></h2>'
                                  }
                                }
                              ]
                            }
                          }
                        ],
                        width: 59.4,
                        paddingRightSuffix: "%",
                        paddingRight: 20,
                        padding: 0,
                        paddingSuffix: "px",
                        paddingTopSuffix: "px",
                        paddingTop: 0,
                        paddingBottomSuffix: "px",
                        paddingBottom: 0,
                        paddingLeftSuffix: "px",
                        paddingLeft: 20,
                        tabletWidth: 64,
                        tabletPaddingRightSuffix: "px",
                        tabletPaddingRight: 40,
                        tabletPadding: 15,
                        tabletPaddingSuffix: "px"
                      }
                    },
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h4 class="brz-tp-heading4"><span class="brz-cp-color2">Address</span></h4>'
                                  }
                                }
                              ],
                              marginBottomSuffix: "px",
                              marginBottom: 0,
                              margin: 0,
                              marginSuffix: "px",
                              tabletMarginType: "grouped",
                              mobileMarginBottomSuffix: "px",
                              mobileMarginBottom: 0,
                              mobileMargin: 0,
                              mobileMarginSuffix: "px"
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-tp-paragraph"><span class="brz-cp-color7">353 Alexandrea Locks Suite 145</span></p>'
                                  }
                                }
                              ],
                              marginTopSuffix: "px",
                              marginTop: 0,
                              margin: 0,
                              marginSuffix: "px",
                              tabletMarginTopSuffix: "px",
                              tabletMarginTop: 0,
                              tabletMargin: 0,
                              tabletMarginSuffix: "px",
                              tabletMarginType: "ungrouped",
                              tabletMarginBottomSuffix: "px",
                              tabletMarginBottom: 30,
                              mobileMarginTopSuffix: "px",
                              mobileMarginTop: 0,
                              mobileMargin: 0,
                              mobileMarginSuffix: "px"
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h4 class="brz-tp-heading4"><span class="brz-cp-color2">Contact</span></h4>'
                                  }
                                }
                              ],
                              marginBottomSuffix: "px",
                              marginBottom: 0,
                              margin: 0,
                              marginSuffix: "px",
                              tabletMarginBottomSuffix: "px",
                              tabletMarginBottom: 0,
                              tabletMargin: 0,
                              tabletMarginSuffix: "px",
                              mobileMarginBottomSuffix: "px",
                              mobileMarginBottom: 0,
                              mobileMargin: 0,
                              mobileMarginSuffix: "px"
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-tp-paragraph"><span class="brz-cp-color7">landen_friesen@yahoo.com</span></p>'
                                  }
                                }
                              ],
                              marginTopSuffix: "px",
                              marginTop: 0,
                              margin: 0,
                              marginSuffix: "px",
                              tabletMarginType: "grouped",
                              mobileMarginTopSuffix: "px",
                              mobileMarginTop: 0,
                              mobileMargin: 0,
                              mobileMarginSuffix: "px"
                            }
                          }
                        ],
                        paddingLeft: 15,
                        paddingLeftSuffix: "%",
                        padding: 15,
                        tabsState: "tabNormal",
                        tabsCurrentElement: "tabCurrentElement",
                        tabsColor: "tabOverlay",
                        width: 40.6,
                        tabletWidth: 36,
                        verticalAlign: "center",
                        paddingType: "ungrouped",
                        paddingSuffix: "px",
                        paddingRightSuffix: "px",
                        paddingRight: 0,
                        paddingTopSuffix: "px",
                        paddingTop: 0,
                        paddingBottomSuffix: "px",
                        paddingBottom: 0
                      }
                    }
                  ],
                  size: 85,
                  tabsState: "tabNormal",
                  tabsCurrentElement: "tabCurrentElement",
                  tabsColor: "tabOverlay",
                  tabletPaddingRightSuffix: "px",
                  tabletPaddingRight: 20,
                  tabletPadding: 0,
                  tabletPaddingSuffix: "px",
                  tabletPaddingLeftSuffix: "px",
                  tabletPaddingLeft: 20,
                  mobilePaddingRightSuffix: "px",
                  mobilePaddingRight: 30,
                  mobilePadding: 0,
                  mobilePaddingSuffix: "px",
                  mobilePaddingLeftSuffix: "px",
                  mobilePaddingLeft: 30
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--spacer"],
                  items: [
                    {
                      type: "Spacer",
                      value: {
                        _styles: ["spacer"],
                        height: 60,
                        tabletHeight: 30,
                        mobileHeight: 10
                      }
                    }
                  ],
                  showOnTablet: "on",
                  showOnMobile: "on"
                }
              },
              {
                type: "Row",
                value: {
                  _styles: ["row", "hide-row-borders", "padding-0"],
                  items: [
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 209
                                  }
                                }
                              ]
                            }
                          }
                        ],
                        width: 100,
                        tabsState: "",
                        bgImageWidth: 1440,
                        bgImageHeight: 271,
                        bgImageSrc: "d09-Sneaker-Wide.jpg",
                        bgColorOpacity: 0,
                        tempBgColorOpacity: 1,
                        borderRadius: 0,
                        borderTopLeftRadius: 0,
                        borderTopRightRadius: 0,
                        borderBottomLeftRadius: 0,
                        borderBottomRightRadius: 0,
                        tempBorderTopLeftRadius: 0,
                        tempBorderTopRightRadius: 0,
                        tempBorderBottomLeftRadius: 0,
                        tempBorderBottomRightRadius: 0,
                        mobilePaddingRight: 10,
                        mobilePaddingLeft: 10,
                        bgPositionX: 50,
                        bgPositionY: 0,
                        verticalAlign: "center",
                        paddingType: "grouped",
                        paddingSuffix: "px",
                        paddingTopSuffix: "px",
                        paddingRightSuffix: "px",
                        paddingBottomSuffix: "px",
                        paddingLeftSuffix: "px",
                        padding: 40,
                        paddingTop: 40,
                        paddingRight: 40,
                        paddingBottom: 40,
                        paddingLeft: 40
                      }
                    }
                  ],
                  mobileReverseColumns: "off"
                }
              }
            ],
            paddingType: "ungrouped",
            paddingTop: 80,
            paddingBottom: 0,
            padding: 75,
            tabsState: "tabNormal",
            tabsCurrentElement: "tabCurrentElement",
            tabsColor: "tabOverlay",
            tabletPaddingType: "ungrouped",
            tabletPaddingTop: 50,
            tabletPaddingBottom: 15,
            tabletPadding: 50,
            mobilePaddingType: "ungrouped",
            mobilePaddingTop: 25,
            mobilePaddingBottom: 15,
            mobilePadding: 25,
            containerType: "fullWidth"
          }
        }
      ]
    }
  }
};
