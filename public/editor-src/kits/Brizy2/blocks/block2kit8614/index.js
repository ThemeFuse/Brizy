module.exports = {
  id: "block2kit8614",
  thumbnailWidth: 600,
  thumbnailHeight: 280,
  title: "block2kit8614",

  keywords: "features,icons,image",
  cat: [5584, 100],
  type: 1,

  resolve: {
    type: "Section",
    value: {
      _styles: ["section"],
      items: [
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            items: [
              {
                type: "Row",
                value: {
                  _styles: ["row", "hide-row-borders", "padding-0"],
                  items: [
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h2 class="brz-tp-heading2"><span class="brz-cp-color8">Expanding your company\u2019s brand awareness </span></h2>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-tp-subtitle"><span class="brz-cp-color6" style="opacity: 0.75;"><span>The majestic Rocky Mountains are a major tourist location in the western United States.</span></span></p>'
                                  }
                                }
                              ],
                              paddingType: "ungrouped",
                              paddingRightSuffix: "px",
                              paddingRight: 100,
                              padding: 0,
                              paddingSuffix: "px"
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 40,
                                    tabletHeight: 22,
                                    mobileHeight: 18
                                  }
                                }
                              ],
                              showOnTablet: "on",
                              showOnMobile: "on"
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--iconText"],
                              items: [
                                {
                                  type: "IconText",
                                  value: {
                                    _styles: ["iconText"],
                                    items: [
                                      {
                                        type: "Icon",
                                        value: {
                                          _styles: ["icon", "iconText--icon"],
                                          name: "zoom-split",
                                          type: "glyph",
                                          size: "custom",
                                          customSize: 20,
                                          borderRadius: 23,
                                          padding: 14,
                                          tempPadding: 14,
                                          borderWidth: 0,
                                          borderRadiusType: "custom",
                                          fillType: "filled",
                                          borderColorOpacity: 0,
                                          borderColorPalette: 0,
                                          bgColorOpacity: 1,
                                          bgColorPalette: "color3",
                                          tempBorderWidth: 3,
                                          hoverBgColorOpacity: 1,
                                          colorPalette: "color2",
                                          colorOpacity: 1,
                                          tempBgColorPalette: "color3",
                                          borderTopLeftRadius: 4,
                                          borderTopRightRadius: 4,
                                          borderBottomLeftRadius: 4,
                                          borderBottomRightRadius: 4,
                                          tempBorderTopLeftRadius: 4,
                                          tempBorderTopRightRadius: 4,
                                          tempBorderBottomLeftRadius: 4,
                                          tempBorderBottomRightRadius: 4,
                                          tempBorderRadiusType: "custom",
                                          hoverBorderColorOpacity: 1,
                                          tempBorderRadius: 24,
                                          hoverColorPalette: "color8",
                                          hoverColorOpacity: 1,
                                          hoverColorHex: "#ffffff",
                                          hoverBgColorPalette: "color3",
                                          tempHoverBgColorPalette: "color3",
                                          hoverBorderRadius: 4,
                                          hoverBorderTopLeftRadius: 4,
                                          hoverBorderTopRightRadius: 4,
                                          hoverBorderBottomLeftRadius: 4,
                                          hoverBorderBottomRightRadius: 4,
                                          tempHoverBorderTopLeftRadius: 4,
                                          tempHoverBorderTopRightRadius: 4,
                                          tempHoverBorderBottomLeftRadius: 4,
                                          tempHoverBorderBottomRightRadius: 4,
                                          tabsState: "",
                                          tabsColor: ""
                                        }
                                      },
                                      {
                                        type: "RichText",
                                        value: {
                                          _styles: ["text", "iconText--text"],
                                          text:
                                            '<h6 class="brz-tp-heading6 brz-mt-lg-14"><span class="brz-cp-color8">Reinvent Search Architectures</span></h6>'
                                        }
                                      }
                                    ]
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--iconText"],
                              items: [
                                {
                                  type: "IconText",
                                  value: {
                                    _styles: ["iconText"],
                                    items: [
                                      {
                                        type: "Icon",
                                        value: {
                                          _styles: ["icon", "iconText--icon"],
                                          name: "lock",
                                          type: "glyph",
                                          size: "custom",
                                          customSize: 20,
                                          borderRadius: 23,
                                          padding: 14,
                                          tempPadding: 14,
                                          borderWidth: 0,
                                          borderRadiusType: "custom",
                                          fillType: "filled",
                                          borderColorOpacity: 0,
                                          borderColorPalette: 0,
                                          bgColorOpacity: 1,
                                          bgColorPalette: "color3",
                                          tempBorderWidth: 3,
                                          hoverBgColorOpacity: 1,
                                          colorPalette: "color2",
                                          colorOpacity: 1,
                                          tempBgColorPalette: "color3",
                                          borderTopLeftRadius: 4,
                                          borderTopRightRadius: 4,
                                          borderBottomLeftRadius: 4,
                                          borderBottomRightRadius: 4,
                                          tempBorderTopLeftRadius: 4,
                                          tempBorderTopRightRadius: 4,
                                          tempBorderBottomLeftRadius: 4,
                                          tempBorderBottomRightRadius: 4,
                                          tempBorderRadiusType: "custom",
                                          hoverBorderColorOpacity: 1,
                                          tempBorderRadius: 24,
                                          hoverColorPalette: "color8",
                                          hoverColorOpacity: 1,
                                          hoverColorHex: "#ffffff",
                                          hoverBgColorPalette: "color3",
                                          tempHoverBgColorPalette: "color3",
                                          hoverBorderRadius: 4,
                                          hoverBorderTopLeftRadius: 4,
                                          hoverBorderTopRightRadius: 4,
                                          hoverBorderBottomLeftRadius: 4,
                                          hoverBorderBottomRightRadius: 4,
                                          tempHoverBorderTopLeftRadius: 4,
                                          tempHoverBorderTopRightRadius: 4,
                                          tempHoverBorderBottomLeftRadius: 4,
                                          tempHoverBorderBottomRightRadius: 4,
                                          tabsState: "",
                                          tabsColor: ""
                                        }
                                      },
                                      {
                                        type: "RichText",
                                        value: {
                                          _styles: ["text", "iconText--text"],
                                          text:
                                            '<h6 class="brz-mt-lg-14 brz-tp-heading6"><span class="brz-cp-color8">Innovate Security solutions</span></h6>'
                                        }
                                      }
                                    ]
                                  }
                                }
                              ]
                            }
                          }
                        ],
                        paddingLeft: 25,
                        paddingLeftSuffix: "%",
                        padding: 15,
                        paddingRight: 12,
                        paddingRightSuffix: "%",
                        tabsState: "tabNormal",
                        tabsCurrentElement: "tabCurrentElement",
                        tabsColor: "tabOverlay",
                        verticalAlign: "center",
                        paddingSuffix: "px",
                        mobilePaddingRightSuffix: "px",
                        mobilePaddingRight: 20,
                        mobilePadding: 0,
                        mobilePaddingSuffix: "px",
                        mobilePaddingLeftSuffix: "px",
                        mobilePaddingLeft: 20
                      }
                    },
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 489,
                                    tabletHeight: 456,
                                    mobileHeight: 308
                                  }
                                }
                              ],
                              showOnMobile: "on"
                            }
                          }
                        ],
                        tabsCurrentElement: "tabCurrentElement",
                        bgImageWidth: 1348,
                        bgImageHeight: 1344,
                        bgImageSrc: "d03-Img-Cup-Tea-1.jpg",
                        bgColorOpacity: 0,
                        tempBgColorOpacity: 1,
                        borderRadius: 0,
                        borderTopLeftRadius: 0,
                        borderTopRightRadius: 0,
                        borderBottomLeftRadius: 0,
                        borderBottomRightRadius: 0,
                        tempBorderTopLeftRadius: 0,
                        tempBorderTopRightRadius: 0,
                        tempBorderBottomLeftRadius: 0,
                        tempBorderBottomRightRadius: 0,
                        tabletPaddingRight: 10,
                        tabletPaddingLeft: 10,
                        mobilePaddingRight: 10,
                        mobilePaddingLeft: 10,
                        tabsState: "",
                        tabsColor: "tabOverlay",
                        paddingRightSuffix: "px",
                        paddingRight: 50,
                        padding: 50,
                        paddingSuffix: "px",
                        paddingTopSuffix: "px",
                        paddingTop: 50,
                        paddingBottomSuffix: "px",
                        paddingBottom: 50,
                        paddingType: "grouped",
                        paddingLeftSuffix: "px",
                        paddingLeft: 50,
                        mobileBorderRadius: 6,
                        mobileBorderTopLeftRadius: 6,
                        mobileBorderTopRightRadius: 6,
                        mobileBorderBottomLeftRadius: 6,
                        mobileBorderBottomRightRadius: 6,
                        tempMobileBorderRadius: 6,
                        tempMobileBorderTopLeftRadius: 6,
                        tempMobileBorderTopRightRadius: 6,
                        tempMobileBorderBottomLeftRadius: 6,
                        tempMobileBorderBottomRightRadius: 6,
                        mobileBorderStyle: "solid",
                        mobileBorderWidth: 0,
                        mobileBorderColorOpacity: 0,
                        mobileBorderColorPalette: ""
                      }
                    }
                  ],
                  mobileReverseColumns: "on"
                }
              }
            ],
            containerType: "fullWidth",
            tabsState: "tabNormal",
            tabsCurrentElement: "tabCurrentElement",
            tabsColor: "",
            padding: 0,
            paddingTop: 0,
            paddingBottom: 0,
            tabletPadding: 0,
            tabletPaddingTop: 0,
            tabletPaddingBottom: 0,
            bgColorPalette: "color2",
            tempBgColorPalette: "color2",
            bgColorOpacity: 1,
            borderRadius: 0,
            borderTopLeftRadius: 0,
            borderTopRightRadius: 0,
            borderBottomLeftRadius: 0,
            borderBottomRightRadius: 0,
            tempBorderTopLeftRadius: 0,
            tempBorderTopRightRadius: 0,
            tempBorderBottomLeftRadius: 0,
            tempBorderBottomRightRadius: 0
          }
        }
      ]
    }
  }
};
