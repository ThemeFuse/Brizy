module.exports = {
  id: "block2kit1460",
  thumbnailWidth: 600,
  thumbnailHeight: 267,
  title: "block2kit1460",

  keywords: "background,button,hero",
  cat: [5582, 100],
  type: 0,

  resolve: {
    type: "Section",
    value: {
      _styles: ["section"],
      items: [
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            items: [
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--richText"],
                  items: [
                    {
                      type: "RichText",
                      value: {
                        _styles: ["richText"],
                        text:
                          '<h1 class="brz-tp-heading1 brz-text-lg-right brz-text-xs-center"><span class="brz-cp-color2">Education costs money. But then so does ignorance</span></h1>'
                      }
                    }
                  ],
                  paddingType: "ungrouped",
                  paddingLeftSuffix: "%",
                  paddingLeft: 54,
                  padding: 0,
                  paddingSuffix: "px",
                  tabletPaddingType: "ungrouped",
                  tabletPaddingLeftSuffix: "%",
                  tabletPaddingLeft: 10,
                  tabletPadding: 0,
                  tabletPaddingSuffix: "px"
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--richText"],
                  items: [
                    {
                      type: "RichText",
                      value: {
                        _styles: ["richText"],
                        text:
                          '<p class="brz-mb-lg-20 brz-text-lg-right brz-tp-subtitle brz-text-xs-center"><span class="brz-cp-color2"><span>Before cell phones became so popular, watches were much more attractive to carry around because they are the most convenient way to keep track of the time. </span></span></p>'
                      }
                    }
                  ],
                  tabletPaddingType: "ungrouped",
                  paddingLeftSuffix: "%",
                  tabletPaddingLeft: 170,
                  tabletPadding: 0,
                  tabletPaddingSuffix: "px",
                  mobilePaddingType: "ungrouped",
                  paddingRightSuffix: "px",
                  mobilePaddingRight: 60,
                  mobilePadding: 0,
                  mobilePaddingSuffix: "px",
                  mobilePaddingLeft: 60,
                  paddingType: "ungrouped",
                  paddingLeft: 60,
                  padding: 0,
                  paddingSuffix: "px"
                }
              },
              {
                type: "Cloneable",
                value: {
                  _styles: ["wrapper-clone", "wrapper-clone--button"],
                  items: [
                    {
                      type: "Button",
                      value: {
                        _styles: ["button"],
                        text: "Buy Now",
                        iconName: "",
                        iconType: "",
                        colorPalette: "color2",
                        colorOpacity: 1,
                        bgColorPalette: "color8",
                        tempBgColorPalette: "color8",
                        bgColorOpacity: 1,
                        borderRadiusType: "custom",
                        fillType: "filled",
                        borderColorPalette: "color8",
                        tempBorderColorPalette: "color8",
                        hoverBgColorOpacity: 1,
                        hoverBgColorPalette: "color2",
                        hoverBorderColorPalette: "",
                        tempHoverBorderColorPalette: "",
                        hoverBgColorHex: "#1c1c1c",
                        tempHoverBgColorOpacity: 1,
                        tempHoverBgColorPalette: "color3",
                        hoverBorderColorHex: "#1c1c1c",
                        hoverBorderColorOpacity: 0.8,
                        tempBorderRadiusType: "custom",
                        paddingRight: 42,
                        paddingLeft: 42,
                        borderRadius: 4,
                        borderWidth: 2,
                        borderColorOpacity: 1,
                        tempBorderRadius: 4,
                        tempBorderWidth: 2,
                        tabletSize: "medium",
                        tabletFontSize: 12,
                        tabletPaddingTB: 14,
                        tabletPaddingTop: 14,
                        tabletPaddingBottom: 14,
                        tempTabletPaddingTB: 14,
                        tempTabletPaddingTop: 14,
                        tempTabletPaddingBottom: 14,
                        tabletPaddingRL: 42,
                        tabletPaddingRight: 42,
                        tabletPaddingLeft: 42,
                        tempTabletPaddingRL: 42,
                        tempTabletPaddingRight: 42,
                        tempTabletPaddingLeft: 42,
                        tabletBorderRadius: 6,
                        mobileSize: "medium",
                        mobileFontSize: 12,
                        mobilePaddingTB: 14,
                        mobilePaddingTop: 14,
                        mobilePaddingBottom: 14,
                        tempMobilePaddingTB: 14,
                        tempMobilePaddingTop: 14,
                        tempMobilePaddingBottom: 14,
                        mobilePaddingRL: 42,
                        mobilePaddingRight: 42,
                        mobilePaddingLeft: 42,
                        tempMobilePaddingRL: 42,
                        tempMobilePaddingRight: 42,
                        tempMobilePaddingLeft: 42,
                        mobileBorderRadius: 6
                      }
                    },
                    {
                      type: "Button",
                      value: {
                        _styles: ["button"],
                        text: "Learn More",
                        iconName: "",
                        iconType: "",
                        colorPalette: "color8",
                        colorOpacity: 1,
                        bgColorPalette: "color3",
                        tempBgColorPalette: "color3",
                        bgColorOpacity: 1,
                        borderRadiusType: "custom",
                        fillType: "filled",
                        borderColorPalette: "color3",
                        tempBorderColorPalette: "color3",
                        hoverBgColorOpacity: 1,
                        hoverBgColorPalette: "color2",
                        hoverBorderColorPalette: "",
                        tempHoverBorderColorPalette: "",
                        hoverBgColorHex: "#1c1c1c",
                        tempHoverBgColorOpacity: 1,
                        tempHoverBgColorPalette: "color3",
                        hoverBorderColorHex: "#1c1c1c",
                        hoverBorderColorOpacity: 0.8,
                        tempBorderRadiusType: "custom",
                        paddingRight: 42,
                        paddingLeft: 42,
                        borderRadius: 4,
                        borderWidth: 2,
                        borderColorOpacity: 1,
                        tempBorderRadius: 4,
                        tempBorderWidth: 2,
                        tabletSize: "medium",
                        tabletFontSize: 12,
                        tabletPaddingTB: 14,
                        tabletPaddingTop: 14,
                        tabletPaddingBottom: 14,
                        tempTabletPaddingTB: 14,
                        tempTabletPaddingTop: 14,
                        tempTabletPaddingBottom: 14,
                        tabletPaddingRL: 42,
                        tabletPaddingRight: 42,
                        tabletPaddingLeft: 42,
                        tempTabletPaddingRL: 42,
                        tempTabletPaddingRight: 42,
                        tempTabletPaddingLeft: 42,
                        tabletBorderRadius: 6,
                        mobileSize: "medium",
                        mobileFontSize: 12,
                        mobilePaddingTB: 14,
                        mobilePaddingTop: 14,
                        mobilePaddingBottom: 14,
                        tempMobilePaddingTB: 14,
                        tempMobilePaddingTop: 14,
                        tempMobilePaddingBottom: 14,
                        mobilePaddingRL: 42,
                        mobilePaddingRight: 42,
                        mobilePaddingLeft: 42,
                        tempMobilePaddingRL: 42,
                        tempMobilePaddingRight: 42,
                        tempMobilePaddingLeft: 42,
                        mobileBorderRadius: 6
                      }
                    }
                  ],
                  horizontalAlign: "right",
                  itemPadding: 20,
                  itemPaddingRight: 20,
                  itemPaddingLeft: 20,
                  mobileHorizontalAlign: "center"
                }
              }
            ],
            paddingType: "ungrouped",
            paddingTop: 170,
            paddingBottom: 150,
            padding: 75,
            tabsCurrentElement: "tabCurrentElement",
            bgImageWidth: 2880,
            bgImageHeight: 1282,
            bgImageSrc: "d03-Img-Stairway.jpg",
            bgColorOpacity: 0,
            tempBgColorOpacity: 1,
            borderRadius: 0,
            borderTopLeftRadius: 0,
            borderTopRightRadius: 0,
            borderBottomLeftRadius: 0,
            borderBottomRightRadius: 0,
            tempBorderTopLeftRadius: 0,
            tempBorderTopRightRadius: 0,
            tempBorderBottomLeftRadius: 0,
            tempBorderBottomRightRadius: 0,
            tabsState: "tabNormal",
            tabsColor: "tabOverlay",
            mobileBgColorPalette: "color8",
            tempMobileBgColorPalette: "color8",
            mobileBgColorOpacity: 0.64,
            mobileBorderRadius: 0,
            mobileBgColorHex: "#ffffff",
            tempMobileBgColorOpacity: 0.64,
            tabletBgColorPalette: "color8",
            tempTabletBgColorPalette: "color8",
            tabletBgColorOpacity: 0.35,
            tabletBorderRadius: 0,
            tabletBgColorHex: "#ffffff",
            tempTabletBgColorOpacity: 0.35
          }
        }
      ]
    }
  }
};
