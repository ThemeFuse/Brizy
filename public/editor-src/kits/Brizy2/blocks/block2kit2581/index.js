module.exports = {
  id: "block2kit2581",
  thumbnailWidth: 600,
  thumbnailHeight: 253,
  title: "block2kit2581",

  keywords: "background,button,features,image",
  cat: [5584, 100],
  type: 1,

  resolve: {
    type: "Section",
    value: {
      _styles: ["section"],
      items: [
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            items: [
              {
                type: "Row",
                value: {
                  _styles: ["row", "hide-row-borders", "padding-0"],
                  items: [
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--image"],
                              items: [
                                {
                                  type: "Image",
                                  value: {
                                    _styles: ["image"],
                                    imageWidth: 1108,
                                    imageHeight: 912,
                                    imageSrc: "d03-Img-Lemmons.jpg",
                                    height: 100,
                                    positionX: 50,
                                    positionY: 50,
                                    imagePopulation: "",
                                    borderRadiusType: "custom",
                                    borderRadius: 6,
                                    tempBorderRadius: 6
                                  }
                                }
                              ]
                            }
                          }
                        ],
                        verticalAlign: "center",
                        tabsState: "tabNormal",
                        tabsCurrentElement: "tabCurrentElement",
                        tabsColor: "tabOverlay"
                      }
                    },
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-tp-abovetitle"><span class="brz-cp-color8" style="opacity: 0.7;">MODERN DESIGN INCORPORATES</span></p>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h2 class="brz-tp-heading2"><span class="brz-cp-color8">If you had to eat just one thing for the rest of your life, what would you choose?</span></h2>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 25
                                  }
                                }
                              ],
                              showOnTablet: "off",
                              showOnMobile: "off"
                            }
                          },
                          {
                            type: "Cloneable",
                            value: {
                              _styles: [
                                "wrapper-clone",
                                "wrapper-clone--button"
                              ],
                              items: [
                                {
                                  type: "Button",
                                  value: {
                                    _styles: ["button"],
                                    text: "See Others",
                                    iconName: "",
                                    iconType: "",
                                    fillType: "outline",
                                    tempFillType: "outline",
                                    paddingRL: 42,
                                    paddingRight: 42,
                                    paddingLeft: 42,
                                    paddingTB: 14,
                                    paddingTop: 14,
                                    paddingBottom: 14,
                                    borderRadiusType: "custom",
                                    borderRadius: 4,
                                    borderWidth: 2,
                                    borderColorOpacity: 1,
                                    borderColorPalette: "color8",
                                    bgColorOpacity: 0,
                                    bgColorPalette: "color3",
                                    hoverBgColorOpacity: 1,
                                    hoverBorderColorOpacity: 1,
                                    tempBorderColorPalette: "color8",
                                    colorPalette: "color8",
                                    colorOpacity: 1,
                                    hoverBgColorPalette: "color8",
                                    hoverBorderColorPalette: "color8",
                                    tempHoverBorderColorPalette: "color2",
                                    hoverBgColorHex: "#1c1c1c",
                                    tempHoverBgColorOpacity: 1,
                                    tempHoverBgColorPalette: "color3",
                                    hoverBorderColorHex: "#ffffff",
                                    tempBorderRadiusType: "custom",
                                    tempBorderRadius: 4,
                                    hoverColorPalette: "color2",
                                    hoverColorOpacity: 1
                                  }
                                },
                                {
                                  type: "Button",
                                  value: {
                                    _styles: ["button"],
                                    text: "Make Lemonade",
                                    iconName: "",
                                    iconType: "",
                                    fillType: "filled",
                                    tempFillType: "outline",
                                    paddingRL: 42,
                                    paddingRight: 42,
                                    paddingLeft: 42,
                                    paddingTB: 14,
                                    paddingTop: 14,
                                    paddingBottom: 14,
                                    borderRadiusType: "custom",
                                    borderRadius: 4,
                                    borderWidth: 2,
                                    borderColorOpacity: 1,
                                    borderColorPalette: "color3",
                                    bgColorOpacity: 1,
                                    bgColorPalette: "color3",
                                    hoverBgColorOpacity: 1,
                                    hoverBorderColorOpacity: 0.8,
                                    tempBorderColorPalette: "color3",
                                    colorPalette: "color2",
                                    colorOpacity: 1,
                                    hoverBgColorPalette: "color8",
                                    hoverBorderColorPalette: "color8",
                                    tempHoverBorderColorPalette: "color8",
                                    hoverBgColorHex: "#1c1c1c",
                                    tempHoverBgColorOpacity: 1,
                                    tempHoverBgColorPalette: "color3",
                                    hoverBorderColorHex: "#1c1c1c",
                                    tempBgColorPalette: "color3",
                                    hoverColorPalette: "color2",
                                    hoverColorOpacity: 1,
                                    tempBorderRadiusType: "custom",
                                    tempBorderRadius: 4
                                  }
                                }
                              ],
                              horizontalAlign: "left",
                              itemPadding: 20,
                              itemPaddingRight: 20,
                              itemPaddingLeft: 20
                            }
                          }
                        ],
                        verticalAlign: "center",
                        paddingLeft: 120,
                        paddingLeftSuffix: "px",
                        padding: 15,
                        tabsState: "tabNormal",
                        tabsCurrentElement: "tabCurrentElement",
                        tabsColor: "tabOverlay"
                      }
                    }
                  ]
                }
              }
            ],
            tabsCurrentElement: "tabCurrentElement",
            bgImageWidth: 2880,
            bgImageHeight: 1280,
            bgImageSrc: "d04-Img-Bg-ModernBuilding-black-1.jpg",
            bgColorOpacity: 0,
            tempBgColorOpacity: 0.8,
            borderRadius: 0,
            borderTopLeftRadius: 0,
            borderTopRightRadius: 0,
            borderBottomLeftRadius: 0,
            borderBottomRightRadius: 0,
            tempBorderTopLeftRadius: 0,
            tempBorderTopRightRadius: 0,
            tempBorderBottomLeftRadius: 0,
            tempBorderBottomRightRadius: 0,
            tabsState: "tabNormal",
            tabsColor: "tabOverlay",
            bgPositionX: 97,
            bgPositionY: 20,
            paddingType: "ungrouped",
            paddingTop: 75,
            paddingBottom: 75,
            padding: 75,
            bgColorPalette: "",
            tempBgColorPalette: "color2",
            bgColorHex: "#1c1c1c"
          }
        }
      ]
    }
  }
};
