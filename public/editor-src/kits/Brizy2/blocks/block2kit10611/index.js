module.exports = {
  id: "block2kit10611",
  thumbnailWidth: 600,
  thumbnailHeight: 284,
  title: "block2kit10611",

  keywords: "background,button,hero",
  cat: [5582, 100],
  type: 1,

  resolve: {
    type: "Section",
    value: {
      _styles: ["section"],
      items: [
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            items: [
              {
                type: "Row",
                value: {
                  _styles: ["row", "hide-row-borders", "padding-0"],
                  items: [
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-tp-abovetitle"><span style="opacity: 1;" class="brz-cp-color3"><span>INNOVATIVE DESIGN</span></span></p>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h1 class="brz-tp-heading1"><span class="brz-cp-color8"><span>Binary watch face smart watch because why not</span></span></h1>'
                                  }
                                }
                              ],
                              paddingType: "ungrouped",
                              paddingRightSuffix: "%",
                              paddingRight: 50,
                              padding: 0,
                              paddingSuffix: "px"
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-mb-lg-20 brz-mt-lg-10 brz-tp-subtitle"><span class="brz-cp-color6" style="opacity: 0.75;"><span>Before cell phones became so popular, watches were much more attractive to carry around because they are the most convenient way to keep track of the time. </span></span></p>'
                                  }
                                }
                              ],
                              paddingType: "ungrouped",
                              paddingRightSuffix: "%",
                              paddingRight: 60,
                              padding: 0,
                              paddingSuffix: "px"
                            }
                          },
                          {
                            type: "Cloneable",
                            value: {
                              _styles: [
                                "wrapper-clone",
                                "wrapper-clone--button"
                              ],
                              items: [
                                {
                                  type: "Button",
                                  value: {
                                    _styles: ["button"],
                                    text: "Buy Now",
                                    bgColorPalette: "color8",
                                    tempBgColorPalette: "color8",
                                    bgColorOpacity: 1,
                                    borderRadiusType: "custom",
                                    fillType: "filled",
                                    borderColorPalette: 0,
                                    tempBorderColorPalette: "color8",
                                    hoverBgColorOpacity: 1,
                                    colorPalette: "color2",
                                    colorOpacity: 1,
                                    iconName: "",
                                    iconType: "",
                                    borderWidth: 0,
                                    tempBorderWidth: 2,
                                    paddingRight: 42,
                                    paddingLeft: 42,
                                    borderRadius: 4,
                                    borderColorOpacity: 0,
                                    tempBorderRadiusType: "custom",
                                    hoverBorderColorOpacity: 0.8,
                                    tempBorderRadius: 4,
                                    hoverBgColorPalette: "color2",
                                    hoverBorderColorPalette: "",
                                    tempHoverBorderColorPalette: "",
                                    hoverBgColorHex: "#1c1c1c",
                                    tempHoverBgColorOpacity: 1,
                                    tempHoverBgColorPalette: "color3",
                                    hoverBorderColorHex: "#1c1c1c",
                                    tabletSize: "medium",
                                    tabletFontSize: 12,
                                    tabletPaddingTB: 14,
                                    tabletPaddingTop: 14,
                                    tabletPaddingBottom: 14,
                                    tempTabletPaddingTB: 14,
                                    tempTabletPaddingTop: 14,
                                    tempTabletPaddingBottom: 14,
                                    tabletPaddingRL: 42,
                                    tabletPaddingRight: 42,
                                    tabletPaddingLeft: 42,
                                    tempTabletPaddingRL: 42,
                                    tempTabletPaddingRight: 42,
                                    tempTabletPaddingLeft: 42,
                                    tabletBorderRadius: 4,
                                    hoverTransition: 30
                                  }
                                },
                                {
                                  type: "Button",
                                  value: {
                                    _styles: ["button"],
                                    text: "Learn More",
                                    bgColorPalette: "color3",
                                    tempBgColorPalette: "color3",
                                    bgColorOpacity: 1,
                                    borderRadiusType: "custom",
                                    fillType: "filled",
                                    borderColorPalette: 0,
                                    tempBorderColorPalette: 0,
                                    hoverBgColorOpacity: 1,
                                    colorPalette: "color8",
                                    colorOpacity: 1,
                                    iconName: "",
                                    iconType: "",
                                    borderWidth: 0,
                                    tempBorderWidth: 2,
                                    paddingRight: 42,
                                    paddingLeft: 42,
                                    borderRadius: 4,
                                    borderColorOpacity: 0,
                                    tempBorderRadiusType: "custom",
                                    hoverBorderColorOpacity: 0.8,
                                    tempBorderRadius: 4,
                                    hoverBgColorPalette: "color2",
                                    hoverBorderColorPalette: "",
                                    tempHoverBorderColorPalette: "",
                                    hoverBgColorHex: "#1c1c1c",
                                    tempHoverBgColorOpacity: 1,
                                    tempHoverBgColorPalette: "color3",
                                    hoverBorderColorHex: "#1c1c1c",
                                    tabletSize: "medium",
                                    tabletFontSize: 12,
                                    tabletPaddingTB: 14,
                                    tabletPaddingTop: 14,
                                    tabletPaddingBottom: 14,
                                    tempTabletPaddingTB: 14,
                                    tempTabletPaddingTop: 14,
                                    tempTabletPaddingBottom: 14,
                                    tabletPaddingRL: 42,
                                    tabletPaddingRight: 42,
                                    tabletPaddingLeft: 42,
                                    tempTabletPaddingRL: 42,
                                    tempTabletPaddingRight: 42,
                                    tempTabletPaddingLeft: 42,
                                    tabletBorderRadius: 4,
                                    hoverTransition: 30
                                  }
                                }
                              ],
                              horizontalAlign: "left"
                            }
                          }
                        ],
                        width: 100,
                        tabletPaddingRightSuffix: "%",
                        tabletPaddingRight: 20,
                        tabletPadding: 15,
                        tabletPaddingSuffix: "px",
                        tabletPaddingLeftSuffix: "px",
                        tabletPaddingLeft: 50,
                        mobilePaddingRightSuffix: "px",
                        mobilePaddingRight: 50,
                        mobilePadding: 0,
                        mobilePaddingSuffix: "px",
                        mobilePaddingLeftSuffix: "px",
                        mobilePaddingLeft: 50
                      }
                    }
                  ]
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--spacer"],
                  items: [
                    {
                      type: "Spacer",
                      value: {
                        _styles: ["spacer"],
                        height: 10
                      }
                    }
                  ],
                  showOnTablet: "off",
                  showOnMobile: "off"
                }
              }
            ],
            paddingType: "ungrouped",
            paddingTop: 180,
            paddingBottom: 180,
            padding: 75,
            tabsCurrentElement: "tabCurrentElement",
            bgImageWidth: 2880,
            bgImageHeight: 1520,
            bgImageSrc: "d03-Img-Bg-Watches.jpg",
            bgColorOpacity: 0.76,
            tempBgColorOpacity: 0.76,
            borderRadius: 0,
            borderTopLeftRadius: 0,
            borderTopRightRadius: 0,
            borderBottomLeftRadius: 0,
            borderBottomRightRadius: 0,
            tempBorderTopLeftRadius: 0,
            tempBorderTopRightRadius: 0,
            tempBorderBottomLeftRadius: 0,
            tempBorderBottomRightRadius: 0,
            tabsState: "tabNormal",
            tabsColor: "tabOverlay",
            mobileBgPositionX: 45,
            mobileBgPositionY: 0,
            mobileBorderRadius: 0,
            mobileBorderTopLeftRadius: 0,
            mobileBorderTopRightRadius: 0,
            mobileBorderBottomLeftRadius: 0,
            mobileBorderBottomRightRadius: 0,
            tempMobileBorderTopLeftRadius: 0,
            tempMobileBorderTopRightRadius: 0,
            tempMobileBorderBottomLeftRadius: 0,
            tempMobileBorderBottomRightRadius: 0,
            mobilePaddingType: "ungrouped",
            mobilePaddingTop: 25,
            mobilePaddingBottom: 59,
            mobilePadding: 25,
            tabletBgPositionX: 22,
            tabletBgPositionY: 17,
            tabletBorderRadius: 0,
            bgColorPalette: "color2",
            tempBgColorPalette: "color2",
            bgColorHex: "#1c1c1c"
          }
        },
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            items: [
              {
                type: "Row",
                value: {
                  _styles: ["row", "hide-row-borders", "padding-0"],
                  items: [
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-tp-abovetitle"><span style="opacity: 1;" class="brz-cp-color3"><span>INNOVATIVE DESIGN</span></span></p>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h1 class="brz-tp-heading1"><span class="brz-cp-color8"><span>Binary watch face smart watch because why not</span></span></h1>'
                                  }
                                }
                              ],
                              paddingType: "ungrouped",
                              paddingRightSuffix: "%",
                              paddingRight: 50,
                              padding: 0,
                              paddingSuffix: "px"
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-mb-lg-20 brz-mt-lg-10 brz-tp-subtitle"><span class="brz-cp-color6" style="opacity: 0.75;"><span>Before cell phones became so popular, watches were much more attractive to carry around because they are the most convenient way to keep track of the time. </span></span></p>'
                                  }
                                }
                              ],
                              paddingType: "ungrouped",
                              paddingRightSuffix: "%",
                              paddingRight: 60,
                              padding: 0,
                              paddingSuffix: "px"
                            }
                          },
                          {
                            type: "Cloneable",
                            value: {
                              _styles: [
                                "wrapper-clone",
                                "wrapper-clone--button"
                              ],
                              items: [
                                {
                                  type: "Button",
                                  value: {
                                    _styles: ["button"],
                                    text: "Buy Now",
                                    bgColorPalette: "color8",
                                    tempBgColorPalette: "color8",
                                    bgColorOpacity: 1,
                                    borderRadiusType: "custom",
                                    fillType: "filled",
                                    borderColorPalette: 0,
                                    tempBorderColorPalette: "color8",
                                    hoverBgColorOpacity: 1,
                                    colorPalette: "color2",
                                    colorOpacity: 1,
                                    iconName: "",
                                    iconType: "",
                                    borderWidth: 0,
                                    tempBorderWidth: 2,
                                    paddingRight: 42,
                                    paddingLeft: 42,
                                    borderRadius: 4,
                                    borderColorOpacity: 0,
                                    tempBorderRadiusType: "custom",
                                    hoverBorderColorOpacity: 0.8,
                                    tempBorderRadius: 4,
                                    hoverBgColorPalette: "color2",
                                    hoverBorderColorPalette: "",
                                    tempHoverBorderColorPalette: "",
                                    hoverBgColorHex: "#1c1c1c",
                                    tempHoverBgColorOpacity: 1,
                                    tempHoverBgColorPalette: "color3",
                                    hoverBorderColorHex: "#1c1c1c",
                                    tabletSize: "medium",
                                    tabletFontSize: 12,
                                    tabletPaddingTB: 14,
                                    tabletPaddingTop: 14,
                                    tabletPaddingBottom: 14,
                                    tempTabletPaddingTB: 14,
                                    tempTabletPaddingTop: 14,
                                    tempTabletPaddingBottom: 14,
                                    tabletPaddingRL: 42,
                                    tabletPaddingRight: 42,
                                    tabletPaddingLeft: 42,
                                    tempTabletPaddingRL: 42,
                                    tempTabletPaddingRight: 42,
                                    tempTabletPaddingLeft: 42,
                                    tabletBorderRadius: 4,
                                    hoverTransition: 30
                                  }
                                },
                                {
                                  type: "Button",
                                  value: {
                                    _styles: ["button"],
                                    text: "Learn More",
                                    bgColorPalette: "color3",
                                    tempBgColorPalette: "color3",
                                    bgColorOpacity: 1,
                                    borderRadiusType: "custom",
                                    fillType: "filled",
                                    borderColorPalette: 0,
                                    tempBorderColorPalette: 0,
                                    hoverBgColorOpacity: 1,
                                    colorPalette: "color8",
                                    colorOpacity: 1,
                                    iconName: "",
                                    iconType: "",
                                    borderWidth: 0,
                                    tempBorderWidth: 2,
                                    paddingRight: 42,
                                    paddingLeft: 42,
                                    borderRadius: 4,
                                    borderColorOpacity: 0,
                                    tempBorderRadiusType: "custom",
                                    hoverBorderColorOpacity: 0.8,
                                    tempBorderRadius: 4,
                                    hoverBgColorPalette: "color2",
                                    hoverBorderColorPalette: "",
                                    tempHoverBorderColorPalette: "",
                                    hoverBgColorHex: "#1c1c1c",
                                    tempHoverBgColorOpacity: 1,
                                    tempHoverBgColorPalette: "color3",
                                    hoverBorderColorHex: "#1c1c1c",
                                    tabletSize: "medium",
                                    tabletFontSize: 12,
                                    tabletPaddingTB: 14,
                                    tabletPaddingTop: 14,
                                    tabletPaddingBottom: 14,
                                    tempTabletPaddingTB: 14,
                                    tempTabletPaddingTop: 14,
                                    tempTabletPaddingBottom: 14,
                                    tabletPaddingRL: 42,
                                    tabletPaddingRight: 42,
                                    tabletPaddingLeft: 42,
                                    tempTabletPaddingRL: 42,
                                    tempTabletPaddingRight: 42,
                                    tempTabletPaddingLeft: 42,
                                    tabletBorderRadius: 4,
                                    hoverTransition: 30
                                  }
                                }
                              ],
                              horizontalAlign: "left"
                            }
                          }
                        ],
                        width: 100,
                        tabletPaddingRightSuffix: "%",
                        tabletPaddingRight: 20,
                        tabletPadding: 15,
                        tabletPaddingSuffix: "px",
                        tabletPaddingLeftSuffix: "px",
                        tabletPaddingLeft: 50,
                        mobilePaddingRightSuffix: "px",
                        mobilePaddingRight: 50,
                        mobilePadding: 0,
                        mobilePaddingSuffix: "px",
                        mobilePaddingLeftSuffix: "px",
                        mobilePaddingLeft: 50
                      }
                    }
                  ]
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--spacer"],
                  items: [
                    {
                      type: "Spacer",
                      value: {
                        _styles: ["spacer"],
                        height: 10
                      }
                    }
                  ],
                  showOnTablet: "off",
                  showOnMobile: "off"
                }
              }
            ],
            paddingType: "ungrouped",
            paddingTop: 180,
            paddingBottom: 180,
            padding: 75,
            tabsCurrentElement: "tabCurrentElement",
            bgImageWidth: 2880,
            bgImageHeight: 1520,
            bgImageSrc: "d03-Img-Bg-Watches.jpg",
            bgColorOpacity: 0.76,
            tempBgColorOpacity: 0.76,
            borderRadius: 0,
            borderTopLeftRadius: 0,
            borderTopRightRadius: 0,
            borderBottomLeftRadius: 0,
            borderBottomRightRadius: 0,
            tempBorderTopLeftRadius: 0,
            tempBorderTopRightRadius: 0,
            tempBorderBottomLeftRadius: 0,
            tempBorderBottomRightRadius: 0,
            tabsState: "tabNormal",
            tabsColor: "tabOverlay",
            mobileBgPositionX: 45,
            mobileBgPositionY: 0,
            mobileBorderRadius: 0,
            mobileBorderTopLeftRadius: 0,
            mobileBorderTopRightRadius: 0,
            mobileBorderBottomLeftRadius: 0,
            mobileBorderBottomRightRadius: 0,
            tempMobileBorderTopLeftRadius: 0,
            tempMobileBorderTopRightRadius: 0,
            tempMobileBorderBottomLeftRadius: 0,
            tempMobileBorderBottomRightRadius: 0,
            mobilePaddingType: "ungrouped",
            mobilePaddingTop: 25,
            mobilePaddingBottom: 59,
            mobilePadding: 25,
            tabletBgPositionX: 22,
            tabletBgPositionY: 17,
            tabletBorderRadius: 0,
            bgColorPalette: "color2",
            tempBgColorPalette: "color2",
            bgColorHex: "#1c1c1c"
          }
        }
      ],
      slider: "on",
      sliderDotsColorPalette: "color6",
      tempSliderDotsColorPalette: "color6",
      sliderDotsColorOpacity: 1,
      sliderArrowsColorPalette: "color8",
      tempSliderArrowsColorPalette: "color8",
      sliderArrowsColorOpacity: 0.7,
      sliderArrows: "thin",
      sliderAutoPlay: "on",
      sliderAutoPlaySpeed: 5
    }
  }
};
