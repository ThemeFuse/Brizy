module.exports = {
  id: "block2kit1426",
  thumbnailWidth: 600,
  thumbnailHeight: 267,
  title: "block2kit1426",

  keywords: "button,hero,image",
  cat: [5582, 100],
  type: 0,

  resolve: {
    type: "Section",
    value: {
      _styles: ["section"],
      items: [
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            items: [
              {
                type: "Row",
                value: {
                  _styles: ["row", "hide-row-borders", "padding-0"],
                  items: [
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h2 class="brz-text-xs-center brz-tp-heading2"><span class="brz-cp-color2">If the wind will not serve, take to the oars.</span></h2>'
                                  }
                                }
                              ],
                              paddingType: "ungrouped",
                              paddingRightSuffix: "px",
                              paddingRight: 20,
                              padding: 0,
                              paddingSuffix: "px"
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 10,
                                    tabletHeight: 0,
                                    mobileHeight: 0
                                  }
                                }
                              ],
                              showOnTablet: "off",
                              showOnMobile: "off"
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-tp-subtitle brz-text-xs-center"><span class="brz-cp-color7"><span>The more the better. So you will get also a lot of keywords which you must pay for only the minimal commandment of 5 cents.</span></span></p>'
                                  }
                                }
                              ],
                              paddingType: "ungrouped",
                              paddingRightSuffix: "px",
                              paddingRight: 35,
                              padding: 0,
                              paddingSuffix: "px"
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 20,
                                    tabletHeight: 0,
                                    mobileHeight: 0
                                  }
                                }
                              ],
                              showOnTablet: "off",
                              showOnMobile: "off"
                            }
                          },
                          {
                            type: "Cloneable",
                            value: {
                              _styles: [
                                "wrapper-clone",
                                "wrapper-clone--button"
                              ],
                              items: [
                                {
                                  type: "Button",
                                  value: {
                                    _styles: ["button"],
                                    text: "Get iOS Version",
                                    iconName: "apple",
                                    iconType: "glyph",
                                    iconPosition: "left",
                                    borderRadiusType: "custom",
                                    tempBorderRadiusType: "custom",
                                    paddingRight: 26,
                                    paddingLeft: 26,
                                    fillType: "filled",
                                    borderRadius: 4,
                                    borderWidth: 2,
                                    borderColorOpacity: 1,
                                    borderColorPalette: "color2",
                                    bgColorOpacity: 1,
                                    bgColorPalette: "color2",
                                    hoverBgColorOpacity: 1,
                                    hoverBorderColorOpacity: 1,
                                    tempBorderRadius: 4,
                                    tempBorderWidth: 2,
                                    hoverBgColorPalette: "color3",
                                    hoverBorderColorPalette: "color3",
                                    tempHoverBorderColorPalette: "",
                                    hoverBgColorHex: "#1c1c1c",
                                    tempHoverBgColorOpacity: 1,
                                    tempHoverBgColorPalette: "color3",
                                    hoverBorderColorHex: "#0ad1bd",
                                    fontStyle: "button",
                                    size: "small",
                                    fontSize: 11,
                                    paddingTB: 11,
                                    paddingTop: 11,
                                    paddingBottom: 11,
                                    tempPaddingTB: 11,
                                    tempPaddingTop: 11,
                                    tempPaddingBottom: 11,
                                    paddingRL: 26,
                                    tempPaddingRL: 26,
                                    tempPaddingRight: 26,
                                    tempPaddingLeft: 26,
                                    tempBgColorPalette: "color2",
                                    tempBorderColorPalette: "color2",
                                    hoverTransition: 30
                                  }
                                },
                                {
                                  type: "Button",
                                  value: {
                                    _styles: ["button"],
                                    text: "Android Version",
                                    iconName: "android",
                                    iconType: "glyph",
                                    iconPosition: "left",
                                    borderRadiusType: "custom",
                                    tempBorderRadiusType: "custom",
                                    paddingRight: 26,
                                    paddingLeft: 26,
                                    fillType: "outline",
                                    borderRadius: 4,
                                    borderWidth: 2,
                                    borderColorOpacity: 1,
                                    borderColorPalette: "",
                                    bgColorOpacity: 0,
                                    bgColorPalette: "color2",
                                    hoverBgColorOpacity: 1,
                                    hoverBorderColorOpacity: 0,
                                    tempBorderRadius: 4,
                                    tempBorderWidth: 2,
                                    hoverBgColorPalette: "color3",
                                    hoverBorderColorPalette: "color3",
                                    tempHoverBorderColorPalette: "",
                                    hoverBgColorHex: "#1c1c1c",
                                    tempHoverBgColorOpacity: 1,
                                    tempHoverBgColorPalette: "color3",
                                    hoverBorderColorHex: "#0ad1bd",
                                    fontStyle: "button",
                                    size: "small",
                                    fontSize: 11,
                                    paddingTB: 11,
                                    paddingTop: 11,
                                    paddingBottom: 11,
                                    tempPaddingTB: 11,
                                    tempPaddingTop: 11,
                                    tempPaddingBottom: 11,
                                    paddingRL: 26,
                                    tempPaddingRL: 26,
                                    tempPaddingRight: 26,
                                    tempPaddingLeft: 26,
                                    tempBgColorPalette: "color2",
                                    tempBorderColorPalette: "",
                                    bgColorHex: "#1c1c1c",
                                    tempBgColorOpacity: 1,
                                    borderColorHex: "#1c1c1c",
                                    colorPalette: "color2",
                                    colorOpacity: 1,
                                    hoverTransition: 30
                                  }
                                }
                              ],
                              horizontalAlign: "left",
                              itemPadding: 20,
                              itemPaddingRight: 20,
                              itemPaddingLeft: 20,
                              mobileItemPadding: 15,
                              mobileItemPaddingRight: 15,
                              mobileItemPaddingLeft: 15,
                              mobileHorizontalAlign: "center"
                            }
                          }
                        ],
                        paddingLeft: 15,
                        paddingLeftSuffix: "px",
                        padding: 15,
                        paddingRight: 40,
                        paddingRightSuffix: "px",
                        tabsState: "tabNormal",
                        tabsCurrentElement: "tabCurrentElement",
                        tabsColor: "tabOverlay",
                        width: 43,
                        verticalAlign: "center",
                        tabletWidth: 50,
                        mobilePaddingRightSuffix: "px",
                        mobilePaddingRight: 20,
                        mobilePadding: 0,
                        mobilePaddingSuffix: "px",
                        mobilePaddingLeftSuffix: "px",
                        mobilePaddingLeft: 20,
                        paddingSuffix: "px"
                      }
                    },
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--image"],
                              items: [
                                {
                                  type: "Image",
                                  value: {
                                    _styles: ["image"],
                                    imageWidth: 1498,
                                    imageHeight: 880,
                                    imageSrc: "d09-laptop-mockup.png",
                                    height: 100,
                                    positionX: 50,
                                    positionY: 50,
                                    imagePopulation: "",
                                    resize: 100,
                                    mobileResize: 90
                                  }
                                }
                              ],
                              marginType: "grouped"
                            }
                          }
                        ],
                        width: 57,
                        tabletWidth: 50,
                        verticalAlign: "center",
                        tabsState: "tabNormal",
                        tabsCurrentElement: "tabCurrentElement",
                        tabsColor: "tabOverlay",
                        paddingType: "grouped",
                        padding: 10,
                        paddingSuffix: "px",
                        paddingTop: 10,
                        paddingRight: 10,
                        paddingBottom: 10,
                        paddingLeft: 10
                      }
                    }
                  ],
                  mobileReverseColumns: "on"
                }
              }
            ],
            paddingType: "ungrouped",
            paddingTop: 100,
            paddingBottom: 100,
            padding: 75
          }
        }
      ]
    }
  }
};
