module.exports = {
  id: "block1kit1540",
  thumbnailWidth: 600,
  thumbnailHeight: 222,
  title: "block1kit1540",

  keywords: "button,call to action,cover,hero,image background,logos,partners",
  cat: [4395, 100],
  type: 1,

  resolve: {
    type: "Section",
    value: {
      _styles: ["section"],
      items: [
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            bgColorHex: "#191b21",
            bgColorOpacity: 0.9,
            paddingBottom: 0,
            containerType: "fullWidth",
            padding: 90,
            tempBgColorOpacity: 0.9,
            paddingType: "ungrouped",
            items: [
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--richText"],
                  marginBottom: 5,
                  items: [
                    {
                      type: "RichText",
                      value: {
                        _styles: ["richText"],
                        text:
                          '<h2 class="brz-text-lg-center brz-tp-heading2"><span class="brz-cp-color8">Want to work together?</span></h2>'
                      }
                    }
                  ]
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--richText"],
                  marginTop: 5,
                  items: [
                    {
                      type: "RichText",
                      value: {
                        _styles: ["richText"],
                        text:
                          '<p class="brz-tp-subtitle brz-text-lg-center brz-mb-lg-20"><em class="brz-cp-color7">Leave us a message and we\'ll get in touch!</em></p>'
                      }
                    }
                  ]
                }
              },
              {
                type: "Cloneable",
                value: {
                  _styles: ["wrapper-clone", "wrapper-clone--button"],
                  items: [
                    {
                      type: "Button",
                      value: {
                        _styles: ["button"],
                        size: "large",
                        fontSize: 13,
                        paddingTB: 19,
                        paddingTop: 19,
                        paddingBottom: 19,
                        tempPaddingTB: 19,
                        tempPaddingTop: 19,
                        tempPaddingBottom: 19,
                        paddingRL: 44,
                        paddingRight: 44,
                        paddingLeft: 44,
                        tempPaddingRL: 44,
                        tempPaddingRight: 44,
                        tempPaddingLeft: 44,
                        borderRadiusType: "rounded",
                        tempBorderRadiusType: "rounded",
                        borderRadius: 33,
                        mobileBorderRadius: 24,
                        borderColorPalette: "color8",
                        hoverBorderColorPalette: "",
                        tempHoverBorderColorPalette: "",
                        hoverBgColorHex: "#142850",
                        hoverBgColorOpacity: 1,
                        hoverBorderColorHex: "#142850",
                        tempHoverBgColorOpacity: 1,
                        text: "CONTACT US",
                        bgColorHex: "#b76969",
                        bgColorPalette: "color8",
                        tempBgColorPalette: "color8",
                        colorPalette: "color3",
                        tempBorderColorPalette: "color8",
                        borderColorHex: "#b76969"
                      }
                    }
                  ]
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--spacer"],
                  items: [
                    {
                      type: "Spacer",
                      value: {
                        _styles: ["spacer"],
                        height: 75,
                        mobileHeight: 75,
                        tabletHeight: 35
                      }
                    }
                  ],
                  showOnMobile: "off"
                }
              },
              {
                type: "Row",
                value: {
                  _styles: ["row", "hide-row-borders", "padding-0"],
                  items: [
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        width: 32.3,
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--image"],
                              items: [
                                {
                                  type: "Image",
                                  value: {
                                    _styles: ["image"],
                                    imageSrc:
                                      "d42adedfa61513ba75ed9fa3736efb0bdcf97a44.png",
                                    imageWidth: 220,
                                    imageHeight: 65,
                                    resize: 42,
                                    mobileResize: 48,
                                    height: 52,
                                    mobileHeight: 50,
                                    tabletResize: 62
                                  }
                                }
                              ],
                              horizontalAlign: "right",
                              paddingType: "ungrouped",
                              paddingTop: 40,
                              paddingBottom: 40,
                              mobileHorizontalAlign: "center"
                            }
                          }
                        ],
                        borderWidth: 1,
                        borderTopWidth: 1,
                        tempBorderWidth: 1,
                        tempBorderTopWidth: 1,
                        tempBorderRightWidth: 0,
                        tempBorderBottomWidth: 0,
                        tempBorderLeftWidth: 0,
                        borderColorOpacity: 0.15,
                        borderTopLeftRadius: 2,
                        borderTopRightRadius: 2,
                        borderBottomLeftRadius: 2,
                        borderBottomRightRadius: 2,
                        tempBorderTopLeftRadius: 2,
                        tempBorderTopRightRadius: 2,
                        tempBorderBottomRightRadius: 2,
                        tempBorderBottomLeftRadius: 2,
                        borderWidthType: "ungrouped",
                        verticalAlign: "center",
                        borderColorHex: "#ffffff",
                        tempBorderColorOpacity: 0.15
                      }
                    },
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        width: 17.7,
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--image"],
                              items: [
                                {
                                  type: "Image",
                                  value: {
                                    _styles: ["image"],
                                    imageSrc:
                                      "7d42404e1ce99b24a62d58ddcc4771d79aad4118.png",
                                    imageWidth: 220,
                                    imageHeight: 65,
                                    mobileHeight: 109,
                                    resize: 70,
                                    mobileResize: 50,
                                    tabletResize: 100
                                  }
                                }
                              ],
                              paddingType: "ungrouped",
                              paddingTop: 30,
                              paddingBottom: 30
                            }
                          }
                        ],
                        borderWidth: 1,
                        borderTopWidth: 1,
                        borderLeftWidth: 1,
                        tempBorderWidth: 1,
                        tempBorderTopWidth: 1,
                        tempBorderRightWidth: 0,
                        tempBorderBottomWidth: 0,
                        tempBorderLeftWidth: 1,
                        borderColorOpacity: 0.15,
                        borderTopLeftRadius: 4,
                        borderTopRightRadius: 4,
                        borderBottomLeftRadius: 4,
                        borderBottomRightRadius: 4,
                        tempBorderTopLeftRadius: 4,
                        tempBorderTopRightRadius: 4,
                        tempBorderBottomRightRadius: 4,
                        tempBorderBottomLeftRadius: 4,
                        borderWidthType: "ungrouped",
                        verticalAlign: "center",
                        borderColorHex: "#ffffff",
                        tempBorderColorOpacity: 0.15
                      }
                    },
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        width: 17.7,
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--image"],
                              items: [
                                {
                                  type: "Image",
                                  value: {
                                    _styles: ["image"],
                                    imageSrc:
                                      "1073f2ab4cd44836ae957bad9e6275bf5bd9c1f3.png",
                                    imageWidth: 220,
                                    imageHeight: 65,
                                    resize: 70,
                                    mobileResize: 50,
                                    height: 98,
                                    tabletResize: 90
                                  }
                                }
                              ],
                              paddingType: "ungrouped",
                              paddingTop: 30,
                              paddingBottom: 30
                            }
                          }
                        ],
                        borderWidth: 1,
                        borderTopWidth: 1,
                        borderLeftWidth: 1,
                        tempBorderWidth: 1,
                        tempBorderTopWidth: 1,
                        tempBorderRightWidth: 0,
                        tempBorderBottomWidth: 0,
                        tempBorderLeftWidth: 1,
                        borderColorOpacity: 0.15,
                        borderTopLeftRadius: 4,
                        borderTopRightRadius: 4,
                        borderBottomLeftRadius: 4,
                        borderBottomRightRadius: 4,
                        tempBorderTopLeftRadius: 4,
                        tempBorderTopRightRadius: 4,
                        tempBorderBottomRightRadius: 4,
                        tempBorderBottomLeftRadius: 4,
                        borderWidthType: "ungrouped",
                        verticalAlign: "center",
                        borderColorHex: "#ffffff",
                        tempBorderColorOpacity: 0.15
                      }
                    },
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        width: 32.3,
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--image"],
                              items: [
                                {
                                  type: "Image",
                                  value: {
                                    _styles: ["image"],
                                    imageSrc:
                                      "fd25012edbda5c51236373a86ecd1c3a513d99d1.png",
                                    imageWidth: 220,
                                    imageHeight: 65,
                                    resize: 42,
                                    height: 55,
                                    mobileResize: 54,
                                    tabletResize: 55
                                  }
                                }
                              ],
                              horizontalAlign: "left",
                              paddingType: "ungrouped",
                              paddingTop: 40,
                              paddingBottom: 40,
                              mobileHorizontalAlign: "center"
                            }
                          }
                        ],
                        borderWidth: 1,
                        borderTopWidth: 1,
                        borderLeftWidth: 1,
                        tempBorderWidth: 1,
                        tempBorderTopWidth: 1,
                        tempBorderRightWidth: 0,
                        tempBorderBottomWidth: 0,
                        tempBorderLeftWidth: 1,
                        borderColorOpacity: 0.15,
                        borderTopLeftRadius: 4,
                        borderTopRightRadius: 4,
                        borderBottomLeftRadius: 4,
                        borderBottomRightRadius: 4,
                        tempBorderTopLeftRadius: 4,
                        tempBorderTopRightRadius: 4,
                        tempBorderBottomRightRadius: 4,
                        tempBorderBottomLeftRadius: 4,
                        borderWidthType: "ungrouped",
                        verticalAlign: "center",
                        borderColorHex: "#ffffff",
                        tempBorderColorOpacity: 0.15
                      }
                    }
                  ]
                }
              }
            ],
            bgImageWidth: 1920,
            bgImageHeight: 900,
            bgImageSrc: "aabcb23b967eb7d80787877fc43397532f91fe94.jpeg",
            mobileBgImageWidth: 1920,
            mobileBgImageHeight: 900,
            mobileBgImageSrc: "aabcb23b967eb7d80787877fc43397532f91fe94.jpeg",
            mobileBgColorHex: "#191b21",
            mobileBgColorOpacity: 0.9,
            bgPositionY: 100,
            mobileBgPositionY: 100,
            mobilePaddingRightSuffix: "px",
            tempMobilePaddingRightSuffix: "px",
            mobilePaddingSuffix: "px",
            tempMobilePaddingSuffix: "px",
            mobilePaddingRight: 15,
            tempMobilePaddingRight: 15,
            mobilePaddingLeftSuffix: "px",
            tempMobilePaddingLeftSuffix: "px",
            mobilePaddingLeft: 15,
            tempMobilePaddingLeft: 15
          }
        }
      ]
    }
  }
};
