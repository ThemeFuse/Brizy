module.exports = {
  id: "block1kit1830",
  thumbnailWidth: 600,
  thumbnailHeight: 339,
  title: "block1kit1830",

  keywords: "Blog,header,Post",
  cat: [4395, 100],
  type: 0,

  resolve: {
    type: "Section",
    value: {
      _styles: ["section"],
      items: [
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            bgColorHex: "#ffffff",
            bgColorOpacity: 0.85,
            paddingTop: 200,
            paddingBottom: 200,
            padding: 250,
            tempBgColorOpacity: 0.85,
            items: [
              {
                type: "Row",
                value: {
                  _styles: ["row", "hide-row-borders", "padding-0"],
                  items: [
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-fs-lg-80 brz-ff-montserrat brz-ls-lg-m_1_5 brz-fs-xs-34 brz-lh-xs-1_3 brz-lh-lg-1_3 brz-text-lg-left brz-text-xs-center brz-fs-sm-50"><strong class="brz-cp-color2">Start reading quality&nbsp;news &amp;</strong><strong style="color: rgb(255, 255, 255);">&nbsp;</strong><strong class="brz-cp-color3">articles</strong><strong style="color: rgb(255, 255, 255);">.</strong></p>'
                                  }
                                }
                              ],
                              paddingType: "ungrouped",
                              paddingRight: 150,
                              marginTop: 0
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-tp-heading6 brz-text-xs-center"><span style="opacity: 1;" class="brz-cp-color7">Subscribe to our newsletter feed and receive all the news instantly</span></p>'
                                  }
                                }
                              ]
                            }
                          }
                        ],
                        width: 59.7,
                        tabletWidth: 48
                      }
                    },
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-mt-lg-10 brz-text-lg-center brz-tp-heading6 brz-mb-lg-10"><span class="brz-cp-color2">SIGN UP FOR FREE</span></p>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--form2"],
                              items: [
                                {
                                  type: "Form2",
                                  value: {
                                    _styles: ["form2"],
                                    items: [
                                      {
                                        type: "Form2Fields",
                                        value: {
                                          items: [
                                            {
                                              type: "Form2Field",
                                              value: {
                                                type: "Text",
                                                label: "Your Name",
                                                required: "on",
                                                options: [
                                                  "Option 1",
                                                  "Option 2"
                                                ],
                                                placeholder: "Your Name"
                                              }
                                            },
                                            {
                                              type: "Form2Field",
                                              value: {
                                                type: "Email",
                                                label: "Email Address",
                                                required: "on",
                                                options: [
                                                  "Option 1",
                                                  "Option 2"
                                                ],
                                                placeholder: "Email Address"
                                              }
                                            }
                                          ],
                                          tabsState: "",
                                          tabsColor: "",
                                          bgColorHex: "#ffffff",
                                          bgColorOpacity: 0,
                                          tempBgColorOpacity: 1,
                                          bgColorPalette: "",
                                          tempBgColorPalette: "",
                                          borderRadius: 0,
                                          borderTopLeftRadius: 0,
                                          borderTopRightRadius: 0,
                                          borderBottomLeftRadius: 0,
                                          borderBottomRightRadius: 0,
                                          tempBorderTopLeftRadius: 0,
                                          tempBorderTopRightRadius: 0,
                                          tempBorderBottomLeftRadius: 0,
                                          tempBorderBottomRightRadius: 0,
                                          fontStyle: "paragraph",
                                          borderColorPalette: "color7",
                                          tempBorderColorPalette: "color7",
                                          borderColorOpacity: 0.7,
                                          borderStyle: "solid",
                                          borderWidth: 1,
                                          borderTopWidth: 1,
                                          borderRightWidth: 1,
                                          borderBottomWidth: 1,
                                          borderLeftWidth: 1,
                                          tempBorderTopWidth: 1,
                                          tempBorderRightWidth: 1,
                                          tempBorderBottomWidth: 1,
                                          tempBorderLeftWidth: 1,
                                          borderColorHex: "#73777f",
                                          tempBorderColorOpacity: 0.7,
                                          colorHex: "#73777f",
                                          colorOpacity: 0.85,
                                          tempColorOpacity: 0.85,
                                          colorPalette: "",
                                          tempColorPalette: ""
                                        }
                                      },
                                      {
                                        type: "Button",
                                        value: {
                                          _styles: ["button", "submit"],
                                          text: "SIGNUP NOW TO NEWSLETTER",
                                          iconName: "newsletter",
                                          iconType: "outline",
                                          iconPosition: "left",
                                          tabsState: "",
                                          tabsColor: "",
                                          hoverBgColorPalette: "color2",
                                          hoverBgColorOpacity: 1,
                                          hoverBorderColorPalette: "",
                                          tempHoverBorderColorPalette: "",
                                          hoverBgColorHex: "#142850",
                                          tempHoverBgColorOpacity: 1,
                                          tempHoverBgColorPalette: "color3",
                                          hoverBorderColorHex: "#142850",
                                          hoverBorderColorOpacity: 0,
                                          tabletSize: "small",
                                          tabletFontSize: 11,
                                          tabletPaddingTB: 11,
                                          tabletPaddingTop: 11,
                                          tabletPaddingBottom: 11,
                                          tempTabletPaddingTB: 11,
                                          tempTabletPaddingTop: 11,
                                          tempTabletPaddingBottom: 11,
                                          tabletPaddingRL: 26,
                                          tabletPaddingRight: 26,
                                          tabletPaddingLeft: 26,
                                          tempTabletPaddingRL: 26,
                                          tempTabletPaddingRight: 26,
                                          tempTabletPaddingLeft: 26,
                                          tabletBorderRadius: 2
                                        }
                                      }
                                    ]
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-tp-paragraph brz-text-lg-center brz-mb-lg-10 brz-mt-lg-10 brz-text-xs-center"><em class="brz-cp-color7">By clicking "Sign up" you agree to our&nbsp;</em><em class="brz-cp-color3">Terms of Service.</em></p>'
                                  }
                                }
                              ],
                              paddingType: "ungrouped",
                              paddingRight: 49,
                              paddingRightSuffix: "px",
                              padding: 0,
                              paddingLeft: 49,
                              paddingLeftSuffix: "px"
                            }
                          }
                        ],
                        width: 40.3,
                        paddingType: "grouped",
                        padding: 30,
                        paddingSuffix: "px",
                        paddingTop: 30,
                        paddingRight: 30,
                        paddingBottom: 30,
                        paddingLeft: 30,
                        verticalAlign: "center",
                        tabletWidth: 51.4
                      }
                    }
                  ]
                }
              }
            ],
            bgImageWidth: 1920,
            bgImageHeight: 840,
            bgImageSrc: "c8955b22d10fb349cd0bd74ee93517e4.jpg",
            mobilePadding: 60,
            mobilePaddingTop: 60,
            mobilePaddingBottom: 60,
            bgPositionX: 50,
            bgPositionY: 50,
            tempMobileBgColorOpacity: 1,
            bgColorPalette: "color8",
            containerType: "boxed",
            paddingType: "ungrouped",
            mobilePaddingRightSuffix: "px",
            tempMobilePaddingRightSuffix: "px",
            mobilePaddingSuffix: "px",
            tempMobilePaddingSuffix: "px",
            mobilePaddingRight: 15,
            tempMobilePaddingRight: 15,
            mobilePaddingLeftSuffix: "px",
            tempMobilePaddingLeftSuffix: "px",
            mobilePaddingLeft: 15,
            tempMobilePaddingLeft: 15,
            mobilePaddingType: "ungrouped"
          }
        }
      ]
    }
  }
};
