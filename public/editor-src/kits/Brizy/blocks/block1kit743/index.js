module.exports = {
  id: "block1kit743",
  thumbnailWidth: 600,
  thumbnailHeight: 348,
  title: "block1kit743",

  keywords:
    "button,features,icon box,image background,projects,services,three columns",
  cat: [4447, 100],
  type: 1,

  resolve: {
    type: "Section",
    value: {
      _styles: ["section"],
      items: [
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            bgColorHex: "#191b21",
            bgColorOpacity: 1,
            mobileBgColorHex: "#191b21",
            mobileBgColorOpacity: 1,
            items: [
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--richText"],
                  items: [
                    {
                      type: "RichText",
                      value: {
                        _styles: ["richText"],
                        text:
                          '<p class="brz-tp-abovetitle brz-text-lg-center"><span class="brz-cp-color3">FEATURES &amp; SERVICES</span></p>'
                      }
                    }
                  ],
                  marginBottom: 0
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--richText"],
                  items: [
                    {
                      type: "RichText",
                      value: {
                        _styles: ["richText"],
                        text:
                          '<h2 class="brz-text-lg-center brz-tp-heading2"><span class="brz-cp-color8">Services We Can Help You With</span></h2>'
                      }
                    }
                  ]
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--spacer"],
                  items: [
                    {
                      type: "Spacer",
                      value: {
                        _styles: ["spacer"],
                        height: 15,
                        mobileHeight: 15
                      }
                    }
                  ],
                  showOnMobile: "off"
                }
              },
              {
                type: "Row",
                value: {
                  _styles: ["row", "hide-row-borders", "padding-0"],
                  items: [
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        width: 33.3,
                        items: [
                          {
                            type: "Cloneable",
                            value: {
                              _styles: ["wrapper-clone", "wrapper-clone--icon"],
                              items: [
                                {
                                  type: "Icon",
                                  value: {
                                    _styles: ["icon"],
                                    name: "desktop",
                                    size: "large",
                                    customSize: 64,
                                    type: "outline",
                                    bgColorHex: "#009900",
                                    tempBgColorOpacity: 0.8,
                                    tempFillType: "outline",
                                    hoverBorderColorHex: "#009900",
                                    borderColorHex: "#cccccc",
                                    tempBorderColorOpacity: 0.7,
                                    hoverBgColorHex: "#216fbd",
                                    borderRadius: 35,
                                    mobileSize: "large",
                                    mobileCustomSize: 64,
                                    tempPadding: 0,
                                    colorHex: "#ffffff",
                                    hoverColorHex: "#ffffff",
                                    hoverColorOpacity: 1,
                                    colorPalette: "color8",
                                    hoverColorPalette: "color8"
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h4 class="brz-text-lg-center brz-tp-heading4"><span class="brz-cp-color8">Web Design</span></h4>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-tp-paragraph brz-text-lg-center"><span class="brz-cp-color7" style="background-color: transparent;"><span>This is a front-end page builder where what you see is what you get. We designed it so you can be in control.</span></span></p>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 10,
                                    mobileHeight: 10
                                  }
                                }
                              ],
                              showOnTablet: "off",
                              showOnMobile: "off"
                            }
                          },
                          {
                            type: "Cloneable",
                            value: {
                              _styles: [
                                "wrapper-clone",
                                "wrapper-clone--button"
                              ],
                              items: [
                                {
                                  type: "Button",
                                  value: {
                                    _styles: ["button"],
                                    size: "small",
                                    paddingTB: 11,
                                    paddingTop: 11,
                                    paddingBottom: 11,
                                    tempPaddingTB: 11,
                                    tempPaddingTop: 11,
                                    tempPaddingBottom: 11,
                                    paddingRL: 26,
                                    paddingRight: 26,
                                    paddingLeft: 26,
                                    tempPaddingRL: 26,
                                    tempPaddingRight: 26,
                                    tempPaddingLeft: 26,
                                    tempFillType: "outline",
                                    bgColorPalette: "color1",
                                    tempBgColorPalette: "color1",
                                    bgColorHex: "#191b21",
                                    bgColorOpacity: 0,
                                    fillType: "outline",
                                    colorPalette: "color3",
                                    iconName: "",
                                    iconType: "",
                                    text: "FIND OUT MORE",
                                    hoverBgColorOpacity: 1,
                                    borderColorOpacity: 1,
                                    tempHoverBgColorOpacity: 1,
                                    fontWeight: 600,
                                    mobileFontSize: 12,
                                    fontSize: 11,
                                    hoverBorderColorOpacity: 0.8
                                  }
                                }
                              ]
                            }
                          }
                        ],
                        bgColorOpacity: 0.9,
                        tempBgColorOpacity: 0.9,
                        bgColorHex: "",
                        borderTopLeftRadius: 4,
                        borderTopRightRadius: 4,
                        borderBottomRightRadius: 4,
                        borderBottomLeftRadius: 4,
                        marginRight: 15,
                        marginLeft: 15,
                        verticalAlign: "center",
                        paddingTop: 70,
                        paddingRight: 65,
                        paddingBottom: 70,
                        paddingLeft: 65,
                        tempBorderTopLeftRadius: 4,
                        tempBorderTopRightRadius: 4,
                        tempBorderBottomRightRadius: 4,
                        tempBorderBottomLeftRadius: 4,
                        bgImageWidth: 1280,
                        bgImageHeight: 1920,
                        bgImageSrc:
                          "d3e637a2ecd213d20b7259a13ae31b1d1bd146fa.jpeg",
                        mobileBgImageWidth: 1280,
                        mobileBgImageHeight: 1920,
                        mobileBgImageSrc:
                          "d3e637a2ecd213d20b7259a13ae31b1d1bd146fa.jpeg",
                        bgPositionY: 91,
                        bgColorPalette: "color1",
                        mobileBgColorPalette: "color1",
                        mobileBgColorHex: "",
                        mobileBgColorOpacity: 0.9,
                        margin: 15,
                        marginTop: 15,
                        marginBottom: 15,
                        tabletMarginType: "grouped",
                        tabletMarginSuffix: "px",
                        tabletMarginTopSuffix: "px",
                        tabletMarginRightSuffix: "px",
                        tabletMarginBottomSuffix: "px",
                        tabletMarginLeftSuffix: "px",
                        tabletMargin: 10,
                        tabletMarginTop: 10,
                        tabletMarginRight: 10,
                        tabletMarginBottom: 10,
                        tabletMarginLeft: 10,
                        mobilePaddingSuffix: "px",
                        mobilePaddingTopSuffix: "px",
                        mobilePaddingRightSuffix: "px",
                        mobilePaddingBottomSuffix: "px",
                        mobilePaddingLeftSuffix: "px",
                        tempMobilePaddingSuffix: "px",
                        tempMobilePaddingTopSuffix: "px",
                        tempMobilePaddingRightSuffix: "px",
                        tempMobilePaddingBottomSuffix: "px",
                        tempMobilePaddingLeftSuffix: "px",
                        mobilePadding: 15,
                        mobilePaddingTop: 15,
                        mobilePaddingRight: 15,
                        mobilePaddingBottom: 15,
                        mobilePaddingLeft: 15,
                        tempMobilePadding: 15,
                        tempMobilePaddingTop: 15,
                        tempMobilePaddingRight: 15,
                        tempMobilePaddingBottom: 15,
                        tempMobilePaddingLeft: 15,
                        mobileMarginRightSuffix: "px",
                        mobileMarginRight: 15,
                        mobileMargin: 15,
                        mobileMarginSuffix: "px",
                        mobileMarginLeftSuffix: "px",
                        mobileMarginLeft: 15
                      }
                    },
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        width: 33.4,
                        items: [
                          {
                            type: "Cloneable",
                            value: {
                              _styles: ["wrapper-clone", "wrapper-clone--icon"],
                              items: [
                                {
                                  type: "Icon",
                                  value: {
                                    _styles: ["icon"],
                                    name: "code-editor",
                                    size: "large",
                                    customSize: 64,
                                    type: "outline",
                                    bgColorHex: "#009900",
                                    tempBgColorOpacity: 0.8,
                                    tempFillType: "outline",
                                    hoverBorderColorHex: "#009900",
                                    borderColorHex: "#cccccc",
                                    tempBorderColorOpacity: 0.7,
                                    hoverBgColorHex: "#216fbd",
                                    borderRadius: 35,
                                    mobileSize: "large",
                                    mobileCustomSize: 64,
                                    tempPadding: 0,
                                    colorHex: "#ffffff",
                                    hoverColorHex: "#ffffff",
                                    hoverColorOpacity: 1,
                                    colorPalette: "color8",
                                    hoverColorPalette: "color8"
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h4 class="brz-text-lg-center brz-tp-heading4"><span class="brz-cp-color8">Development</span></h4>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-text-lg-center brz-tp-paragraph"><span style="background-color: transparent;" class="brz-cp-color7">No more buffering - create, style and enjoy. Our user interface was built on React - the most efficient library.</span></p>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 10,
                                    mobileHeight: 10
                                  }
                                }
                              ],
                              showOnTablet: "off",
                              showOnMobile: "off"
                            }
                          },
                          {
                            type: "Cloneable",
                            value: {
                              _styles: [
                                "wrapper-clone",
                                "wrapper-clone--button"
                              ],
                              items: [
                                {
                                  type: "Button",
                                  value: {
                                    _styles: ["button"],
                                    size: "small",
                                    paddingTB: 11,
                                    paddingTop: 11,
                                    paddingBottom: 11,
                                    tempPaddingTB: 11,
                                    tempPaddingTop: 11,
                                    tempPaddingBottom: 11,
                                    paddingRL: 26,
                                    paddingRight: 26,
                                    paddingLeft: 26,
                                    tempPaddingRL: 26,
                                    tempPaddingRight: 26,
                                    tempPaddingLeft: 26,
                                    tempFillType: "outline",
                                    bgColorPalette: "color1",
                                    tempBgColorPalette: "color1",
                                    bgColorHex: "#191b21",
                                    bgColorOpacity: 0,
                                    fillType: "outline",
                                    colorPalette: "color3",
                                    iconName: "",
                                    iconType: "",
                                    text: "FIND OUT MORE",
                                    hoverBgColorOpacity: 1,
                                    borderColorOpacity: 1,
                                    tempHoverBgColorOpacity: 1,
                                    fontWeight: 600,
                                    mobileFontSize: 12,
                                    fontSize: 11,
                                    hoverBorderColorOpacity: 0.8
                                  }
                                }
                              ]
                            }
                          }
                        ],
                        bgColorOpacity: 0.85,
                        tempBgColorOpacity: 0.85,
                        bgColorHex: "",
                        borderTopLeftRadius: 4,
                        borderTopRightRadius: 4,
                        borderBottomRightRadius: 4,
                        borderBottomLeftRadius: 4,
                        marginRight: 15,
                        marginLeft: 15,
                        verticalAlign: "center",
                        paddingTop: 70,
                        paddingRight: 65,
                        paddingBottom: 70,
                        paddingLeft: 65,
                        tempBorderTopLeftRadius: 4,
                        tempBorderTopRightRadius: 4,
                        tempBorderBottomRightRadius: 4,
                        tempBorderBottomLeftRadius: 4,
                        bgImageWidth: 1536,
                        bgImageHeight: 1920,
                        bgImageSrc:
                          "32a61f3becbfc6ffe65fda9c27afd59d398ccb01.jpeg",
                        mobileBgImageWidth: 1536,
                        mobileBgImageHeight: 1920,
                        mobileBgImageSrc:
                          "32a61f3becbfc6ffe65fda9c27afd59d398ccb01.jpeg",
                        bgPositionX: 100,
                        bgPositionY: 91,
                        bgColorPalette: "color1",
                        mobileBgColorPalette: "color1",
                        mobileBgColorHex: "",
                        mobileBgColorOpacity: 0.85,
                        margin: 15,
                        marginTop: 15,
                        marginBottom: 15,
                        tabletMarginType: "grouped",
                        tabletMarginSuffix: "px",
                        tabletMarginTopSuffix: "px",
                        tabletMarginRightSuffix: "px",
                        tabletMarginBottomSuffix: "px",
                        tabletMarginLeftSuffix: "px",
                        tabletMargin: 10,
                        tabletMarginTop: 10,
                        tabletMarginRight: 10,
                        tabletMarginBottom: 10,
                        tabletMarginLeft: 10,
                        mobilePaddingSuffix: "px",
                        mobilePaddingTopSuffix: "px",
                        mobilePaddingRightSuffix: "px",
                        mobilePaddingBottomSuffix: "px",
                        mobilePaddingLeftSuffix: "px",
                        tempMobilePaddingSuffix: "px",
                        tempMobilePaddingTopSuffix: "px",
                        tempMobilePaddingRightSuffix: "px",
                        tempMobilePaddingBottomSuffix: "px",
                        tempMobilePaddingLeftSuffix: "px",
                        mobilePadding: 15,
                        mobilePaddingTop: 15,
                        mobilePaddingRight: 15,
                        mobilePaddingBottom: 15,
                        mobilePaddingLeft: 15,
                        tempMobilePadding: 15,
                        tempMobilePaddingTop: 15,
                        tempMobilePaddingRight: 15,
                        tempMobilePaddingBottom: 15,
                        tempMobilePaddingLeft: 15,
                        mobileMarginRightSuffix: "px",
                        mobileMarginRight: 15,
                        mobileMargin: 15,
                        mobileMarginSuffix: "px",
                        mobileMarginLeftSuffix: "px",
                        mobileMarginLeft: 15
                      }
                    },
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        width: 33.3,
                        items: [
                          {
                            type: "Cloneable",
                            value: {
                              _styles: ["wrapper-clone", "wrapper-clone--icon"],
                              items: [
                                {
                                  type: "Icon",
                                  value: {
                                    _styles: ["icon"],
                                    name: "chat-46",
                                    size: "large",
                                    customSize: 64,
                                    type: "outline",
                                    bgColorHex: "#009900",
                                    tempBgColorOpacity: 0.8,
                                    tempFillType: "outline",
                                    hoverBorderColorHex: "#009900",
                                    borderColorHex: "#cccccc",
                                    tempBorderColorOpacity: 0.7,
                                    hoverBgColorHex: "#216fbd",
                                    borderRadius: 35,
                                    mobileSize: "large",
                                    mobileCustomSize: 64,
                                    tempPadding: 0,
                                    colorHex: "#ffffff",
                                    hoverColorHex: "#ffffff",
                                    hoverColorOpacity: 1,
                                    colorPalette: "color8",
                                    hoverColorPalette: "color8"
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h4 class="brz-text-lg-center brz-tp-heading4"><span class="brz-cp-color8">Consulting</span></h4>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-text-lg-center brz-tp-paragraph"><span style="background-color: transparent;" class="brz-cp-color7">Let\'s not get technical! Create up-to-date designs without writing a single line of code. Build clean websites.</span></p>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 10,
                                    mobileHeight: 10
                                  }
                                }
                              ],
                              showOnTablet: "off",
                              showOnMobile: "off"
                            }
                          },
                          {
                            type: "Cloneable",
                            value: {
                              _styles: [
                                "wrapper-clone",
                                "wrapper-clone--button"
                              ],
                              items: [
                                {
                                  type: "Button",
                                  value: {
                                    _styles: ["button"],
                                    size: "small",
                                    paddingTB: 11,
                                    paddingTop: 11,
                                    paddingBottom: 11,
                                    tempPaddingTB: 11,
                                    tempPaddingTop: 11,
                                    tempPaddingBottom: 11,
                                    paddingRL: 26,
                                    paddingRight: 26,
                                    paddingLeft: 26,
                                    tempPaddingRL: 26,
                                    tempPaddingRight: 26,
                                    tempPaddingLeft: 26,
                                    tempFillType: "outline",
                                    bgColorPalette: "color1",
                                    tempBgColorPalette: "color1",
                                    bgColorHex: "#191b21",
                                    bgColorOpacity: 0,
                                    hoverBorderColorHex: "#191b21",
                                    fillType: "outline",
                                    colorPalette: "color3",
                                    iconName: "",
                                    iconType: "",
                                    text: "FIND OUT MORE",
                                    borderColorOpacity: 1,
                                    fontWeight: 600,
                                    mobileFontSize: 12,
                                    fontSize: 11,
                                    hoverBorderColorOpacity: 0.8
                                  }
                                }
                              ]
                            }
                          }
                        ],
                        bgColorOpacity: 0.85,
                        tempBgColorOpacity: 0.85,
                        bgColorHex: "",
                        borderTopLeftRadius: 4,
                        borderTopRightRadius: 4,
                        borderBottomRightRadius: 4,
                        borderBottomLeftRadius: 4,
                        marginRight: 15,
                        marginLeft: 15,
                        verticalAlign: "center",
                        paddingTop: 70,
                        paddingRight: 65,
                        paddingBottom: 70,
                        paddingLeft: 65,
                        tempBorderTopLeftRadius: 4,
                        tempBorderTopRightRadius: 4,
                        tempBorderBottomRightRadius: 4,
                        tempBorderBottomLeftRadius: 4,
                        bgImageWidth: 1920,
                        bgImageHeight: 1280,
                        bgImageSrc:
                          "1f0e63565b71c046a2b753528d54c137faaad949.jpeg",
                        mobileBgImageWidth: 1920,
                        mobileBgImageHeight: 1280,
                        mobileBgImageSrc:
                          "1f0e63565b71c046a2b753528d54c137faaad949.jpeg",
                        bgPositionX: 20,
                        bgPositionY: 49,
                        bgColorPalette: "color1",
                        mobileBgColorPalette: "color1",
                        mobileBgColorHex: "",
                        mobileBgColorOpacity: 0.85,
                        margin: 15,
                        marginTop: 15,
                        marginBottom: 15,
                        tabletMarginType: "grouped",
                        tabletMarginSuffix: "px",
                        tabletMarginTopSuffix: "px",
                        tabletMarginRightSuffix: "px",
                        tabletMarginBottomSuffix: "px",
                        tabletMarginLeftSuffix: "px",
                        tabletMargin: 10,
                        tabletMarginTop: 10,
                        tabletMarginRight: 10,
                        tabletMarginBottom: 10,
                        tabletMarginLeft: 10,
                        mobilePaddingSuffix: "px",
                        mobilePaddingTopSuffix: "px",
                        mobilePaddingRightSuffix: "px",
                        mobilePaddingBottomSuffix: "px",
                        mobilePaddingLeftSuffix: "px",
                        tempMobilePaddingSuffix: "px",
                        tempMobilePaddingTopSuffix: "px",
                        tempMobilePaddingRightSuffix: "px",
                        tempMobilePaddingBottomSuffix: "px",
                        tempMobilePaddingLeftSuffix: "px",
                        mobilePadding: 15,
                        mobilePaddingTop: 15,
                        mobilePaddingRight: 15,
                        mobilePaddingBottom: 15,
                        mobilePaddingLeft: 15,
                        tempMobilePadding: 15,
                        tempMobilePaddingTop: 15,
                        tempMobilePaddingRight: 15,
                        tempMobilePaddingBottom: 15,
                        tempMobilePaddingLeft: 15,
                        mobileMarginRightSuffix: "px",
                        mobileMarginRight: 15,
                        mobileMargin: 15,
                        mobileMarginSuffix: "px",
                        mobileMarginLeftSuffix: "px",
                        mobileMarginLeft: 15
                      }
                    }
                  ]
                }
              }
            ],
            mobilePaddingRightSuffix: "px",
            tempMobilePaddingRightSuffix: "px",
            mobilePaddingSuffix: "px",
            tempMobilePaddingSuffix: "px",
            mobilePaddingRight: 15,
            tempMobilePaddingRight: 15,
            mobilePaddingLeftSuffix: "px",
            tempMobilePaddingLeftSuffix: "px",
            mobilePaddingLeft: 15,
            tempMobilePaddingLeft: 15
          }
        }
      ]
    }
  }
};
