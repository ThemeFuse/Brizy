module.exports = {
  id: "block1kit3480",
  thumbnailWidth: 600,
  thumbnailHeight: 444,
  title: "block1kit3480",

  keywords: "button,four columns,pricing",
  cat: [4618, 100],
  type: 1,

  resolve: {
    type: "Section",
    value: {
      _styles: ["section"],
      items: [
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            bgColorHex: "#191b21",
            bgColorOpacity: 1,
            paddingType: "ungrouped",
            padding: 90,
            paddingTop: 60,
            paddingBottom: 90,
            bgAttachment: "animated",
            items: [
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--richText"],
                  marginTop: 0,
                  items: [
                    {
                      type: "RichText",
                      value: {
                        _styles: ["richText"],
                        text:
                          '<h1 class="brz-text-lg-center brz-tp-heading1"><span style="background-color: transparent;" class="brz-cp-color8">Affordable Pricing&nbsp;</span></h1>'
                      }
                    }
                  ],
                  mobileMarginBottom: 0
                }
              },
              {
                type: "Row",
                value: {
                  _styles: ["wrapper", "wrapper--richText"],
                  items: [
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        width: 100,
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--iconText"],
                              items: [
                                {
                                  type: "IconText",
                                  value: {
                                    _styles: ["iconText"],
                                    items: [
                                      {
                                        type: "Icon",
                                        value: {
                                          _styles: ["icon", "iconText--icon"],
                                          name: "phone-call",
                                          fillType: "filled",
                                          padding: 15,
                                          borderRadiusType: "rounded",
                                          borderRadius: 30,
                                          borderColorPalette: "color7",
                                          bgColorOpacity: 1,
                                          bgColorPalette: "color7",
                                          hoverBgColorOpacity: 1,
                                          hoverBgColorPalette: "color8",
                                          hoverBorderColorPalette: "color8",
                                          mobilePadding: 10,
                                          mobileBorderRadius: 30,
                                          size: "custom",
                                          customSize: 24,
                                          mobileSize: "custom",
                                          mobileCustomSize: 24,
                                          tempPadding: 15,
                                          tempBgColorPalette: "color7",
                                          colorPalette: "color8",
                                          hoverBgColorHex: "#239ddb",
                                          borderColorOpacity: 1,
                                          borderWidth: 3,
                                          tempBorderColorPalette: "color7",
                                          hoverBorderColorOpacity: 1
                                        }
                                      },
                                      {
                                        type: "RichText",
                                        value: {
                                          _styles: ["text", "iconText--text"],
                                          text:
                                            '<p class="brz-mt-lg-13 brz-tp-paragraph brz-mt-xs-0"><span class="brz-cp-color7">Give us a call for corporate pricing packs</span></p>'
                                        }
                                      }
                                    ]
                                  }
                                }
                              ],
                              marginTop: 0,
                              mobileMarginBottom: 0,
                              mobileMarginTop: 0
                            }
                          }
                        ],
                        mobileMarginRightSuffix: "px",
                        mobileMarginRight: -100,
                        mobileMargin: 10,
                        mobileMarginSuffix: "px",
                        mobileMarginLeftSuffix: "px",
                        mobileMarginLeft: -100
                      }
                    }
                  ],
                  size: 40,
                  paddingType: "ungrouped",
                  paddingTop: 0,
                  paddingBottom: 0
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--spacer"],
                  items: [
                    {
                      type: "Spacer",
                      value: {
                        _styles: ["spacer"],
                        mobileHeight: 30
                      }
                    }
                  ],
                  showOnMobile: "off"
                }
              },
              {
                type: "Row",
                value: {
                  _styles: ["row", "hide-row-borders", "padding-0"],
                  items: [
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        width: 25,
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h4 class="brz-tp-heading4 brz-text-lg-center"><span style="background-color: transparent;" class="brz-cp-color8">basic</span></h4>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              marginTop: 0,
                              marginBottom: 0,
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h1 class="brz-text-lg-center brz-tp-heading1"><strong class="brz-cp-color3">$19</strong></h1>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              marginTop: 0,
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-text-lg-center brz-tp-button"><span class="brz-cp-color7" style="opacity: 0.5;">PER MONTH</span></p>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 20,
                                    mobileHeight: 20
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Limitless concepts</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Free fonts</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Creative Layouts</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Free Support</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Expert Reviews</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Community access</em></p>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 20,
                                    mobileHeight: 20
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Cloneable",
                            value: {
                              _styles: [
                                "wrapper-clone",
                                "wrapper-clone--button"
                              ],
                              items: [
                                {
                                  type: "Button",
                                  value: {
                                    _styles: ["button"],
                                    text: "BUY NOW",
                                    borderRadiusType: "rounded",
                                    tempBorderRadiusType: "rounded",
                                    borderRadius: 27,
                                    borderColorOpacity: 0.5,
                                    mobileBorderRadius: 24,
                                    iconName: "",
                                    iconType: "",
                                    mobileSize: "medium",
                                    tempFillType: "outline",
                                    bgColorPalette: "color1",
                                    colorPalette: "color7",
                                    tempBgColorPalette: "color1",
                                    borderColorPalette: "color7",
                                    tempBorderColorPalette: "color7",
                                    borderColorHex: "#73777f",
                                    tempBorderColorOpacity: 0.5,
                                    bgColorHex: "#191b21",
                                    bgColorOpacity: 0,
                                    fillType: "outline",
                                    hoverBgColorOpacity: 1,
                                    tempHoverBgColorOpacity: 1
                                  }
                                }
                              ]
                            }
                          }
                        ],
                        padding: 0,
                        verticalAlign: "center",
                        borderColorHex: "#ffffff",
                        borderColorOpacity: 0.2,
                        tempBorderColorOpacity: 0.2,
                        borderWidth: 3,
                        borderTopWidth: 3,
                        borderRightWidth: 3,
                        borderBottomWidth: 3,
                        borderLeftWidth: 3,
                        borderTopLeftRadius: 4,
                        borderTopRightRadius: 4,
                        borderBottomRightRadius: 4,
                        borderBottomLeftRadius: 4,
                        tempBorderWidth: 3,
                        tempBorderTopWidth: 3,
                        tempBorderRightWidth: 3,
                        tempBorderBottomWidth: 3,
                        tempBorderLeftWidth: 3,
                        tempBorderTopLeftRadius: 4,
                        tempBorderTopRightRadius: 4,
                        tempBorderBottomRightRadius: 4,
                        tempBorderBottomLeftRadius: 4,
                        paddingTop: 40,
                        paddingBottom: 60,
                        marginType: "ungrouped",
                        marginRight: 15,
                        marginLeft: 15,
                        bgColorHex: "#ffffff",
                        marginTop: 40,
                        marginBottom: 40,
                        borderColorPalette: "color8",
                        bgColorPalette: "color8",
                        mobileBgColorPalette: "color8",
                        mobileBgColorHex: "#ffffff",
                        bgColorOpacity: 0.1,
                        mobileBgColorOpacity: 0.1,
                        tempBgColorOpacity: 0.1,
                        tabletMarginType: "grouped",
                        tabletMarginSuffix: "px",
                        tabletMarginTopSuffix: "px",
                        tabletMarginRightSuffix: "px",
                        tabletMarginBottomSuffix: "px",
                        tabletMarginLeftSuffix: "px",
                        tabletMargin: 10,
                        tabletMarginTop: 10,
                        tabletMarginRight: 10,
                        tabletMarginBottom: 10,
                        tabletMarginLeft: 10,
                        tabletWidth: 49.7,
                        tabletPaddingTopSuffix: "px",
                        tempTabletPaddingTopSuffix: "px",
                        tabletPaddingSuffix: "px",
                        tempTabletPaddingSuffix: "px",
                        tabletPaddingTop: 15,
                        tempTabletPaddingTop: 15,
                        tabletPaddingBottomSuffix: "px",
                        tempTabletPaddingBottomSuffix: "px",
                        tabletPaddingBottom: 15,
                        tempTabletPaddingBottom: 15,
                        mobilePaddingSuffix: "px",
                        mobilePaddingTopSuffix: "px",
                        mobilePaddingRightSuffix: "px",
                        mobilePaddingBottomSuffix: "px",
                        mobilePaddingLeftSuffix: "px",
                        tempMobilePaddingSuffix: "px",
                        tempMobilePaddingTopSuffix: "px",
                        tempMobilePaddingRightSuffix: "px",
                        tempMobilePaddingBottomSuffix: "px",
                        tempMobilePaddingLeftSuffix: "px",
                        mobilePadding: 15,
                        mobilePaddingTop: 15,
                        mobilePaddingRight: 15,
                        mobilePaddingBottom: 15,
                        mobilePaddingLeft: 15,
                        tempMobilePadding: 15,
                        tempMobilePaddingTop: 15,
                        tempMobilePaddingRight: 15,
                        tempMobilePaddingBottom: 15,
                        tempMobilePaddingLeft: 15
                      }
                    },
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        width: 25,
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h4 class="brz-text-lg-center brz-tp-heading4"><span style="background-color: transparent;" class="brz-cp-color8">standard</span></h4>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              marginTop: 0,
                              marginBottom: 0,
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h1 class="brz-tp-heading1 brz-text-lg-center"><strong class="brz-cp-color3">$29</strong></h1>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              marginTop: 0,
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-text-lg-center brz-tp-button"><span class="brz-cp-color7" style="opacity: 0.5;">PER MONTH</span></p>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 20,
                                    mobileHeight: 20
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Limitless concepts</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Free fonts</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Creative Layouts</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Free Support</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Expert Reviews</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Community access</em></p>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 20,
                                    mobileHeight: 20
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Cloneable",
                            value: {
                              _styles: [
                                "wrapper-clone",
                                "wrapper-clone--button"
                              ],
                              items: [
                                {
                                  type: "Button",
                                  value: {
                                    _styles: ["button"],
                                    text: "BUY NOW",
                                    borderRadiusType: "rounded",
                                    tempBorderRadiusType: "rounded",
                                    borderRadius: 27,
                                    borderColorOpacity: 0.5,
                                    mobileBorderRadius: 24,
                                    iconName: "",
                                    iconType: "",
                                    mobileSize: "medium",
                                    tempFillType: "outline",
                                    bgColorPalette: "color1",
                                    colorPalette: "color7",
                                    tempBgColorPalette: "color1",
                                    borderColorPalette: "color7",
                                    tempBorderColorPalette: "color7",
                                    borderColorHex: "#73777f",
                                    tempBorderColorOpacity: 0.5,
                                    bgColorHex: "#191b21",
                                    bgColorOpacity: 0,
                                    fillType: "outline",
                                    hoverBgColorOpacity: 1,
                                    tempHoverBgColorOpacity: 1
                                  }
                                }
                              ]
                            }
                          }
                        ],
                        padding: 0,
                        verticalAlign: "center",
                        borderColorHex: "#ffffff",
                        borderColorOpacity: 0.2,
                        tempBorderColorOpacity: 0.2,
                        borderWidth: 3,
                        borderTopWidth: 3,
                        borderRightWidth: 3,
                        borderBottomWidth: 3,
                        borderLeftWidth: 3,
                        borderTopLeftRadius: 4,
                        borderTopRightRadius: 4,
                        borderBottomRightRadius: 4,
                        borderBottomLeftRadius: 4,
                        tempBorderWidth: 3,
                        tempBorderTopWidth: 3,
                        tempBorderRightWidth: 3,
                        tempBorderBottomWidth: 3,
                        tempBorderLeftWidth: 3,
                        tempBorderTopLeftRadius: 4,
                        tempBorderTopRightRadius: 4,
                        tempBorderBottomRightRadius: 4,
                        tempBorderBottomLeftRadius: 4,
                        bgColorOpacity: 0.1,
                        paddingTop: 40,
                        paddingBottom: 60,
                        marginType: "ungrouped",
                        marginRight: 15,
                        marginLeft: 15,
                        bgColorHex: "",
                        marginTop: 40,
                        marginBottom: 40,
                        bgColorPalette: "color8",
                        mobileBgColorPalette: "color8",
                        mobileBgColorHex: "",
                        mobileBgColorOpacity: 0.1,
                        borderColorPalette: "color8",
                        tempBgColorOpacity: 0.1,
                        tabletMarginType: "grouped",
                        tabletMarginSuffix: "px",
                        tabletMarginTopSuffix: "px",
                        tabletMarginRightSuffix: "px",
                        tabletMarginBottomSuffix: "px",
                        tabletMarginLeftSuffix: "px",
                        tabletMargin: 10,
                        tabletMarginTop: 10,
                        tabletMarginRight: 10,
                        tabletMarginBottom: 10,
                        tabletMarginLeft: 10,
                        tabletWidth: 50.1,
                        tabletPaddingTopSuffix: "px",
                        tempTabletPaddingTopSuffix: "px",
                        tabletPaddingSuffix: "px",
                        tempTabletPaddingSuffix: "px",
                        tabletPaddingTop: 15,
                        tempTabletPaddingTop: 15,
                        tabletPaddingBottomSuffix: "px",
                        tempTabletPaddingBottomSuffix: "px",
                        tabletPaddingBottom: 15,
                        tempTabletPaddingBottom: 15,
                        mobilePaddingSuffix: "px",
                        mobilePaddingTopSuffix: "px",
                        mobilePaddingRightSuffix: "px",
                        mobilePaddingBottomSuffix: "px",
                        mobilePaddingLeftSuffix: "px",
                        tempMobilePaddingSuffix: "px",
                        tempMobilePaddingTopSuffix: "px",
                        tempMobilePaddingRightSuffix: "px",
                        tempMobilePaddingBottomSuffix: "px",
                        tempMobilePaddingLeftSuffix: "px",
                        mobilePadding: 15,
                        mobilePaddingTop: 15,
                        mobilePaddingRight: 15,
                        mobilePaddingBottom: 15,
                        mobilePaddingLeft: 15,
                        tempMobilePadding: 15,
                        tempMobilePaddingTop: 15,
                        tempMobilePaddingRight: 15,
                        tempMobilePaddingBottom: 15,
                        tempMobilePaddingLeft: 15
                      }
                    },
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        width: 25,
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h4 class="brz-tp-heading4 brz-text-lg-center"><span style="background-color: transparent;" class="brz-cp-color8">premium</span></h4>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              marginTop: 0,
                              marginBottom: 0,
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h1 class="brz-tp-heading1 brz-text-lg-center"><strong class="brz-cp-color8">$39</strong></h1>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              marginTop: 0,
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-text-lg-center brz-tp-button"><span class="brz-cp-color7" style="opacity: 0.5;">PER MONTH</span></p>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 20,
                                    mobileHeight: 20
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Limitless concepts</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Free fonts</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Creative Layouts</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Free Support</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Expert Reviews</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Community access</em></p>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 20,
                                    mobileHeight: 20
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Cloneable",
                            value: {
                              _styles: [
                                "wrapper-clone",
                                "wrapper-clone--button"
                              ],
                              items: [
                                {
                                  type: "Button",
                                  value: {
                                    _styles: ["button"],
                                    text: "BUY NOW",
                                    borderRadiusType: "rounded",
                                    tempBorderRadiusType: "rounded",
                                    borderRadius: 27,
                                    borderColorOpacity: 0.5,
                                    mobileBorderRadius: 24,
                                    iconName: "",
                                    iconType: "",
                                    mobileSize: "medium",
                                    tempFillType: "outline",
                                    borderColorHex: "#73777f",
                                    tempBorderColorOpacity: 0.5,
                                    bgColorHex: "#191b21",
                                    hoverBgColorOpacity: 1,
                                    tempHoverBgColorOpacity: 1,
                                    hoverBgColorPalette: "color2",
                                    hoverBorderColorPalette: "color2",
                                    tempHoverBorderColorPalette: "color2"
                                  }
                                }
                              ]
                            }
                          }
                        ],
                        padding: 0,
                        verticalAlign: "center",
                        borderColorHex: "#239ddb",
                        borderColorOpacity: 1,
                        borderWidth: 4,
                        borderTopWidth: 4,
                        borderRightWidth: 4,
                        borderBottomWidth: 4,
                        borderLeftWidth: 4,
                        borderTopLeftRadius: 4,
                        borderTopRightRadius: 4,
                        borderBottomRightRadius: 4,
                        borderBottomLeftRadius: 4,
                        tempBorderTopLeftRadius: 4,
                        tempBorderTopRightRadius: 4,
                        tempBorderBottomRightRadius: 4,
                        tempBorderBottomLeftRadius: 4,
                        bgColorOpacity: 0.1,
                        paddingTop: 40,
                        paddingBottom: 60,
                        marginRight: 15,
                        marginLeft: 15,
                        bgColorHex: "",
                        bgColorPalette: "color8",
                        mobileBgColorPalette: "color8",
                        mobileBgColorHex: "",
                        mobileBgColorOpacity: 0.1,
                        borderColorPalette: "color3",
                        tempBgColorOpacity: 0.1,
                        margin: 15,
                        marginTop: 15,
                        marginBottom: 15,
                        tabletWidth: 49.7,
                        tabletPaddingTopSuffix: "px",
                        tempTabletPaddingTopSuffix: "px",
                        tabletPaddingSuffix: "px",
                        tempTabletPaddingSuffix: "px",
                        tabletPaddingTop: 15,
                        tempTabletPaddingTop: 15,
                        tabletPaddingBottomSuffix: "px",
                        tempTabletPaddingBottomSuffix: "px",
                        tabletPaddingBottom: 15,
                        tempTabletPaddingBottom: 15,
                        mobilePaddingSuffix: "px",
                        mobilePaddingTopSuffix: "px",
                        mobilePaddingRightSuffix: "px",
                        mobilePaddingBottomSuffix: "px",
                        mobilePaddingLeftSuffix: "px",
                        tempMobilePaddingSuffix: "px",
                        tempMobilePaddingTopSuffix: "px",
                        tempMobilePaddingRightSuffix: "px",
                        tempMobilePaddingBottomSuffix: "px",
                        tempMobilePaddingLeftSuffix: "px",
                        mobilePadding: 15,
                        mobilePaddingTop: 15,
                        mobilePaddingRight: 15,
                        mobilePaddingBottom: 15,
                        mobilePaddingLeft: 15,
                        tempMobilePadding: 15,
                        tempMobilePaddingTop: 15,
                        tempMobilePaddingRight: 15,
                        tempMobilePaddingBottom: 15,
                        tempMobilePaddingLeft: 15
                      }
                    },
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        width: 25,
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h4 class="brz-tp-heading4 brz-text-lg-center"><span style="background-color: transparent;" class="brz-cp-color8">ultra</span></h4>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              marginTop: 0,
                              marginBottom: 0,
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h1 class="brz-text-lg-center brz-tp-heading1"><strong class="brz-cp-color3">$59</strong></h1>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              marginTop: 0,
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-text-lg-center brz-tp-button"><span class="brz-cp-color7" style="opacity: 0.5;">PER MONTH</span></p>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 20,
                                    mobileHeight: 20
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Limitless concepts</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Free fonts</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Creative Layouts</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Free Support</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Expert Reviews</em></p><p class="brz-text-lg-center brz-tp-paragraph"><em class="brz-cp-color7">Community access</em></p>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 20,
                                    mobileHeight: 20
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Cloneable",
                            value: {
                              _styles: [
                                "wrapper-clone",
                                "wrapper-clone--button"
                              ],
                              items: [
                                {
                                  type: "Button",
                                  value: {
                                    _styles: ["button"],
                                    text: "BUY NOW",
                                    borderRadiusType: "rounded",
                                    tempBorderRadiusType: "rounded",
                                    borderRadius: 27,
                                    borderColorOpacity: 0.5,
                                    mobileBorderRadius: 24,
                                    iconName: "",
                                    iconType: "",
                                    mobileSize: "medium",
                                    tempFillType: "outline",
                                    bgColorPalette: "color1",
                                    colorPalette: "color7",
                                    tempBgColorPalette: "color1",
                                    borderColorPalette: "color7",
                                    tempBorderColorPalette: "color7",
                                    borderColorHex: "#73777f",
                                    tempBorderColorOpacity: 0.5,
                                    bgColorHex: "#191b21",
                                    bgColorOpacity: 0,
                                    fillType: "outline",
                                    hoverBgColorOpacity: 1,
                                    tempHoverBgColorOpacity: 1
                                  }
                                }
                              ]
                            }
                          }
                        ],
                        padding: 0,
                        verticalAlign: "center",
                        borderColorHex: "#ffffff",
                        borderColorOpacity: 0.2,
                        tempBorderColorOpacity: 0.2,
                        borderWidth: 3,
                        borderTopWidth: 3,
                        borderRightWidth: 3,
                        borderBottomWidth: 3,
                        borderLeftWidth: 3,
                        borderTopLeftRadius: 4,
                        borderTopRightRadius: 4,
                        borderBottomRightRadius: 4,
                        borderBottomLeftRadius: 4,
                        tempBorderWidth: 3,
                        tempBorderTopWidth: 3,
                        tempBorderRightWidth: 3,
                        tempBorderBottomWidth: 3,
                        tempBorderLeftWidth: 3,
                        tempBorderTopLeftRadius: 4,
                        tempBorderTopRightRadius: 4,
                        tempBorderBottomRightRadius: 4,
                        tempBorderBottomLeftRadius: 4,
                        bgColorOpacity: 0.1,
                        paddingTop: 40,
                        paddingBottom: 60,
                        marginType: "ungrouped",
                        marginRight: 15,
                        marginLeft: 15,
                        bgColorHex: "#ffffff",
                        marginTop: 40,
                        marginBottom: 40,
                        bgColorPalette: "color8",
                        mobileBgColorPalette: "color8",
                        mobileBgColorHex: "#ffffff",
                        mobileBgColorOpacity: 0.1,
                        borderColorPalette: "color8",
                        tempBgColorOpacity: 0.1,
                        tabletMarginType: "grouped",
                        tabletMarginSuffix: "px",
                        tabletMarginTopSuffix: "px",
                        tabletMarginRightSuffix: "px",
                        tabletMarginBottomSuffix: "px",
                        tabletMarginLeftSuffix: "px",
                        tabletMargin: 10,
                        tabletMarginTop: 10,
                        tabletMarginRight: 10,
                        tabletMarginBottom: 10,
                        tabletMarginLeft: 10,
                        tabletWidth: 50.1,
                        tabletPaddingTopSuffix: "px",
                        tempTabletPaddingTopSuffix: "px",
                        tabletPaddingSuffix: "px",
                        tempTabletPaddingSuffix: "px",
                        tabletPaddingTop: 15,
                        tempTabletPaddingTop: 15,
                        tabletPaddingBottomSuffix: "px",
                        tempTabletPaddingBottomSuffix: "px",
                        tabletPaddingBottom: 15,
                        tempTabletPaddingBottom: 15,
                        mobilePaddingSuffix: "px",
                        mobilePaddingTopSuffix: "px",
                        mobilePaddingRightSuffix: "px",
                        mobilePaddingBottomSuffix: "px",
                        mobilePaddingLeftSuffix: "px",
                        tempMobilePaddingSuffix: "px",
                        tempMobilePaddingTopSuffix: "px",
                        tempMobilePaddingRightSuffix: "px",
                        tempMobilePaddingBottomSuffix: "px",
                        tempMobilePaddingLeftSuffix: "px",
                        mobilePadding: 15,
                        mobilePaddingTop: 15,
                        mobilePaddingRight: 15,
                        mobilePaddingBottom: 15,
                        mobilePaddingLeft: 15,
                        tempMobilePadding: 15,
                        tempMobilePaddingTop: 15,
                        tempMobilePaddingRight: 15,
                        tempMobilePaddingBottom: 15,
                        tempMobilePaddingLeft: 15
                      }
                    }
                  ]
                }
              }
            ],
            mobileBgColorHex: "#191b21",
            mobileBgColorOpacity: 1,
            mobilePaddingRightSuffix: "px",
            tempMobilePaddingRightSuffix: "px",
            mobilePaddingSuffix: "px",
            tempMobilePaddingSuffix: "px",
            mobilePaddingRight: 15,
            tempMobilePaddingRight: 15,
            mobilePaddingLeftSuffix: "px",
            tempMobilePaddingLeftSuffix: "px",
            mobilePaddingLeft: 15,
            tempMobilePaddingLeft: 15
          }
        }
      ]
    }
  }
};
