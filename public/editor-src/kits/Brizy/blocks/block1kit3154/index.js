module.exports = {
  id: "block1kit3154",
  thumbnailWidth: 600,
  thumbnailHeight: 345,
  title: "block1kit3154",

  keywords: "address,email,info,map,phone",
  cat: [4509, 100],
  type: 1,

  resolve: {
    type: "Section",
    value: {
      _styles: ["section"],
      items: [
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            bgColorHex: "#191b21",
            bgColorOpacity: 1,
            items: [
              {
                type: "Row",
                value: {
                  _styles: ["row", "hide-row-borders", "padding-0"],
                  items: [
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<h3 class="brz-text-lg-left brz-tp-heading3"><span style="opacity: 1;" class="brz-cp-color8">Contact Info</span></h3>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-text-lg-left brz-tp-paragraph brz-mb-lg-10"><span style="background-color: transparent;" class="brz-cp-color7">Build clean, minimalistic, beautifully crafted websites perfectly fit for any device.&nbsp;Just click, modify watch results.</span></p>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--iconText"],
                              items: [
                                {
                                  type: "IconText",
                                  value: {
                                    _styles: ["iconText"],
                                    items: [
                                      {
                                        type: "Icon",
                                        value: {
                                          _styles: ["icon", "iconText--icon"],
                                          size: "small",
                                          customSize: 32,
                                          borderRadius: 3,
                                          mobileSize: "small",
                                          mobileCustomSize: 32,
                                          name: "pin-add",
                                          colorHex: "#ffffff",
                                          colorPalette: "",
                                          hoverColorOpacity: 1,
                                          hoverColorPalette: "color8"
                                        }
                                      },
                                      {
                                        type: "RichText",
                                        value: {
                                          _styles: ["text", "iconText--text"],
                                          text:
                                            '<h6 class="brz-tp-heading6"><span class="brz-cp-color3">132 Terry Road, Hartford, 06105, Connecticut, USA</span></h6>'
                                        }
                                      }
                                    ],
                                    iconSpacing: 15
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--iconText"],
                              items: [
                                {
                                  type: "IconText",
                                  value: {
                                    _styles: ["iconText"],
                                    items: [
                                      {
                                        type: "Icon",
                                        value: {
                                          _styles: ["icon", "iconText--icon"],
                                          size: "small",
                                          customSize: 32,
                                          borderRadius: 3,
                                          mobileSize: "small",
                                          mobileCustomSize: 32,
                                          name: "mobile-button",
                                          colorHex: "#ffffff",
                                          colorPalette: "",
                                          hoverColorHex: "#ffffff",
                                          hoverColorPalette: "color8",
                                          hoverColorOpacity: 1
                                        }
                                      },
                                      {
                                        type: "RichText",
                                        value: {
                                          _styles: ["text", "iconText--text"],
                                          text:
                                            '<h6 class="brz-tp-heading6 brz-mt-lg-5"><span class="brz-cp-color3">455-23.45.78</span></h6>'
                                        }
                                      }
                                    ],
                                    iconSpacing: 15
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--iconText"],
                              items: [
                                {
                                  type: "IconText",
                                  value: {
                                    _styles: ["iconText"],
                                    items: [
                                      {
                                        type: "Icon",
                                        value: {
                                          _styles: ["icon", "iconText--icon"],
                                          size: "small",
                                          customSize: 32,
                                          borderRadius: 3,
                                          mobileSize: "small",
                                          mobileCustomSize: 32,
                                          name: "at-sign",
                                          colorHex: "#ffffff",
                                          colorPalette: "",
                                          hoverColorHex: "#ffffff",
                                          hoverColorPalette: "color8",
                                          hoverColorOpacity: 1
                                        }
                                      },
                                      {
                                        type: "RichText",
                                        value: {
                                          _styles: ["text", "iconText--text"],
                                          text:
                                            '<h6 class="brz-mt-lg-5 brz-tp-heading6"><span class="brz-cp-color3">email@domain.com</span></h6>'
                                        }
                                      }
                                    ],
                                    iconSpacing: 15
                                  }
                                }
                              ]
                            }
                          }
                        ],
                        paddingTop: 0,
                        paddingBottom: 0,
                        verticalAlign: "center",
                        paddingLeft: 90,
                        paddingRight: 90,
                        marginType: "ungrouped",
                        bgColorPalette: "color8",
                        bgColorHex: "#ffffff",
                        bgColorOpacity: 0.1,
                        mobileBgColorPalette: "color8",
                        mobileBgColorHex: "#ffffff",
                        mobileBgColorOpacity: 0.1,
                        tempBgColorOpacity: 0.1,
                        borderRadiusType: "ungrouped",
                        borderTopLeftRadius: 10,
                        tempBorderTopLeftRadius: 10,
                        borderBottomLeftRadius: 10,
                        tempBorderBottomLeftRadius: 10,
                        tabletPaddingLeftSuffix: "px",
                        tempTabletPaddingLeftSuffix: "px",
                        tabletPaddingSuffix: "px",
                        tempTabletPaddingSuffix: "px",
                        tabletPaddingLeft: 35,
                        tempTabletPaddingLeft: 35,
                        mobilePaddingSuffix: "px",
                        mobilePaddingTopSuffix: "px",
                        mobilePaddingRightSuffix: "px",
                        mobilePaddingBottomSuffix: "px",
                        mobilePaddingLeftSuffix: "px",
                        tempMobilePaddingSuffix: "px",
                        tempMobilePaddingTopSuffix: "px",
                        tempMobilePaddingRightSuffix: "px",
                        tempMobilePaddingBottomSuffix: "px",
                        tempMobilePaddingLeftSuffix: "px",
                        mobilePadding: 25,
                        mobilePaddingTop: 25,
                        mobilePaddingRight: 25,
                        mobilePaddingBottom: 25,
                        mobilePaddingLeft: 25,
                        tempMobilePadding: 25,
                        tempMobilePaddingTop: 25,
                        tempMobilePaddingRight: 25,
                        tempMobilePaddingBottom: 25,
                        tempMobilePaddingLeft: 25
                      }
                    },
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 20,
                                    mobileHeight: 20
                                  }
                                }
                              ],
                              showOnMobile: "off"
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--map"],
                              items: [
                                {
                                  type: "Map",
                                  value: {
                                    _styles: ["map"],
                                    height: 600,
                                    tabletHeight: 400
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 20,
                                    mobileHeight: 20
                                  }
                                }
                              ],
                              showOnMobile: "off"
                            }
                          }
                        ],
                        paddingTop: 0,
                        paddingBottom: 0,
                        paddingRight: 100,
                        bgColorPalette: "color8",
                        bgColorHex: "#ffffff",
                        bgColorOpacity: 0.1,
                        mobileBgColorPalette: "color8",
                        mobileBgColorHex: "#ffffff",
                        mobileBgColorOpacity: 0.1,
                        tempBgColorOpacity: 0.1,
                        borderRadiusType: "ungrouped",
                        borderTopRightRadius: 10,
                        tempBorderTopRightRadius: 10,
                        borderBottomRightRadius: 10,
                        tempBorderBottomRightRadius: 10
                      }
                    }
                  ]
                }
              }
            ],
            mobileBgColorHex: "#191b21",
            mobileBgColorOpacity: 1,
            mobilePaddingRightSuffix: "px",
            tempMobilePaddingRightSuffix: "px",
            mobilePaddingSuffix: "px",
            tempMobilePaddingSuffix: "px",
            mobilePaddingRight: 15,
            tempMobilePaddingRight: 15,
            mobilePaddingLeftSuffix: "px",
            tempMobilePaddingLeftSuffix: "px",
            mobilePaddingLeft: 15,
            tempMobilePaddingLeft: 15
          }
        }
      ]
    }
  }
};
