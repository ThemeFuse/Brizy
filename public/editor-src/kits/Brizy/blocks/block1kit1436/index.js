module.exports = {
  id: "block1kit1436",
  thumbnailWidth: 600,
  thumbnailHeight: 354,
  title: "block1kit1436",

  keywords: "button,call to action,cover,header,hero,image background",
  cat: [4395, 100],
  type: 1,

  resolve: {
    type: "Section",
    value: {
      _styles: ["section"],
      items: [
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            bgColorHex: "#191b21",
            bgColorOpacity: 0.85,
            paddingTop: 150,
            paddingBottom: 0,
            tempBgColorOpacity: 0.85,
            padding: 150,
            paddingType: "ungrouped",
            items: [
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--richText"],
                  items: [
                    {
                      type: "RichText",
                      value: {
                        _styles: ["richText"],
                        text:
                          '<p class="brz-text-lg-center brz-tp-abovetitle"><span class="brz-cp-color3">CREATE STUNNING WEBSITES</span></p>'
                      }
                    }
                  ]
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--richText"],
                  items: [
                    {
                      type: "RichText",
                      value: {
                        _styles: ["richText"],
                        text:
                          '<h1 class="brz-text-lg-center brz-tp-heading1"><span class="brz-cp-color8" style="opacity: 1;"><span>Easy to&nbsp;</span></span><strong class="brz-cp-color8" style="opacity: 1;"><span>Create</span></strong><span class="brz-cp-color8" style="opacity: 1;"><span> amazing websites</span></span></h1>'
                      }
                    }
                  ],
                  paddingType: "ungrouped",
                  paddingRight: 50,
                  marginTop: 0,
                  paddingLeft: 50
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--spacer"],
                  items: [
                    {
                      type: "Spacer",
                      value: {
                        _styles: ["spacer"],
                        height: 40,
                        mobileHeight: 30
                      }
                    }
                  ],
                  showOnTablet: "off",
                  showOnMobile: "off"
                }
              },
              {
                type: "Cloneable",
                value: {
                  _styles: ["wrapper-clone", "wrapper-clone--button"],
                  items: [
                    {
                      type: "Button",
                      value: {
                        _styles: ["button"],
                        borderRadiusType: "rounded",
                        tempBorderRadiusType: "rounded",
                        borderRadius: 27,
                        mobileBorderRadius: 24,
                        text: "TRY FOR FREE",
                        hoverBgColorPalette: "color2",
                        tempHoverBorderColorPalette: "",
                        hoverBgColorHex: "#142850",
                        hoverBgColorOpacity: 0.99,
                        borderWidth: 0
                      }
                    },
                    {
                      type: "Button",
                      value: {
                        _styles: ["button"],
                        borderRadiusType: "rounded",
                        tempBorderRadiusType: "rounded",
                        borderRadius: 27,
                        mobileBorderRadius: 24,
                        text: "GO PRO | $39",
                        tempHoverBorderColorPalette: "",
                        iconName: "",
                        iconType: "",
                        bgColorPalette: "color8",
                        hoverBorderColorOpacity: 1,
                        tempBgColorPalette: "color8",
                        borderColorPalette: "color8",
                        tempBorderColorPalette: "color8",
                        colorPalette: "color2",
                        bgColorHex: "#ffffff",
                        borderColorHex: "#ffffff",
                        hoverBgColorOpacity: 0.03,
                        tempHoverBgColorOpacity: 1,
                        borderColorOpacity: 1,
                        hoverBorderColorPalette: "color8"
                      }
                    }
                  ],
                  itemPadding: 20,
                  itemPaddingRight: 20,
                  itemPaddingLeft: 20
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--spacer"],
                  items: [
                    {
                      type: "Spacer",
                      value: {
                        _styles: ["spacer"],
                        height: 70,
                        mobileHeight: 30,
                        tabletHeight: 20
                      }
                    }
                  ],
                  showOnMobile: "off"
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--image"],
                  items: [
                    {
                      type: "Image",
                      value: {
                        _styles: ["image"],
                        imageSrc:
                          "8085a9b59494ea66f252cfde1405f6039fb9a424.jpeg",
                        imageWidth: 970,
                        imageHeight: 330,
                        resize: 90,
                        mobileResize: 90
                      }
                    }
                  ],
                  marginBottom: 0,
                  marginTop: 0,
                  mobileMarginBottom: 0
                }
              }
            ],
            bgImageWidth: 1920,
            bgImageHeight: 881,
            bgImageSrc: "014861fb4ee85e752356c52e1937261cedb41e61.jpeg",
            mobileBgImageWidth: 1920,
            mobileBgImageHeight: 881,
            mobileBgImageSrc: "014861fb4ee85e752356c52e1937261cedb41e61.jpeg",
            mobileBgColorHex: "#191b21",
            mobileBgColorOpacity: 0.85,
            mobilePadding: 0,
            mobilePaddingBottom: 0,
            mobilePaddingType: "ungrouped",
            mobilePaddingRightSuffix: "px",
            tempMobilePaddingRightSuffix: "px",
            mobilePaddingSuffix: "px",
            tempMobilePaddingSuffix: "px",
            mobilePaddingRight: 15,
            tempMobilePaddingRight: 15,
            mobilePaddingLeftSuffix: "px",
            tempMobilePaddingLeftSuffix: "px",
            mobilePaddingLeft: 15,
            tempMobilePaddingLeft: 15
          }
        }
      ]
    }
  }
};
