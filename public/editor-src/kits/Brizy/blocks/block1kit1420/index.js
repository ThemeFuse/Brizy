module.exports = {
  id: "block1kit1420",
  thumbnailWidth: 600,
  thumbnailHeight: 359,
  title: "block1kit1420",

  keywords: "button,call to action,cover,header,image background,promo",
  cat: [4395, 100],
  type: 1,

  resolve: {
    type: "Section",
    value: {
      _styles: ["section"],
      items: [
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            bgColorHex: "#191b21",
            bgColorOpacity: 0.8,
            paddingTop: 250,
            paddingBottom: 250,
            padding: 250,
            tempBgColorOpacity: 0.8,
            items: [
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--richText"],
                  items: [
                    {
                      type: "RichText",
                      value: {
                        _styles: ["richText"],
                        text:
                          '<h1 class="brz-text-lg-center brz-tp-heading1"><span style="opacity: 1;" class="brz-cp-color8">Creating&nbsp;</span><strong style="opacity: 1;" class="brz-cp-color8">Stunning Websites</strong><span style="opacity: 1;" class="brz-cp-color8"> Fast, Visually, Only Drag &amp; Drops</span></h1>'
                      }
                    }
                  ],
                  paddingType: "ungrouped",
                  marginTop: 0
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--richText"],
                  items: [
                    {
                      type: "RichText",
                      value: {
                        _styles: ["richText"],
                        text:
                          '<p class="brz-tp-subtitle brz-text-lg-center"><em class="brz-cp-color3">No coding skills required, you only need to know how to use the mouse and keyboard.</em></p>'
                      }
                    }
                  ]
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--spacer"],
                  items: [
                    {
                      type: "Spacer",
                      value: {
                        _styles: ["spacer"],
                        mobileHeight: 10
                      }
                    }
                  ],
                  showOnTablet: "off"
                }
              },
              {
                type: "Cloneable",
                value: {
                  _styles: ["wrapper-clone", "wrapper-clone--button"],
                  items: [
                    {
                      type: "Button",
                      value: {
                        _styles: ["button"],
                        text: "START NOW | It's Free",
                        iconName: "",
                        iconType: "",
                        borderRadiusType: "rounded",
                        tempBorderRadiusType: "rounded",
                        borderRadius: 27,
                        tempBorderRadius: 4,
                        mobileBorderRadius: 24,
                        hoverBorderColorPalette: "",
                        tempHoverBorderColorPalette: "",
                        hoverBorderColorHex: "#142850",
                        bgColorHex: "#ffffff",
                        bgColorPalette: "color8",
                        tempBgColorPalette: "color8",
                        borderColorHex: "#ffffff",
                        borderColorPalette: "color8",
                        tempBorderColorPalette: "color8",
                        colorPalette: "color3"
                      }
                    }
                  ]
                }
              }
            ],
            mobilePadding: 60,
            mobilePaddingTop: 60,
            mobilePaddingBottom: 60,
            mobileBgColorHex: "#191b21",
            mobileBgColorOpacity: 0.8,
            bgImageWidth: 1920,
            bgImageHeight: 881,
            bgImageSrc: "3f0ed29e6e534e6b8be61f7513463cbd7d8492e9.jpeg",
            mobileBgImageWidth: 1920,
            mobileBgImageHeight: 881,
            mobileBgImageSrc: "3f0ed29e6e534e6b8be61f7513463cbd7d8492e9.jpeg",
            containerSize: 90
          }
        }
      ]
    }
  }
};
