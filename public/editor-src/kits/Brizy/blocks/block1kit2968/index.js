module.exports = {
  id: "block1kit2968",
  thumbnailWidth: 600,
  thumbnailHeight: 363,
  title: "block1kit2968",

  keywords: "Gallery",
  cat: [4561, 100],
  type: 0,

  resolve: {
    type: "Section",
    value: {
      _styles: ["section"],
      items: [
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            items: [
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--richText"],
                  items: [
                    {
                      type: "RichText",
                      value: {
                        _styles: ["richText"],
                        text:
                          '<h1 class="brz-tp-heading1 brz-text-lg-center brz-mb-lg-0"><span class="brz-cp-color2">Masonery Gallery</span></h1>'
                      }
                    }
                  ],
                  marginBottom: 5,
                  marginBottomSuffix: "px",
                  margin: 0
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--richText"],
                  items: [
                    {
                      type: "RichText",
                      value: {
                        _styles: ["richText"],
                        text:
                          '<h5 class="brz-mb-lg-40 brz-tp-subtitle brz-text-lg-center"><em class="brz-cp-color7">Click the images for a full view with our Lightbox feature</em></h5>'
                      }
                    }
                  ],
                  marginTop: 0,
                  marginTopSuffix: "px",
                  margin: 0,
                  marginBottom: 0,
                  marginBottomSuffix: "px"
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--imageGallery"],
                  items: [
                    {
                      type: "ImageGallery",
                      value: {
                        _styles: ["imageGallery"],
                        items: [
                          {
                            type: "Image",
                            value: {
                              _styles: ["image"],
                              imageWidth: 600,
                              imageHeight: 900,
                              imageSrc: "6f6e4a6bd03e524ade808017b7096c3b.jpg",
                              height: 138,
                              positionX: 50,
                              positionY: 50,
                              imagePopulation: "",
                              linkType: "lightBox",
                              linkLightBox: "on"
                            }
                          },
                          {
                            type: "Image",
                            value: {
                              _styles: ["image"],
                              imageWidth: 1920,
                              imageHeight: 1280,
                              imageSrc: "55332f8546b37958743c54de981f1ded.jpg",
                              height: 129,
                              positionX: 50,
                              positionY: 50,
                              imagePopulation: "",
                              linkType: "lightBox",
                              linkLightBox: "on"
                            }
                          },
                          {
                            type: "Image",
                            value: {
                              _styles: ["image"],
                              imageWidth: 600,
                              imageHeight: 800,
                              imageSrc: "4dcf9d12fc42907b6470f77b03d921f8.jpg",
                              height: 95,
                              positionX: 50,
                              positionY: 50,
                              imagePopulation: "",
                              linkType: "lightBox",
                              linkLightBox: "on"
                            }
                          },
                          {
                            type: "Image",
                            value: {
                              _styles: ["image"],
                              imageWidth: 1920,
                              imageHeight: 1200,
                              imageSrc: "08e5cde7dc728698e21c14f04beeb9cd.jpg",
                              height: 83,
                              positionX: 50,
                              positionY: 50,
                              imagePopulation: "",
                              linkType: "lightBox",
                              linkLightBox: "on"
                            }
                          },
                          {
                            type: "Image",
                            value: {
                              _styles: ["image"],
                              imageWidth: 600,
                              imageHeight: 900,
                              imageSrc: "cf02306f754fed4d7b6d700a2c16b92a.jpg",
                              height: 101,
                              positionX: 50,
                              positionY: 50,
                              imagePopulation: "",
                              linkType: "lightBox",
                              linkLightBox: "on"
                            }
                          },
                          {
                            type: "Image",
                            value: {
                              _styles: ["image"],
                              imageWidth: 600,
                              imageHeight: 900,
                              imageSrc: "52506642c8976181c44e586a9e614134.jpg",
                              height: 78,
                              positionX: 50,
                              positionY: 50,
                              imagePopulation: "",
                              linkType: "lightBox",
                              linkLightBox: "on"
                            }
                          },
                          {
                            type: "Image",
                            value: {
                              _styles: ["image"],
                              imageWidth: 1650,
                              imageHeight: 837,
                              imageSrc: "0878faae1d00d32f2a8b0db99bbd4a27.jpg",
                              height: 151,
                              positionX: 37,
                              positionY: 46,
                              imagePopulation: "",
                              linkType: "lightBox",
                              linkLightBox: "on"
                            }
                          }
                        ],
                        gridColumn: 4,
                        spacing: 10,
                        lightBox: "on",
                        tabletGridColumn: 3,
                        mobileGridColumn: 2
                      }
                    }
                  ],
                  padding: 0,
                  paddingSuffix: "px",
                  paddingTop: 0,
                  paddingRight: 0,
                  paddingBottom: 0,
                  paddingLeft: 0,
                  marginTop: 0,
                  marginTopSuffix: "px",
                  margin: 0,
                  marginBottom: 0,
                  marginBottomSuffix: "px",
                  tabletMarginRightSuffix: "px",
                  tabletMarginRight: 100,
                  tabletMargin: 0,
                  tabletMarginSuffix: "px",
                  tabletMarginLeftSuffix: "px",
                  tabletMarginLeft: 100,
                  mobileMarginRightSuffix: "px",
                  mobileMarginRight: 30,
                  mobileMargin: 0,
                  mobileMarginSuffix: "px",
                  mobileMarginLeftSuffix: "px",
                  mobileMarginLeft: 30
                }
              }
            ],
            paddingType: "ungrouped",
            paddingTop: 100,
            paddingBottom: 100,
            padding: 75,
            bgColorPalette: "color4",
            bgColorHex: "#66738d",
            bgColorOpacity: 0.19,
            tempBgColorOpacity: 0.19,
            mobilePaddingRightSuffix: "px",
            tempMobilePaddingRightSuffix: "px",
            mobilePaddingSuffix: "px",
            tempMobilePaddingSuffix: "px",
            mobilePaddingRight: 15,
            tempMobilePaddingRight: 15,
            mobilePaddingLeftSuffix: "px",
            tempMobilePaddingLeftSuffix: "px",
            mobilePaddingLeft: 15,
            tempMobilePaddingLeft: 15
          }
        }
      ]
    }
  }
};
