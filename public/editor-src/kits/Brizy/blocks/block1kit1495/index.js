module.exports = {
  id: "block1kit1495",
  thumbnailWidth: 600,
  thumbnailHeight: 303,
  title: "block1kit1495",

  keywords: "button,call to action,cover,header,hero,image background,text",
  cat: [4395, 100],
  type: 0,

  resolve: {
    type: "Section",
    value: {
      _styles: ["section"],
      items: [
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            bgColorHex: "#eef0f2",
            bgColorOpacity: 0.81,
            paddingTop: 150,
            paddingBottom: 150,
            tempBgColorOpacity: 0.81,
            paddingType: "ungrouped",
            padding: 150,
            items: [
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--richText"],
                  items: [
                    {
                      type: "RichText",
                      value: {
                        _styles: ["richText"],
                        text:
                          '<h1 class="brz-text-lg-center brz-tp-heading1 brz-fw-xs-300"><span class="brz-cp-color2">Be The Webdesigner! This Gives You The Power</span></h1>'
                      }
                    }
                  ],
                  paddingType: "ungrouped"
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--spacer"],
                  items: [
                    {
                      type: "Spacer",
                      value: {
                        _styles: ["spacer"],
                        height: 30,
                        mobileHeight: 30
                      }
                    }
                  ],
                  showOnTablet: "off",
                  showOnMobile: "off"
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--richText"],
                  items: [
                    {
                      type: "RichText",
                      value: {
                        _styles: ["richText"],
                        text:
                          '<p class="brz-text-lg-center brz-tp-paragraph"><span class="brz-cp-color7" style="background-color: transparent;"><span>Every editing option is extremely well-thought to make it easy for&nbsp;you to find and work with. This will shorten the time neede1d to&nbsp;build your dream website. It\u2019s easy!</span></span></p>'
                      }
                    }
                  ],
                  marginRight: 80,
                  marginLeft: 80
                }
              },
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--spacer"],
                  items: [
                    {
                      type: "Spacer",
                      value: {
                        _styles: ["spacer"],
                        height: 30,
                        mobileHeight: 30
                      }
                    }
                  ],
                  showOnTablet: "off",
                  showOnMobile: "off"
                }
              },
              {
                type: "Cloneable",
                value: {
                  _styles: ["wrapper-clone", "wrapper-clone--button"],
                  items: [
                    {
                      type: "Button",
                      value: {
                        _styles: ["button"],
                        text: "GET STARTED WITH PRO",
                        iconName: "",
                        iconType: "",
                        bgColorPalette: "color2",
                        tempBgColorPalette: "color2",
                        borderColorPalette: "color2",
                        tempBorderColorPalette: "color2",
                        borderRadiusType: "rounded",
                        tempBorderRadiusType: "rounded",
                        borderRadius: 30,
                        mobileBorderRadius: 24,
                        tempBorderRadius: 30,
                        tempHoverBorderColorPalette: "",
                        hoverBgColorOpacity: 1,
                        tempHoverBgColorOpacity: 1,
                        bgColorHex: "#142850",
                        size: "custom",
                        fontSize: 13,
                        paddingTB: 16,
                        paddingTop: 16,
                        paddingBottom: 16,
                        tempPaddingTB: 16,
                        tempPaddingTop: 16,
                        tempPaddingBottom: 16,
                        paddingRL: 58,
                        paddingRight: 58,
                        paddingLeft: 58,
                        tempPaddingRL: 58,
                        tempPaddingRight: 58,
                        tempPaddingLeft: 58,
                        borderColorHex: "#142850",
                        borderColorOpacity: 1,
                        hoverBorderColorOpacity: 0.8
                      }
                    },
                    {
                      type: "Button",
                      value: {
                        _styles: ["button"],
                        text: "FIND OUT MORE",
                        iconName: "",
                        iconType: "",
                        borderRadiusType: "rounded",
                        tempBorderRadiusType: "rounded",
                        borderRadius: 30,
                        mobileBorderRadius: 24,
                        tempBorderRadius: 27,
                        size: "custom",
                        fontSize: 13,
                        paddingRL: 98,
                        paddingRight: 98,
                        paddingLeft: 98,
                        tempPaddingRL: 98,
                        tempPaddingRight: 98,
                        tempPaddingLeft: 98,
                        tempFillType: "outline",
                        borderColorOpacity: 1,
                        paddingTB: 16,
                        paddingTop: 16,
                        paddingBottom: 16,
                        tempPaddingTB: 16,
                        tempPaddingTop: 16,
                        tempPaddingBottom: 16,
                        bgColorOpacity: 0,
                        fillType: "outline",
                        colorPalette: "color3",
                        hoverBgColorOpacity: 1,
                        tempHoverBgColorOpacity: 1,
                        hoverBorderColorOpacity: 0.8
                      }
                    }
                  ]
                }
              }
            ],
            bgImageWidth: 1920,
            bgImageHeight: 893,
            bgImageSrc: "cb7f93f2ad863736ec5f0a0a6763f8f8f8e641b2.jpeg",
            mobileBgImageWidth: 1920,
            mobileBgImageHeight: 893,
            mobileBgImageSrc: "cb7f93f2ad863736ec5f0a0a6763f8f8f8e641b2.jpeg",
            containerSize: 60
          }
        }
      ]
    }
  }
};
