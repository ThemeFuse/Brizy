module.exports = {
id: "Gabi037Dark",

thumbnailWidth: 600,
thumbnailHeight: 417,
title: "Gabi037Dark",

keywords: "forms, contact, three columns, email, phone, social icons, info",
cat: [0, 11, 15, 13],
type: 1,

resolve: {
    type: "Section",
    value: {
        _styles: [
            "section"
        ],
        items: [
            {
                type: "SectionItem",
                value: {
                    _styles: [
                        "section-item"
                    ],
                    bgColorHex: "#191b21",
                    bgColorOpacity: 1,
                    mobileBgColorHex: "#191b21",
                    mobileBgColorOpacity: 1,
                    items: [
                        {
                            type: "Wrapper",
                            value: {
                                _styles: [
                                    "wrapper",
                                    "wrapper--richText"
                                ],
                                items: [
                                    {
                                        type: "RichText",
                                        value: {
                                            _styles: [
                                                "richText"
                                            ],
                                            text: "<h2 class=\"brz-text-lg-center brz-tp-heading2\"><span class=\"brz-cp-color8\">Come Visit Us</span></h2>"
                                        }
                                    }
                                ]
                            }
                        },
                        {
                            type: "Wrapper",
                            value: {
                                _styles: [
                                    "wrapper",
                                    "wrapper--richText"
                                ],
                                marginTop: 0,
                                items: [
                                    {
                                        type: "RichText",
                                        value: {
                                            _styles: [
                                                "richText"
                                            ],
                                            text: "<p class=\"brz-text-lg-center brz-tp-subtitle\"><em style=\"background-color: transparent;\" class=\"brz-cp-color7\">Just click &amp; drag to create beautiful pages</em></p><p class=\"brz-text-lg-center brz-tp-subtitle\"><em style=\"background-color: transparent;\" class=\"brz-cp-color7\"> in a matter of minutes.</em></p>"
                                        }
                                    }
                                ]
                            }
                        },
                        {
                            type: "Wrapper",
                            value: {
                                _styles: [
                                    "wrapper",
                                    "wrapper--spacer"
                                ],
                                items: [
                                    {
                                        type: "Spacer",
                                        value: {
                                            _styles: [
                                                "spacer"
                                            ],
                                            height: 30,
                                            mobileHeight: 30
                                        }
                                    }
                                ]
                            }
                        },
                        {
                            type: "Row",
                            value: {
                                _styles: [
                                    "row",
                                    "hide-row-borders",
                                    "padding-0"
                                ],
                                items: [
                                    {
                                        type: "Column",
                                        value: {
                                            _styles: [
                                                "column"
                                            ],
                                            items: [
                                                {
                                                    type: "Wrapper",
                                                    value: {
                                                        _styles: [
                                                            "wrapper",
                                                            "wrapper--iconText"
                                                        ],
                                                        items: [
                                                            {
                                                                type: "Form",
                                                                value: {
                                                                    _styles: [
                                                                        "form"
                                                                    ],
                                                                    items: [
                                                                        {
                                                                            type: "FormFields",
                                                                            value: {
                                                                                items: [
                                                                                    {
                                                                                        type: "FormField",
                                                                                        value: {
                                                                                            type: "Text",
                                                                                            label: "Name",
                                                                                            required: false,
                                                                                            options: [
                                                                                                "Option 1",
                                                                                                "Option 2"
                                                                                            ],
                                                                                            width: 50,
                                                                                            mobileWidth: 50
                                                                                        }
                                                                                    },
                                                                                    {
                                                                                        type: "FormField",
                                                                                        value: {
                                                                                            label: "Email",
                                                                                            required: false,
                                                                                            options: [
                                                                                                "Option 1",
                                                                                                "Option 2"
                                                                                            ],
                                                                                            width: 50,
                                                                                            mobileWidth: 50,
                                                                                            type: "Email"
                                                                                        }
                                                                                    },
                                                                                    {
                                                                                        type: "FormField",
                                                                                        value: {
                                                                                            type: "Select",
                                                                                            label: "Project",
                                                                                            required: false,
                                                                                            options: [
                                                                                                "Freelance",
                                                                                                "Business"
                                                                                            ]
                                                                                        }
                                                                                    },
                                                                                    {
                                                                                        type: "FormField",
                                                                                        value: {
                                                                                            type: "Paragraph",
                                                                                            label: "Paragraph",
                                                                                            required: false,
                                                                                            options: [
                                                                                                "Option 1",
                                                                                                "Option 2"
                                                                                            ]
                                                                                        }
                                                                                    }
                                                                                ],
                                                                                fontStyle: "paragraph",
                                                                                fontWeight: 300,
                                                                                lineHeight: 1.6999999999999999555910790149937383830547332763671875,
                                                                                mobileFontStyle: "paragraph",
                                                                                mobileFontSize: 15,
                                                                                mobileLineHeight: 1.600000000000000088817841970012523233890533447265625,
                                                                                bgColorPalette: "color8",
                                                                                colorPalette: "color7",
                                                                                borderColorPalette: "color7",
                                                                                borderColorHex: "#73777f",
                                                                                borderColorOpacity: 0.1499999999999999944488848768742172978818416595458984375
                                                                            }
                                                                        },
                                                                        {
                                                                            type: "Button",
                                                                            value: {
                                                                                _styles: [
                                                                                    "button",
                                                                                    "submit"
                                                                                ],
                                                                                hoverBgColorOpacity: 1,
                                                                                hoverBorderColorPalette: "",
                                                                                tempHoverBorderColorPalette: "",
                                                                                tempHoverBgColorOpacity: 1,
                                                                                hoverBgColorPalette: "color2",
                                                                                text: "SUBMIT",
                                                                                iconName: "",
                                                                                iconType: ""
                                                                            }
                                                                        }
                                                                    ]
                                                                }
                                                            }
                                                        ]
                                                    }
                                                }
                                            ],
                                            width: 100,
                                            paddingTop: 70,
                                            paddingRight: 80,
                                            paddingBottom: 70,
                                            paddingLeft: 80,
                                            padding: 80,
                                            bgColorPalette: "color8",
                                            bgColorHex: "",
                                            bgColorOpacity: 1,
                                            mobileBgColorPalette: "color8",
                                            mobileBgColorHex: "",
                                            mobileBgColorOpacity: 1,
                                            mobilePaddingRight: 10,
                                            mobilePaddingLeft: 10
                                        }
                                    }
                                ]
                            }
                        },
                        {
                            type: "Wrapper",
                            value: {
                                _styles: [
                                    "wrapper",
                                    "wrapper--spacer"
                                ],
                                items: [
                                    {
                                        type: "Spacer",
                                        value: {
                                            _styles: [
                                                "spacer"
                                            ],
                                            height: 30,
                                            mobileHeight: 30
                                        }
                                    }
                                ]
                            }
                        },
                        {
                            type: "Row",
                            value: {
                                _styles: [
                                    "row",
                                    "hide-row-borders",
                                    "padding-0"
                                ],
                                items: [
                                    {
                                        type: "Column",
                                        value: {
                                            _styles: [
                                                "column"
                                            ],
                                            items: [
                                                {
                                                    type: "Wrapper",
                                                    value: {
                                                        _styles: [
                                                            "wrapper",
                                                            "wrapper--richText"
                                                        ],
                                                        items: [
                                                            {
                                                                type: "RichText",
                                                                value: {
                                                                    _styles: [
                                                                        "richText"
                                                                    ],
                                                                    text: "<p class=\"brz-mb-lg-5 brz-text-lg-center brz-tp-abovetitle\"><span class=\"brz-cp-color3\">E-MAIL</span></p><h6 class=\"brz-text-lg-center brz-tp-heading6\"><span class=\"brz-cp-color8\">Hello@Brizy.com</span></h6>"
                                                                }
                                                            }
                                                        ]
                                                    }
                                                }
                                            ],
                                            width: 33.2999999999999971578290569595992565155029296875
                                        }
                                    },
                                    {
                                        type: "Column",
                                        value: {
                                            _styles: [
                                                "column"
                                            ],
                                            items: [
                                                {
                                                    type: "Wrapper",
                                                    value: {
                                                        _styles: [
                                                            "wrapper",
                                                            "wrapper--richText"
                                                        ],
                                                        items: [
                                                            {
                                                                type: "RichText",
                                                                value: {
                                                                    _styles: [
                                                                        "richText"
                                                                    ],
                                                                    text: "<p class=\"brz-mb-lg-5 brz-text-lg-center brz-tp-abovetitle\"><span class=\"brz-cp-color3\">PHONE</span></p><h6 class=\"brz-text-lg-center brz-tp-heading6\"><span class=\"brz-cp-color8\">+(414) 345 62 45</span></h6>"
                                                                }
                                                            }
                                                        ]
                                                    }
                                                }
                                            ],
                                            width: 33.2999999999999971578290569595992565155029296875
                                        }
                                    },
                                    {
                                        type: "Column",
                                        value: {
                                            _styles: [
                                                "column"
                                            ],
                                            width: 33.39999999999999857891452847979962825775146484375,
                                            items: [
                                                {
                                                    type: "Wrapper",
                                                    value: {
                                                        _styles: [
                                                            "wrapper",
                                                            "wrapper--richText"
                                                        ],
                                                        marginBottom: 0,
                                                        items: [
                                                            {
                                                                type: "RichText",
                                                                value: {
                                                                    _styles: [
                                                                        "richText"
                                                                    ],
                                                                    text: "<p class=\"brz-tp-abovetitle brz-text-lg-center brz-mb-lg-5\"><span class=\"brz-cp-color3\">SOCIAL</span></p>"
                                                                }
                                                            }
                                                        ]
                                                    }
                                                },
                                                {
                                                    type: "Cloneable",
                                                    value: {
                                                        _styles: [
                                                            "wrapper-clone",
                                                            "wrapper-clone--icon"
                                                        ],
                                                        marginTop: 0,
                                                        items: [
                                                            {
                                                                type: "Icon",
                                                                value: {
                                                                    _styles: [
                                                                        "icon"
                                                                    ],
                                                                    name: "logo-twitter",
                                                                    size: "custom",
                                                                    customSize: 18,
                                                                    mobileSize: "custom",
                                                                    mobileCustomSize: 72,
                                                                    tempPadding: 0,
                                                                    tempBorderWidth: 1,
                                                                    hoverColorOpacity: 1,
                                                                    hoverBgColorHex: "#216fbd",
                                                                    hoverBorderColorHex: "#bde1f4",
                                                                    colorPalette: "color8"
                                                                }
                                                            },
                                                            {
                                                                type: "Icon",
                                                                value: {
                                                                    _styles: [
                                                                        "icon"
                                                                    ],
                                                                    name: "logo-fb-simple",
                                                                    size: "custom",
                                                                    customSize: 19,
                                                                    mobileSize: "custom",
                                                                    mobileCustomSize: 72,
                                                                    tempPadding: 0,
                                                                    tempBorderWidth: 1,
                                                                    hoverColorOpacity: 1,
                                                                    hoverBgColorHex: "#216fbd",
                                                                    hoverBorderColorHex: "#bde1f4",
                                                                    tempBorderColorOpacity: 0.01000000000000000020816681711721685132943093776702880859375,
                                                                    mobilePadding: 20,
                                                                    tempBgColorOpacity: 0.440000000000000002220446049250313080847263336181640625,
                                                                    colorPalette: "color8"
                                                                }
                                                            },
                                                            {
                                                                type: "Icon",
                                                                value: {
                                                                    _styles: [
                                                                        "icon"
                                                                    ],
                                                                    name: "logo-google-plus",
                                                                    size: "custom",
                                                                    customSize: 23,
                                                                    mobileSize: "custom",
                                                                    mobileCustomSize: 72,
                                                                    tempPadding: 0,
                                                                    tempBorderWidth: 1,
                                                                    hoverColorOpacity: 1,
                                                                    hoverBgColorHex: "#216fbd",
                                                                    hoverBorderColorHex: "#bde1f4",
                                                                    mobilePadding: 20,
                                                                    tempBgColorOpacity: 0.01000000000000000020816681711721685132943093776702880859375,
                                                                    colorPalette: "color8"
                                                                }
                                                            }
                                                        ]
                                                    }
                                                }
                                            ]
                                        }
                                    }
                                ]
                            }
                        }
                    ],
                    containerSize: 70
                }
            }
        ]
    }
}
};