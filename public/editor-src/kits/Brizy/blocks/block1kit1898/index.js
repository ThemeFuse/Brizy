module.exports = {
  id: "block1kit1898",
  thumbnailWidth: 600,
  thumbnailHeight: 330,
  title: "block1kit1898",

  keywords: "Blog,header,Post",
  cat: [4395, 100],
  type: 0,

  resolve: {
    type: "Section",
    value: {
      _styles: ["section"],
      items: [
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            items: [
              {
                type: "Row",
                value: {
                  _styles: ["row"],
                  items: [
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Row",
                            value: {
                              _styles: ["row", "hide-row-borders", "padding-0"],
                              items: [
                                {
                                  type: "Column",
                                  value: {
                                    _styles: ["column"],
                                    items: [
                                      {
                                        type: "Wrapper",
                                        value: {
                                          _styles: [
                                            "wrapper",
                                            "wrapper--image"
                                          ],
                                          items: [
                                            {
                                              type: "Image",
                                              value: {
                                                _styles: ["image"],
                                                resize: 68,
                                                height: 100,
                                                imageWidth: 400,
                                                imageHeight: 400,
                                                imageSrc:
                                                  "1ce72f3e257a3c8cfd6be5f3fc202db9.jpg",
                                                positionX: 50,
                                                positionY: 50,
                                                imagePopulation: "",
                                                borderRadiusType: "rounded",
                                                borderRadius: 47,
                                                tempBorderRadius: 40,
                                                boxShadow: "on",
                                                boxShadowBlur: 0,
                                                boxShadowColorOpacity: 1,
                                                boxShadowSpread: 3,
                                                boxShadowVertical: 0,
                                                boxShadowHorizontal: 0,
                                                boxShadowColorPalette: "color8",
                                                boxShadowColorHex: "",
                                                mobileResize: 25,
                                                tabletResize: 100
                                              }
                                            }
                                          ],
                                          horizontalAlign: "left"
                                        }
                                      }
                                    ],
                                    width: 11.2,
                                    paddingType: "ungrouped",
                                    padding: 0,
                                    paddingSuffix: "px",
                                    paddingTop: 0,
                                    paddingRight: 0,
                                    paddingBottom: 0,
                                    paddingLeft: 0,
                                    paddingRightSuffix: "px",
                                    paddingBottomSuffix: "px",
                                    paddingLeftSuffix: "px"
                                  }
                                },
                                {
                                  type: "Column",
                                  value: {
                                    _styles: ["column"],
                                    items: [
                                      {
                                        type: "Wrapper",
                                        value: {
                                          _styles: [
                                            "wrapper",
                                            "wrapper--richText"
                                          ],
                                          items: [
                                            {
                                              type: "RichText",
                                              value: {
                                                _styles: ["richText"],
                                                text:
                                                  '<p class="brz-lh-lg-1_7 brz-lh-im-1_6 brz-fs-lg-16 brz-fs-im-15 brz-ls-lg-0 brz-ls-im-0 brz-ff-montserrat brz-fw-lg-700 brz-fw-im-700"><span class="brz-cp-color4">By Jonathan Lorne</span></p><p class="brz-lh-lg-1_7 brz-lh-im-1_6 brz-ls-lg-0 brz-ls-im-0 brz-fs-lg-14 brz-fs-im-13 brz-ff-montserrat brz-fw-lg-400 brz-fw-im-400"><span class="brz-cp-color4" style="opacity: 0.66;">Senior Contributor&nbsp;</span></p>'
                                              }
                                            }
                                          ]
                                        }
                                      }
                                    ],
                                    width: 88.8,
                                    verticalAlign: "center",
                                    paddingLeft: 5,
                                    paddingLeftSuffix: "px",
                                    padding: 15,
                                    mobileBgColorPalette: "color8",
                                    mobileBgColorHex: "#ffffff",
                                    mobileBgColorOpacity: 0,
                                    mobilePaddingRight: 0,
                                    mobilePaddingLeft: 0
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 325,
                                    tabletHeight: 250
                                  }
                                }
                              ],
                              showOnMobile: "off"
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-tp-button brz-text-lg-left brz-mt-lg-0 brz-mb-lg-15"><span class="brz-cp-color3">LIVE MUSIC &amp; TRAVEL</span></p>'
                                  }
                                }
                              ],
                              marginTop: 0,
                              marginTopSuffix: "px",
                              margin: 0,
                              marginBottom: 0,
                              marginBottomSuffix: "px"
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-fs-sm-50 brz-fw-sm-900 brz-mt-lg-10 brz-ls-lg-0 brz-fs-lg-53 brz-fw-lg-900 brz-ff-montserrat brz-lh-lg-1_3 brz-mb-lg-20 brz-fw-xs-900 brz-fs-xs-38"><span class="brz-cp-color2">BURNING MAN FESTIVAL EXPERIENCE</span></p>'
                                  }
                                }
                              ],
                              paddingType: "ungrouped",
                              paddingRight: 60,
                              paddingRightSuffix: "px",
                              padding: 0
                            }
                          },
                          {
                            type: "Row",
                            value: {
                              _styles: ["row", "hide-row-borders", "padding-0"],
                              items: [
                                {
                                  type: "Column",
                                  value: {
                                    _styles: ["column"],
                                    items: [
                                      {
                                        type: "Cloneable",
                                        value: {
                                          _styles: [
                                            "wrapper-clone",
                                            "wrapper-clone--button"
                                          ],
                                          items: [
                                            {
                                              type: "Button",
                                              value: {
                                                _styles: ["button"],
                                                iconName: "tail-right",
                                                iconType: "glyph",
                                                text: "READ MORE",
                                                size: "custom",
                                                mobileSize: "small",
                                                fontSize: 12,
                                                paddingTB: 12,
                                                paddingTop: 12,
                                                paddingBottom: 12,
                                                tempPaddingTB: 12,
                                                tempPaddingTop: 12,
                                                tempPaddingBottom: 12,
                                                paddingRL: 36,
                                                paddingRight: 36,
                                                paddingLeft: 36,
                                                tempPaddingRL: 36,
                                                tempPaddingRight: 36,
                                                tempPaddingLeft: 36,
                                                borderRadius: 23,
                                                borderWidth: 0,
                                                mobilePaddingTop: 11,
                                                mobilePaddingRight: 26,
                                                mobilePaddingBottom: 11,
                                                mobilePaddingLeft: 26,
                                                mobileBorderRadius: 24,
                                                tempBorderWidth: 2,
                                                borderRadiusType: "rounded",
                                                fillType: "filled",
                                                borderColorOpacity: 0,
                                                borderColorPalette: 0,
                                                bgColorOpacity: 1,
                                                bgColorPalette: "color3",
                                                hoverBgColorOpacity: 0.8,
                                                tempBorderRadiusType: "rounded",
                                                hoverBorderColorOpacity: 0.8
                                              }
                                            }
                                          ],
                                          horizontalAlign: "left"
                                        }
                                      }
                                    ],
                                    paddingType: "grouped",
                                    padding: 0,
                                    paddingSuffix: "px",
                                    paddingTop: 0,
                                    paddingRight: 0,
                                    paddingBottom: 0,
                                    paddingLeft: 0,
                                    width: 31.2,
                                    tabletPaddingLeft: 0,
                                    tabletPadding: 0,
                                    tabletPaddingLeftSuffix: "px",
                                    tempTabletPaddingLeftSuffix: "px",
                                    tabletPaddingSuffix: "px",
                                    tempTabletPaddingSuffix: "px"
                                  }
                                },
                                {
                                  type: "Column",
                                  value: {
                                    _styles: ["column"],
                                    items: [
                                      {
                                        type: "Wrapper",
                                        value: {
                                          _styles: [
                                            "wrapper",
                                            "wrapper--iconText"
                                          ],
                                          items: [
                                            {
                                              type: "IconText",
                                              value: {
                                                _styles: ["iconText"],
                                                items: [
                                                  {
                                                    type: "Icon",
                                                    value: {
                                                      _styles: [
                                                        "icon",
                                                        "iconText--icon"
                                                      ],
                                                      name:
                                                        "small-triangle-right",
                                                      type: "glyph",
                                                      fillType: "outline",
                                                      tempFillType: "outline",
                                                      padding: 7,
                                                      borderRadiusType:
                                                        "rounded",
                                                      borderRadius: 21,
                                                      borderWidth: 2,
                                                      borderColorOpacity: 1,
                                                      borderColorPalette: "",
                                                      bgColorOpacity: 0,
                                                      bgColorPalette: "",
                                                      hoverBgColorOpacity: 1,
                                                      hoverBgColorPalette:
                                                        "color3",
                                                      hoverBorderColorOpacity: 1,
                                                      hoverBorderColorPalette:
                                                        "color3",
                                                      tempBorderWidth: 2,
                                                      tempPadding: 7,
                                                      size: "custom",
                                                      customSize: 23,
                                                      colorPalette: "",
                                                      colorOpacity: 1,
                                                      tempBorderColorPalette:
                                                        "color2",
                                                      colorHex: "#ffffff",
                                                      tempColorOpacity: 1,
                                                      hoverColorHex: "#ffffff",
                                                      hoverColorOpacity: 0.8,
                                                      borderColorHex: "#ffffff",
                                                      tempBorderColorOpacity: 1,
                                                      hoverBgColorHex:
                                                        "#37589a",
                                                      hoverColorPalette:
                                                        "color8"
                                                    }
                                                  },
                                                  {
                                                    type: "RichText",
                                                    value: {
                                                      _styles: [
                                                        "text",
                                                        "iconText--text"
                                                      ],
                                                      text:
                                                        '<p class="brz-lh-im-1_3 brz-lh-lg-1_4 brz-fs-im-13 brz-fs-lg-14 brz-ff-montserrat brz-ls-im-0 brz-ls-lg-0 brz-fw-lg-500 brz-fw-im-500"><span style="color: rgb(255, 255, 255);">Discover the Dust &amp; Fire 2018 Event</span></p><p class="brz-fs-im-13 brz-fs-lg-14 brz-fw-im-300 brz-fw-lg-300 brz-lh-im-1_6 brz-lh-lg-1_6 brz-ls-im-0 brz-ls-lg-0 brz-ff-montserrat"><span style="color: rgb(255, 255, 255);">23:45&nbsp;</span></p>'
                                                    }
                                                  }
                                                ],
                                                iconSpacing: 14
                                              }
                                            }
                                          ],
                                          padding: 0,
                                          paddingSuffix: "px",
                                          paddingTop: 0,
                                          paddingRight: 0,
                                          paddingBottom: 0,
                                          paddingLeft: 0
                                        }
                                      }
                                    ],
                                    width: 68.8,
                                    mobileBgColorPalette: "color2",
                                    mobileBgColorHex: "#142850",
                                    mobileBgColorOpacity: 0.81,
                                    mobilePaddingRight: 15,
                                    mobilePaddingLeft: 15,
                                    mobileTabsState: "",
                                    mobileTabsColor: "",
                                    mobilePaddingSuffix: "px",
                                    mobilePaddingTopSuffix: "px",
                                    mobilePaddingRightSuffix: "px",
                                    mobilePaddingBottomSuffix: "px",
                                    mobilePaddingLeftSuffix: "px",
                                    tempMobilePaddingSuffix: "px",
                                    tempMobilePaddingTopSuffix: "px",
                                    tempMobilePaddingRightSuffix: "px",
                                    tempMobilePaddingBottomSuffix: "px",
                                    tempMobilePaddingLeftSuffix: "px",
                                    mobilePadding: 15,
                                    mobilePaddingTop: 5,
                                    mobilePaddingBottom: 5,
                                    tempMobilePadding: 15,
                                    tempMobilePaddingTop: 5,
                                    tempMobilePaddingRight: 15,
                                    tempMobilePaddingBottom: 5,
                                    tempMobilePaddingLeft: 15,
                                    mobilePaddingType: "ungrouped"
                                  }
                                }
                              ]
                            }
                          }
                        ],
                        paddingRight: 15,
                        paddingRightSuffix: "%",
                        padding: 15,
                        paddingLeft: 10,
                        paddingLeftSuffix: "%",
                        verticalAlign: "center",
                        paddingTop: 20,
                        paddingTopSuffix: "px",
                        paddingBottom: 30,
                        paddingBottomSuffix: "px",
                        width: 46,
                        paddingType: "grouped",
                        mobileBgColorPalette: "color8",
                        mobileBgColorHex: "#ffffff",
                        mobileBgColorOpacity: 0.88,
                        mobilePaddingRight: 25,
                        mobilePaddingLeft: 25,
                        tabletWidth: 100,
                        mobilePaddingSuffix: "px",
                        mobilePaddingTopSuffix: "px",
                        mobilePaddingRightSuffix: "px",
                        mobilePaddingBottomSuffix: "px",
                        mobilePaddingLeftSuffix: "px",
                        tempMobilePaddingSuffix: "px",
                        tempMobilePaddingTopSuffix: "px",
                        tempMobilePaddingRightSuffix: "px",
                        tempMobilePaddingBottomSuffix: "px",
                        tempMobilePaddingLeftSuffix: "px",
                        mobilePadding: 25,
                        mobilePaddingTop: 25,
                        mobilePaddingBottom: 25,
                        tempMobilePadding: 25,
                        tempMobilePaddingTop: 25,
                        tempMobilePaddingRight: 25,
                        tempMobilePaddingBottom: 25,
                        tempMobilePaddingLeft: 25
                      }
                    },
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"]
                                  }
                                }
                              ],
                              showOnMobile: "off",
                              showOnTablet: "off"
                            }
                          }
                        ],
                        verticalAlign: "center",
                        borderWidth: 0,
                        borderTopWidth: 0,
                        borderRightWidth: 0,
                        borderBottomWidth: 0,
                        borderLeftWidth: 0,
                        tempBorderWidth: 4,
                        tempBorderTopWidth: 4,
                        tempBorderRightWidth: 4,
                        tempBorderBottomWidth: 4,
                        tempBorderLeftWidth: 4,
                        borderColorOpacity: 0,
                        borderRadius: 0,
                        borderTopLeftRadius: 0,
                        borderTopRightRadius: 0,
                        borderBottomLeftRadius: 0,
                        borderBottomRightRadius: 0,
                        borderColorPalette: "color8",
                        borderColorHex: "#ffffff",
                        mobilePaddingRight: 10,
                        mobilePaddingLeft: 10,
                        tempBorderColorOpacity: 0.28,
                        tempBorderRadius: 0,
                        tempBorderTopLeftRadius: 0,
                        tempBorderTopRightRadius: 0,
                        tempBorderBottomRightRadius: 0,
                        tempBorderBottomLeftRadius: 0,
                        bgColorOpacity: 0,
                        margin: 35,
                        marginSuffix: "px",
                        marginTop: 0,
                        marginRight: 100,
                        marginBottom: 0,
                        marginLeft: 100,
                        marginType: "ungrouped",
                        marginRightSuffix: "px",
                        marginLeftSuffix: "px",
                        marginTopSuffix: "px",
                        marginBottomSuffix: "px",
                        width: 54,
                        showOnDesktop: "on",
                        showOnMobile: "off",
                        showOnTablet: "off"
                      }
                    }
                  ],
                  size: 90
                }
              }
            ],
            containerType: "fullWidth",
            bgColorPalette: "color3",
            bgColorHex: "#239ddb",
            bgColorOpacity: 0,
            bgImageWidth: 1920,
            bgImageHeight: 1179,
            bgImageSrc: "00ca36aade1b763eef394af86caa6ec4.jpg",
            bgPositionX: 53,
            bgPositionY: 100,
            tempBgColorOpacity: 0.44,
            tempMobileBgColorOpacity: 0.9,
            paddingType: "ungrouped",
            paddingTop: 50,
            paddingBottom: 50,
            padding: 75,
            tabsColor: "",
            tabletBgColorPalette: "color2",
            tempTabletBgColorPalette: "color2",
            tabletBgColorOpacity: 0.24,
            tabletBorderRadius: 0,
            tabletBgColorHex: "#ffffff",
            tempTabletBgColorOpacity: 0.24
          }
        }
      ]
    }
  }
};
