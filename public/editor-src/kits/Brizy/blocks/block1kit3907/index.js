module.exports = {
  id: "block1kit3907",
  thumbnailWidth: 600,
  thumbnailHeight: 312,
  title: "block1kit3907",

  keywords: "article,Blog,news",
  cat: [4667, 100],
  type: 0,

  resolve: {
    type: "Section",
    value: {
      _styles: ["section"],
      items: [
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            items: [
              {
                type: "Row",
                value: {
                  _styles: ["row"],
                  items: [
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Cloneable",
                            value: {
                              _styles: ["wrapper-clone", "wrapper-clone--icon"],
                              items: [
                                {
                                  type: "Icon",
                                  value: {
                                    _styles: ["icon"],
                                    name: "logo-fb-simple",
                                    type: "glyph",
                                    fillType: "filled",
                                    tempFillType: "filled",
                                    padding: 8,
                                    borderRadiusType: "custom",
                                    borderRadius: 4,
                                    borderWidth: 3,
                                    borderColorOpacity: 1,
                                    borderColorPalette: "",
                                    bgColorOpacity: 1,
                                    bgColorPalette: "",
                                    hoverBgColorOpacity: 1,
                                    hoverBgColorPalette: "",
                                    hoverBorderColorOpacity: 1,
                                    hoverBorderColorPalette: "",
                                    mobilePadding: 10,
                                    mobileBorderRadius: 6,
                                    tempPadding: 8,
                                    size: "custom",
                                    customSize: 15,
                                    colorHex: "#ffffff",
                                    colorOpacity: 1,
                                    colorPalette: "",
                                    tempColorOpacity: 1,
                                    hoverColorHex: "#3b5998",
                                    hoverColorOpacity: 1,
                                    borderColorHex: "#3b5998",
                                    tempBorderColorOpacity: 1,
                                    hoverBgColorHex: "#ffffff",
                                    hoverColorPalette: "",
                                    tempBgColorPalette: "color3",
                                    tempBorderColorPalette: "color3",
                                    hoverBorderColorHex: "#ffffff",
                                    bgColorHex: "#3b5998",
                                    tempBgColorOpacity: 1,
                                    tempBorderRadiusType: "custom",
                                    tempBorderRadius: 4,
                                    tempMobileBorderRadius: 4
                                  }
                                },
                                {
                                  type: "Icon",
                                  value: {
                                    _styles: ["icon"],
                                    name: "logo-twitter",
                                    type: "glyph",
                                    fillType: "filled",
                                    tempFillType: "filled",
                                    padding: 8,
                                    borderRadiusType: "custom",
                                    borderRadius: 4,
                                    borderWidth: 3,
                                    borderColorOpacity: 1,
                                    borderColorPalette: "color3",
                                    bgColorOpacity: 1,
                                    bgColorPalette: "color3",
                                    hoverBgColorOpacity: 1,
                                    hoverBgColorPalette: "",
                                    hoverBorderColorOpacity: 1,
                                    hoverBorderColorPalette: "",
                                    mobilePadding: 10,
                                    mobileBorderRadius: 6,
                                    tempPadding: 8,
                                    size: "custom",
                                    customSize: 15,
                                    colorHex: "#ffffff",
                                    colorOpacity: 1,
                                    colorPalette: "",
                                    tempColorOpacity: 1,
                                    hoverColorHex: "#239ddb",
                                    hoverColorOpacity: 1,
                                    borderColorHex: "#239ddb",
                                    tempBorderColorOpacity: 1,
                                    hoverBgColorHex: "#ffffff",
                                    hoverColorPalette: "",
                                    tempBgColorPalette: "color3",
                                    tempBorderColorPalette: "color3",
                                    hoverBorderColorHex: "#ffffff",
                                    bgColorHex: "#239ddb",
                                    tempBgColorOpacity: 1,
                                    tempBorderRadiusType: "custom"
                                  }
                                },
                                {
                                  type: "Icon",
                                  value: {
                                    _styles: ["icon"],
                                    name: "logo-pinterest",
                                    type: "glyph",
                                    fillType: "filled",
                                    tempFillType: "filled",
                                    padding: 8,
                                    borderRadiusType: "custom",
                                    borderRadius: 4,
                                    borderWidth: 3,
                                    borderColorOpacity: 1,
                                    borderColorPalette: "",
                                    bgColorOpacity: 1,
                                    bgColorPalette: "",
                                    hoverBgColorOpacity: 1,
                                    hoverBgColorPalette: "",
                                    hoverBorderColorOpacity: 1,
                                    hoverBorderColorPalette: "",
                                    mobilePadding: 10,
                                    mobileBorderRadius: 6,
                                    tempPadding: 8,
                                    size: "custom",
                                    customSize: 15,
                                    colorHex: "#ffffff",
                                    colorOpacity: 1,
                                    colorPalette: "",
                                    tempColorOpacity: 1,
                                    hoverColorHex: "#bd081c",
                                    hoverColorOpacity: 1,
                                    borderColorHex: "#bd081c",
                                    tempBorderColorOpacity: 1,
                                    hoverBgColorHex: "#ffffff",
                                    hoverColorPalette: "",
                                    tempBgColorPalette: "color3",
                                    tempBorderColorPalette: "color3",
                                    hoverBorderColorHex: "#ffffff",
                                    bgColorHex: "#bd081c",
                                    tempBgColorOpacity: 1,
                                    tempBorderRadiusType: "custom",
                                    tempBorderRadius: 4,
                                    tempMobileBorderRadius: 4
                                  }
                                }
                              ],
                              horizontalAlign: "left",
                              itemPadding: 15,
                              itemPaddingRight: 15,
                              itemPaddingLeft: 15,
                              paddingType: "ungrouped",
                              paddingLeft: 10,
                              paddingLeftSuffix: "px",
                              padding: 0
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 10,
                                    mobileHeight: 50
                                  }
                                }
                              ],
                              showOnMobile: "off"
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-text-xs-left brz-ff-montserrat brz-fw-lg-700 brz-fs-lg-75 brz-ls-lg-m_5 brz-text-lg-left brz-lh-lg-1_1 brz-fs-sm-55 brz-fs-xs-45"><span style="color: rgb(9, 9, 56);">Best Practices For Mobile Form Design</span></p>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--spacer"],
                              items: [
                                {
                                  type: "Spacer",
                                  value: {
                                    _styles: ["spacer"],
                                    height: 15,
                                    mobileHeight: 50
                                  }
                                }
                              ],
                              showOnMobile: "off"
                            }
                          },
                          {
                            type: "Cloneable",
                            value: {
                              _styles: [
                                "wrapper-clone",
                                "wrapper-clone--button"
                              ],
                              items: [
                                {
                                  type: "Button",
                                  value: {
                                    _styles: ["button"],
                                    text: "by Aaron Laweance",
                                    fillType: "filled",
                                    tempFillType: "outline",
                                    paddingRL: 26,
                                    paddingRight: 26,
                                    paddingLeft: 26,
                                    paddingTB: 11,
                                    paddingTop: 11,
                                    paddingBottom: 11,
                                    borderRadiusType: "square",
                                    borderRadius: 2,
                                    borderWidth: 2,
                                    borderColorOpacity: 1,
                                    borderColorPalette: "",
                                    bgColorOpacity: 1,
                                    bgColorPalette: "",
                                    hoverBgColorOpacity: 1,
                                    hoverBorderColorOpacity: 1,
                                    mobilePaddingRL: 26,
                                    mobilePaddingRight: 26,
                                    mobilePaddingLeft: 26,
                                    iconName: "",
                                    iconType: "",
                                    borderColorHex: "#ffffff",
                                    tempBorderColorOpacity: 1,
                                    tempBorderColorPalette: "",
                                    hoverBgColorHex: "#ffffff",
                                    hoverBgColorPalette: "",
                                    hoverBorderColorPalette: "",
                                    tempHoverBorderColorPalette: "color3",
                                    tempHoverBgColorOpacity: 1,
                                    tempHoverBgColorPalette: "",
                                    hoverBorderColorHex: "#ffffff",
                                    hoverColorPalette: "",
                                    hoverColorOpacity: 0.82,
                                    size: "small",
                                    mobileSize: "small",
                                    fontSize: 11,
                                    tempPaddingTB: 11,
                                    tempPaddingTop: 11,
                                    tempPaddingBottom: 11,
                                    tempPaddingRL: 26,
                                    tempPaddingRight: 26,
                                    tempPaddingLeft: 26,
                                    mobilePaddingTop: 11,
                                    mobilePaddingBottom: 11,
                                    colorHex: "#090938",
                                    colorOpacity: 1,
                                    colorPalette: "",
                                    tempColorOpacity: 1,
                                    hoverColorHex: "#090938",
                                    bgColorHex: "#ffffff",
                                    tempBgColorPalette: "",
                                    tempBgColorOpacity: 1
                                  }
                                }
                              ],
                              horizontalAlign: "left",
                              marginBottom: 10,
                              marginBottomSuffix: "px",
                              margin: 0
                            }
                          }
                        ],
                        width: 100,
                        verticalAlign: "center",
                        paddingRight: 15,
                        paddingRightSuffix: "px",
                        padding: 15,
                        paddingLeft: 48,
                        paddingLeftSuffix: "%",
                        bgImageWidth: 0,
                        bgImageHeight: 0,
                        bgImageSrc: "",
                        bgPositionX: 50,
                        bgPositionY: 50,
                        bgColorOpacity: 0,
                        tempBgColorOpacity: 0.95,
                        mobilePaddingRight: 0,
                        mobilePaddingLeft: 0,
                        bgColorHex: "#ffffff",
                        bgColorPalette: "",
                        borderRadius: 0,
                        borderTopLeftRadius: 0,
                        borderTopRightRadius: 0,
                        borderBottomRightRadius: 0,
                        borderBottomLeftRadius: 0,
                        paddingTop: 5,
                        paddingTopSuffix: "px",
                        paddingBottom: 5,
                        paddingBottomSuffix: "px",
                        paddingType: "ungrouped",
                        tabletPaddingRight: 100,
                        tabletPaddingRightSuffix: "px",
                        tempTabletPaddingRightSuffix: "px",
                        tempTabletPaddingRight: 100
                      }
                    }
                  ],
                  paddingType: "grouped",
                  paddingRight: 0,
                  paddingRightSuffix: "px",
                  padding: 10,
                  paddingLeft: 0,
                  paddingLeftSuffix: "px"
                }
              }
            ],
            containerType: "boxed",
            paddingType: "ungrouped",
            paddingTop: 166,
            paddingBottom: 193,
            padding: 75,
            bgImageWidth: 1920,
            bgImageHeight: 1282,
            bgImageSrc: "2ddbfb5b6a4c4616be4d093b8974bc69.jpg",
            bgPositionX: 54,
            bgPositionY: 38,
            bgColorOpacity: 0,
            tempBgColorOpacity: 0.1,
            mobileBgColorOpacity: 0.5,
            tempMobileBgColorOpacity: 1,
            bgColorHex: "#ffffff",
            bgColorPalette: "color8",
            tabsColor: "",
            tabletBgColorHex: "#ffffff",
            tabletBgColorOpacity: 0.45,
            tempTabletBgColorOpacity: 0.45,
            tabletBgColorPalette: "",
            tempTabletBgColorPalette: "",
            tabletBorderRadius: 0,
            mobilePaddingRightSuffix: "px",
            tempMobilePaddingRightSuffix: "px",
            mobilePaddingSuffix: "px",
            tempMobilePaddingSuffix: "px",
            mobilePaddingRight: 15,
            tempMobilePaddingRight: 15,
            mobilePaddingLeftSuffix: "px",
            tempMobilePaddingLeftSuffix: "px",
            mobilePaddingLeft: 15,
            tempMobilePaddingLeft: 15
          }
        }
      ],
      fullHeight: "off"
    }
  }
};
