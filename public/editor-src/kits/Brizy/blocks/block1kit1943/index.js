module.exports = {
  id: "block1kit1943",
  thumbnailWidth: 600,
  thumbnailHeight: 317,
  title: "block1kit1943",

  keywords: "Blog,header,Post",
  cat: [4395, 100],
  type: 0,

  resolve: {
    type: "Section",
    value: {
      _styles: ["section"],
      items: [
        {
          type: "SectionItem",
          value: {
            _styles: ["section-item"],
            items: [
              {
                type: "Wrapper",
                value: {
                  _styles: ["wrapper", "wrapper--progressBar"],
                  items: [
                    {
                      type: "ProgressBar",
                      value: {
                        _styles: ["progressBar"],
                        showPercentage: "off",
                        percentage: 100,
                        fontSize: 14,
                        fontFamily: "montserrat",
                        lineHeight: 1.2,
                        letterSpacing: 0,
                        fontWeight: 400,
                        fontStyle: "",
                        width: 15,
                        mobileWidth: 65,
                        text:
                          "\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0\u00a0 August 26",
                        tabletWidth: 40
                      }
                    }
                  ],
                  horizontalAlign: "left",
                  paddingType: "ungrouped",
                  paddingLeft: 0,
                  paddingLeftSuffix: "px",
                  padding: 0,
                  marginRight: 100,
                  marginRightSuffix: "px",
                  margin: 0,
                  marginType: "ungrouped"
                }
              },
              {
                type: "Row",
                value: {
                  _styles: ["row"],
                  items: [
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-fs-lg-55 brz-fw-lg-400 brz-ff-playfair_display brz-mb-lg-0 brz-lh-lg-1_3 brz-ls-lg-m_0_5"><span class="brz-cp-color2">Getting over heartbreak</span></p><p class="brz-fs-sm-54 brz-fs-lg-55 brz-fw-lg-400 brz-ff-playfair_display brz-mb-lg-0 brz-lh-lg-1_3 brz-ls-lg-m_0_5 brz-fs-xs-50"><span class="brz-cp-color2">My personal journey.</span></p>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Wrapper",
                            value: {
                              _styles: ["wrapper", "wrapper--richText"],
                              items: [
                                {
                                  type: "RichText",
                                  value: {
                                    _styles: ["richText"],
                                    text:
                                      '<p class="brz-mt-lg-10 brz-text-lg-left brz-tp-paragraph brz-mb-lg-20"><em class="brz-cp-color1">Discover the personal story and rollercoaster feelings that flood you in when dealing with relationship troubles and how to get back.</em></p>'
                                  }
                                }
                              ]
                            }
                          },
                          {
                            type: "Cloneable",
                            value: {
                              _styles: [
                                "wrapper-clone",
                                "wrapper-clone--button"
                              ],
                              items: [
                                {
                                  type: "Button",
                                  value: {
                                    _styles: ["button"],
                                    iconName: "",
                                    iconType: "",
                                    text: "READ ARTICLE",
                                    size: "custom",
                                    mobileSize: "small",
                                    fontSize: 12,
                                    paddingTB: 16,
                                    paddingTop: 16,
                                    paddingBottom: 16,
                                    tempPaddingTB: 16,
                                    tempPaddingTop: 16,
                                    tempPaddingBottom: 16,
                                    paddingRL: 23,
                                    paddingRight: 23,
                                    paddingLeft: 23,
                                    tempPaddingRL: 23,
                                    tempPaddingRight: 23,
                                    tempPaddingLeft: 23,
                                    borderRadius: 2,
                                    borderWidth: 0,
                                    mobilePaddingTop: 11,
                                    mobilePaddingRight: 26,
                                    mobilePaddingBottom: 11,
                                    mobilePaddingLeft: 26,
                                    tempBorderWidth: 2,
                                    borderRadiusType: "square",
                                    fillType: "filled",
                                    borderColorOpacity: 0,
                                    borderColorPalette: 0,
                                    bgColorOpacity: 1,
                                    bgColorPalette: "color3",
                                    hoverBgColorOpacity: 0.8
                                  }
                                }
                              ],
                              horizontalAlign: "left"
                            }
                          }
                        ],
                        paddingRight: 20,
                        paddingRightSuffix: "%",
                        padding: 15,
                        paddingLeft: 10,
                        paddingLeftSuffix: "%",
                        verticalAlign: "center",
                        paddingTop: 20,
                        paddingTopSuffix: "px",
                        paddingBottom: 30,
                        paddingBottomSuffix: "px"
                      }
                    },
                    {
                      type: "Column",
                      value: {
                        _styles: ["column"],
                        items: [
                          {
                            type: "Cloneable",
                            value: {
                              _styles: ["wrapper-clone", "wrapper-clone--icon"],
                              items: [
                                {
                                  type: "Icon",
                                  value: {
                                    _styles: ["icon"],
                                    name: "button-play",
                                    type: "glyph",
                                    fillType: "filled",
                                    tempFillType: "filled",
                                    padding: 33,
                                    borderRadiusType: "rounded",
                                    borderRadius: 52,
                                    borderWidth: 0,
                                    borderColorOpacity: 0,
                                    borderColorPalette: 0,
                                    bgColorOpacity: 1,
                                    bgColorPalette: "color2",
                                    hoverBgColorOpacity: 1,
                                    hoverBgColorPalette: "color3",
                                    hoverBorderColorOpacity: 1,
                                    hoverBorderColorPalette: "color3",
                                    mobileBorderRadius: 60,
                                    tempBgColorPalette: "color2",
                                    borderColorHex: "#239ddb",
                                    tempBorderColorOpacity: 1,
                                    hoverBgColorHex: "#239ddb",
                                    colorPalette: "color8",
                                    colorOpacity: 1,
                                    tempPadding: 33,
                                    tempBorderWidth: 3,
                                    size: "small",
                                    customSize: 32,
                                    hoverColorHex: "#239ddb",
                                    hoverColorOpacity: 1,
                                    hoverColorPalette: "color8",
                                    mobilePadding: 20
                                  }
                                }
                              ],
                              showOnMobile: "on"
                            }
                          }
                        ],
                        verticalAlign: "center",
                        borderWidth: 2,
                        borderTopWidth: 2,
                        borderRightWidth: 2,
                        borderBottomWidth: 2,
                        borderLeftWidth: 2,
                        tempBorderWidth: 4,
                        tempBorderTopWidth: 4,
                        tempBorderRightWidth: 4,
                        tempBorderBottomWidth: 4,
                        tempBorderLeftWidth: 4,
                        borderColorOpacity: 0.28,
                        borderRadius: 6,
                        borderTopLeftRadius: 6,
                        borderTopRightRadius: 6,
                        borderBottomLeftRadius: 6,
                        borderBottomRightRadius: 6,
                        borderColorPalette: "color8",
                        borderColorHex: "#ffffff",
                        mobilePaddingRight: 10,
                        mobilePaddingLeft: 10,
                        tempBorderColorOpacity: 0.28,
                        tempBorderRadius: 0,
                        tempBorderTopLeftRadius: 0,
                        tempBorderTopRightRadius: 0,
                        tempBorderBottomRightRadius: 0,
                        tempBorderBottomLeftRadius: 0,
                        bgColorOpacity: 0,
                        margin: 35,
                        marginSuffix: "px",
                        marginTop: 0,
                        marginRight: 100,
                        marginBottom: 0,
                        marginLeft: 100,
                        marginType: "ungrouped",
                        marginRightSuffix: "px",
                        marginLeftSuffix: "px",
                        marginTopSuffix: "px",
                        marginBottomSuffix: "px",
                        tabletTabsState: "",
                        tabletTabsColor: "",
                        tabletBorderColorPalette: "color2",
                        tempTabletBorderColorPalette: "color2",
                        tabletBorderColorOpacity: 0.28,
                        tabletBorderStyle: "solid",
                        tabletBorderWidth: 2,
                        tabletBorderRadius: 6,
                        tabletPaddingRight: 15,
                        tabletPaddingLeft: 15
                      }
                    }
                  ],
                  size: 92
                }
              }
            ],
            containerType: "fullWidth",
            bgColorPalette: "color5",
            bgColorHex: "#bde1f4",
            bgColorOpacity: 0.46,
            bgImageWidth: 1920,
            bgImageHeight: 1280,
            bgImageSrc: "180f0ec1ed5e48a4a108c8c2bd1575b9.jpg",
            bgPositionX: 54,
            bgPositionY: 44,
            tempBgColorOpacity: 0.46,
            tempMobileBgColorOpacity: 0.9,
            paddingType: "ungrouped",
            paddingTop: 150,
            paddingBottom: 150,
            padding: 75,
            mobilePaddingRightSuffix: "px",
            tempMobilePaddingRightSuffix: "px",
            mobilePaddingSuffix: "px",
            tempMobilePaddingSuffix: "px",
            mobilePaddingRight: 15,
            tempMobilePaddingRight: 15,
            mobilePaddingLeftSuffix: "px",
            tempMobilePaddingLeftSuffix: "px",
            mobilePaddingLeft: 15,
            tempMobilePaddingLeft: 15
          }
        }
      ]
    }
  }
};
