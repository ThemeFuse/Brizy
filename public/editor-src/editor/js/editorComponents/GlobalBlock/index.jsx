...
class GlobalBlock extends EditorComponent {
  ...
  optionalSCU(nextProps) {
    ...
    const blockValue = fromJS(nextProps.globalBlocks[_id]?.data.value);
    const oldBlockValue = fromJS(props.globalBlocks[_id]?.data.value);

    if (!oldBlockValue.equals(blockValue)) {
      return true;
    }
