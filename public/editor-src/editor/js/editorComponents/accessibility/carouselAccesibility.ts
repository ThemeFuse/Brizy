...
export class CarouselAccessibility {
  ...
  private setupRegionAccessibility(): void {
    ...
    slider.setAttribute("id", `brz-carousel-${this.instanceId}`);
    slider.setAttribute("role", "region");
    slider.setAttribute("aria-roledescription", "carousel");
    slider.setAttribute("aria-label", `Carousel ${this.instanceId + 1}`);
  }
  ...
  private setupArrowAccessibility(): void {
    ...
    arrows.forEach(({ element, label }) => {
      ...
      element.setAttribute("aria-controls", `brz-carousel-${this.instanceId}`);
  ...
  private setupDotsAccessibility(): void {
    ...
    this.getDotButtons().forEach((button, index) => {
      ...
      button.setAttribute("aria-controls", `brz-carousel-slide-${this.instanceId}-${index}`);
    });
  }
  ...
  private setupPauseControlAccessibility(): void {
    ...
    pauseButton.setAttribute("aria-controls", `brz-carousel-${this.instanceId}`);
  }
  ...
  private syncSlides(): void {
    ...
    slides.forEach((slide, index) => {
      ...
      slide.setAttribute("id", `brz-carousel-slide-${this.instanceId}-${index}`);
  ...
  private syncSlideFocusableElements(
    ...
    slide
      ...
      .forEach((element) => {
        const savedTabIndex = element.dataset.brzCarouselTabindex;
        ...
        if (isActive) {
          if (savedTabIndex !== undefined) {
            ...
            delete element.dataset.brzCarouselTabindex;
          }
        ...
        if (savedTabIndex === undefined) {
          element.dataset.brzCarouselTabindex =
