...
export default class Triggers extends React.Component {
  ...
  handleChange = () => this.props.onChange(this.state.values);
  ...
  handleTriggerChange(oldTriggerIndex, newName) {
    ...
    const { defaultValue } = this.props.items.find(({ id }) => id === newName);
  ...
  render() {
    ...
    const content = values.map((trigger, index) => {
      const { Component, ...item } =
        this.props.items.find(({ id }) => id === trigger.id) || {};
      ...
      return (
        <ItemWrapper
          showTypeButton={this.props.showTypeButton}
          key={index}
