...
import {
  ...
  SidebarAlign,
  allowedDrawerComponents
} from "visual/redux/types";
...
export function SidebarWrapper({ options }: SidebarProps): JSX.Element {
  ...
  const isOpen =
    !!drawerContentType &&
    allowedDrawerComponents.includes(
      drawerContentType as AllowedDrawerComponentTypes
    );
  ...
  return (
    <Sidebar alignment={align} isOpen={isOpen}>
...
export function SidebarBody({ children, className }: SidebarBodyProps) {
  return (
    <div className={classNames("brz-ed-sidebar-drawer__body", className)}>
      {children}
    </div>
  );
}
