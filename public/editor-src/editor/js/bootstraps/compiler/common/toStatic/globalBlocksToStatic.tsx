...
import { globalBlocksRawInPageSelector } from "visual/redux/selectors";
...
interface Props {
  store: Store;
  compiledBlocks: Array<GlobalBlock>;
  config: ConfigCommon;
}
...
export const globalBlocksToStatic = async (
  ...
  const pageBlocks = globalBlocksRawInPageSelector(reduxState)
    .filter(({ uid }) => compiledBlocks.find((block) => block.uid === uid))
