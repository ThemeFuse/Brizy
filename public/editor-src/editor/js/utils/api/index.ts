...
export const getDynamicContent: GetDynamicContent = ({
  ...
  qs.append("post_id", postId ?? Config.get("page").id);
...
export const getDynamicContent2: GetDynamicContent2 = async ({
  placeholders,
  signal
}) => {
  const apiUrl = Config.get("urls").api;
  ...
  for (const [postId, placeholders_] of Object.entries(placeholders)) {
    if (placeholders_) {
      for (const p of placeholders_) {
        qs.append(`p[${postId}][]`, p);
      }
    }
  }

  const r = await request2(
    `${apiUrl}/projects/${projectId}/placeholders_bulk?${qs}`,
    {
      method: "GET",
      signal
    }
