...
describe("Testing cssStyleBorder that should return CSS for border", () => {
  ...
  test("Grouped width", () => {
    expect(cssStyleBorder({ meta: { isNoEmptyGrouped: true }, value })).toBe(
      "border:5px solid rgba(255, 0, 0, 1);"
    );
  });

  test("Ungrouped width", () => {
    expect(cssStyleBorder({ meta: { isNoEmptyUngrouped: true }, value })).toBe(
      "border-width:5px 5px 5px 5px;border-style:solid;border-color:rgba(255, 0, 0, 1);"
    );
  });

  test("Without neta", () => {
    expect(cssStyleBorder({ meta: {}, value })).toBe("");
  });
});
