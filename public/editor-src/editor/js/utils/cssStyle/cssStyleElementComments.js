...
export function cssStyleElementCommentsHeightNameDate({ v, device }) {
  const nameLineHeight = styleTypography2LineHeight({
    v,
    device,
    prefix: "name"
  });

  const nameFontSize = styleTypography2FontSize({
    v,
    device,
    prefix: "name"
  });

  const dateLineHeight = styleTypography2LineHeight({
    v,
    device,
    prefix: "date"
  });

  const dateFontSize = styleTypography2FontSize({
    v,
    device,
    prefix: "date"
  });

  const replyLineHeight = styleTypography2LineHeight({
    v,
    device,
    prefix: "reply"
  });

  const replyFontSize = styleTypography2FontSize({
    v,
    device,
    prefix: "reply"
  });
  ...
  const height = Math.max(
    ...
    dateLineHeight * dateFontSize,
    replyLineHeight * replyFontSize
  );

  return nameLineHeight === undefined ||
    nameFontSize === undefined ||
    dateLineHeight === undefined ||
    dateFontSize === undefined ||
    replyLineHeight === undefined ||
    replyFontSize === undefined
    ? ""
    : `min-height: ${height}px;`;
}

export function cssStyleElementCommentsDviderReplyHeight({ v, device }) {
  const lineHeightReply = styleTypography2LineHeight({
    v,
    device,
    prefix: "reply"
  });

  const fontSizeReply = styleTypography2FontSize({
    v,
    device,
    prefix: "reply"
  });

  const height = ((lineHeightReply * fontSizeReply) / 100) * 77;

  return lineHeightReply === undefined || fontSizeReply === undefined
    ? ""
    : `height: ${height}px;`;
}
