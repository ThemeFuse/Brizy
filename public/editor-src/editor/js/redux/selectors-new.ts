...
// Retrieve only global blocks with their data;
// used for the compiler in the browser to compile every global block.
export const globalBlocksRawInPageSelector = createSelector(
  blocksOrderSelector,
  globalBlocksSelector,
  blocksDataSelector,
  (blockOrders, globalBlocks, blocksData) => {
    return blockOrders
      .filter((uid) => globalBlocks[uid])
      .map((uid) => ({ uid, data: blocksData[uid] }));
  }
);
