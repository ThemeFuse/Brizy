...
describe("buildHandler", () => {
  ...
  it.each<{ label: string; containerId: unknown }>([
    { label: "missing", containerId: undefined },
    { label: "empty string", containerId: "" },
    { label: "non-string", containerId: 123 }
  ])(
    "add — rejects $label containerId without calling addElement",
    ({ containerId }) => {
      ...
      const result = buildHandler(config, deps)({ containerId, height: 50 });
  ...
  it.each<{ label: string; containerId: unknown }>([
    ...
    "addNoProps — rejects $label containerId without calling addElement",
    ({ containerId }) => {
      ...
      const result = buildHandler(config, deps)({ containerId });
