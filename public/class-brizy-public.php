...
class Brizy_Public {
	...
	public static function render( $view, array $args = array() ) {
		Brizy_View::render( self::path( "views/$view" ), $args );
	}
	...
	function _action_load_editor( $content ) {
		...
		return Brizy_View::get( self::path( 'views/template' ) );
	}
	...
	public function _action_register_static() {
		...
		try {
			Brizy_Editor::get( Brizy_Post::get( get_the_ID() ) )->load();
		} catch ( Exception $exception ) {
			return;
		}
	...
	public function _filter_parse_content_for_images( $content ) {
		...
		$image = $matches[0];
		$new   = Brizy_Media_Upload::upload( $image );
