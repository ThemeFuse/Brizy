...
const actionsReader = parseStrict<PLUGIN_ENV["actions"], Actions>({
  ...
  getAccountProperties: pipe(
    mPipe(Obj.readKey("getAccountProperties"), Str.read),
    onNullish("brizy_get_account_properties")
  ),
  authenticateIntegration: pipe(
    mPipe(Obj.readKey("authenticateIntegration"), Str.read),
    onNullish("brizy_authenticate_integration")
  ),
  createIntegrationGroup: pipe(
    mPipe(Obj.readKey("createIntegrationGroup"), Str.read),
    onNullish("brizy_create_group")
  )
});
