...
class Brizy_Public_AssetEnqueueManager {
	...
	public function enqueueStyles() {
		...
		foreach ( $this->posts as $editorPost ) {
			...
			if(count($postGroups)==0) continue;

			foreach ( $postGroups['freeStyles'] as $i => &$aGroup ) {
				$this->replacePlaceholders( $aGroup, $editorPost->getWpPost(), 'head' );
			}

			$proGroups = apply_filters('brizy_pro_head_assets', $postGroups, $editorPost);

			foreach ( $proGroups as $i => &$aGroup ) {
				$this->replacePlaceholders( $aGroup, $editorPost->getWpPost(), 'head' );
			}

			$styles = array_merge( $styles, array_merge($postGroups['freeStyles'],$proGroups) );
		}
	...
	public function enqueueScripts() {
		...
		foreach ( $this->posts as $editorPost ) {
			...
			$proGroups = apply_filters('brizy_pro_body_assets', $postGroups, $editorPost);

			foreach ( $proGroups as $i => &$aGroup ) {
				$this->replacePlaceholders( $aGroup, $editorPost->getWpPost(), 'body' );
			}

			$assets = array_merge( $assets, array_merge($postGroups['freeScripts'],$proGroups) );
		}
