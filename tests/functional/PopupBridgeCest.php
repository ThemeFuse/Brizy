...
class PopupBridgeCest {
	...
	public function testImport( FunctionalTester $I ) {
		...
		$client->getPopups( [ 'filter' => [ 'uid' => $popupId ] ] )
		       ->willReturn( [ $fakePopup ] )
		       ->shouldBeCalled();

		$bridge = new Brizy_Admin_Cloud_PopupBridge( $client->reveal() );
		$bridge->import( $popupId );
	...
	public function testImportInvalidMediaBlock( FunctionalTester $I ) {
		$prophet        = new Prophet();
		$popupObserver = $prophet->prophesize( Brizy_Editor_Popup::class );
		$popupObserver->getMedia()->willReturn( json_encode( (object) [] ) )->shouldBeCalled();
		$client = $this->getCloudClientObserver();

		$bridge = new Brizy_Admin_Cloud_PopupBridge( $client->reveal() );

		$I->expectThrowable( 'Exception', function () use ( $bridge, $popupObserver ) {
