...
class Brizy_Editor_API {
	...
	public function get_globals() {
		try {
			$this->authorize();
			$id = $this->param( 'id' );
			$this->success( self::create_post_globals( Brizy_Post::get( $id ) ) );
		} catch ( Exception $exception ) {
			$this->error( $exception->getCode(), $exception->getMessage() );
			exit;
		}
	...
	public function set_globals() {
		try {
			$this->authorize();
			$post = Brizy_Post::get( $this->param( 'id' ) );
	...
	public function media() {
		try {
			$this->authorize();
			$project       = Brizy_Post::get( $this->param( 'id' ) );
			$attachment_id = $this->param( 'attachmentId' );

			$this->success( Brizy_User::get()->get_media_id(
				new Brizy_API_Project(
					$project->get_id(),
					$project->get_page_id()
				),
				$attachment_id
	...
	protected function param( $name ) {
		...
		throw new Brizy_Exception_Not_Found( "Parameter '$name' is missing", 400 );
	}
	...
	public static function create_post_arr( Brizy_Post $post ) {
		return array(
	...
	public static function create_post_globals( Brizy_Post $post ) {
		return array(
