...
class Brizy_Admin_Symbols_Manager
{
    ...
    public function getByUID($uid)
    {
        ...
        global $wpdb;
        $row = $wpdb->get_row($wpdb->prepare("SELECT ID,post_status,post_content FROM $wpdb->posts WHERE post_type=%s and  post_excerpt=%s", Brizy_Admin_Symbols_Main::CP_SYMBOL, $uid));
        if ($row) {
            $jsonObj = json_decode(base64_decode($row->post_content));
            $symbol = Brizy_Admin_Symbols_Symbol::createFromJsonObject($jsonObj, $row->ID);
            $symbol->setStatus($row->post_status);
    ...
    public function getByUIDs(array $uids)
    {
        ...
        if (!empty($missing)) {
            ...
            $rows = $wpdb->get_results($wpdb->prepare("SELECT ID, post_status, post_content, post_excerpt FROM $wpdb->posts WHERE post_type=%s AND post_excerpt IN ($placeholders)", $params));
            foreach ($rows as $row) {
                $jsonObj = json_decode(base64_decode($row->post_content));
                if ($jsonObj) {
                    $symbol = Brizy_Admin_Symbols_Symbol::createFromJsonObject($jsonObj, $row->ID);
    ...
    public function createFromJson($jsonString)
    {
        ...
        foreach ($objects as $obj) {
            if (!is_null($obj)) {
                ...
                $new = Brizy_Admin_Symbols_Symbol::createFromJsonObject($obj, $dbOne ? $dbOne->getId() : null);
                if ($dbOne && empty($obj->status ?? null)) {
                    $new->setStatus($dbOne->getStatus());
                }
                $result[] = $new;
            }
        }
    ...
    {
        global $wpdb;
        $rows = $wpdb->get_results($wpdb->prepare("SELECT ID,post_status,post_content,post_excerpt FROM $wpdb->posts WHERE post_type=%s", Brizy_Admin_Symbols_Main::CP_SYMBOL));
        $symbols = [];
        foreach ($rows as $symbolRow) {
            ...
            if ($symbolJsonObj) {
                ...
                $symbol->setStatus($symbolRow->post_status);
        ...
        try {
            foreach ((array)$symbols as $symbol) {
                if ($symbol instanceof Brizy_Admin_Symbols_Symbol) {
                    ...
                    if ($symbol->getId()) {
                        $locked = $wpdb->get_row($wpdb->prepare("SELECT post_content FROM $wpdb->posts WHERE ID=%d AND post_type=%s FOR UPDATE", $symbol->getId(), Brizy_Admin_Symbols_Main::CP_SYMBOL));
                        if (!$locked) {
    ...
    public function validateSymbol($symbol)
    {
        ...
        if (is_null($status) || empty($status) || (!is_null($status) && !in_array($status, [
                    'publish',
                    'draft'
                ], true))) {
            throw new Exception('Invalid symbol status. Allowed: publish, draft.');
        }
