...
class Brizy_Admin_Blocks_Api extends Brizy_Admin_AbstractApi
{
    ...
    public function actionDownloadBlock()
    {
        ...
        if (!$this->param('uid')) {
            $this->error(400, 'Invalid block uid');
        }
    ...
    public function actionUploadBlock()
    {
        try {
            ...
            if (isset($uploadedFile['file'])) {
                $zip = new Brizy_Editor_Zip_Archiver(Brizy_Editor_Project::get(), new Brizy_Admin_Fonts_Manager(), BRIZY_EDITOR_VERSION);
                $block = $zip->createBlockFromZip($uploadedFile['file']);
