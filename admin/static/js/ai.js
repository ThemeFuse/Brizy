document.addEventListener('DOMContentLoaded', function () {
    ...
    function generateTemplate(sessionId) {
        ...
        const ajaxData = {
            action: Brizy_Admin_Data.prefix + Brizy_Admin_Data.aiActions.generateTemplate,
    ...
    function importContentDelete(responseData) {
        ...
        const ajaxData = {
            action: Brizy_Admin_Data.prefix + Brizy_Admin_Data.aiActions.importDelete,
            hash: Brizy_Admin_Data.aiNonce,
        ...
        fetch(Brizy_Admin_Data.url, {
            ...
            .then(response => {
                if (response && response.success) {
                    if (response.data && response.data.editPageUrl) {
                        console.log(response);
                        const editPageUrl = response.data.editPageUrl;
    ...
    function importContentKeep(responseData) {
        ...
        const ajaxData = {
            action: Brizy_Admin_Data.prefix + Brizy_Admin_Data.aiActions.importKeep,
            hash: Brizy_Admin_Data.aiNonce,
            body: JSON.stringify(responseData)
        };
    ...
    function createSession(buttonElement) {
        ...
        const ajaxData = {
            action: Brizy_Admin_Data.prefix + Brizy_Admin_Data.aiActions.createSession,
            hash: Brizy_Admin_Data.aiNonce
        };
    ...
    function typeGenerateModal(response) {
        ...
        if (!modalElement || !modalContentContainer || !installTemplate) {
            return;
        }
    ...
    function handleDeleteClick(event) {
        ...
        if (!responseData || !responseData.data || !responseData.data.aiUrl) {
            return;
        }

        saveContentType('delete');

        const aiUrl = responseData.data.aiUrl + '/wp-admin/admin.php?page=starter-templates';
    ...
    function handleKeepClick(event) {
        ...
        const ajaxData = {
            action: Brizy_Admin_Data.prefix + Brizy_Admin_Data.aiActions.sendProject,
            hash: Brizy_Admin_Data.aiNonce,
